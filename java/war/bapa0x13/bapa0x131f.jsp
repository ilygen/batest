...
<html:html lang="true">
<head>
    <acl:setProgId progId="BAPA0X131F" />
    ...
    <html:javascript formName="DisabledReplacementRatioMaintListForm" page="1" />	          
</head>
<body scroll="no" >
...
<div id="mainContent">
    ...
    <div id="main" class="mainBody">
        <html:form action="/disabledReplacementRatioMaintList" method="post" onsubmit="return validateDisabledReplacementRatioMaintListForm(this);">
        
        <fieldset>
            <legend>&nbsp;失能年金所得替代率參數維護作業&nbsp;</legend>
            ...
            <table width="100%" border="0" align="center" cellpadding="2" cellspacing="2" class="px13">
                <tr>
                    <td colspan="8" align="right">
                        ...
                        <acl:checkButton buttonName="btnConfirm">
                            <input type="button" name="btnConfirm" class="button" value="新　增" onclick="$('page').value='1'; $('method').value='doInsert'; if (document.DisabledReplacementRatioMaintListForm.onsubmit()){document.DisabledReplacementRatioMaintListForm.submit();}else{return false;}" />&nbsp;
                        </acl:checkButton>
                        <acl:checkButton buttonName="btnBack">
                            <input type="button" name="btnBack" class="button" value="返　回" onclick="$('page').value='1'; $('method').value='doBack'; document.DisabledReplacementRatioMaintListForm.submit();" />&nbsp;
                        </acl:checkButton>                                             
                    </td>
                </tr>                
                ...
                <tr align="center">
                    <td>
                        <bean:define id="list" name="<%=ConstantKey.DISABLED_REPLACEMENT_RATIO_MAINT_QUERY_CASE_LIST%>" />
                        ...
                        <display:table name="pageScope.list" id="listItem" pagesize="<%=ConstantKey.LIST_PAGE_SIZE%>" class="px13">                            
                            ...
                            <display:column title="資料明細" style="width:15%; text-align:center;" class="issueinfo_C" headerClass="issuetitle_L">
                                <acl:checkButton buttonName="btnUpdate">
                                    <input type="button" class="button" name="btnUpdate" value="修　改" onclick="location.href='<c:url value="/disabledReplacementRatioMaintList.do"/>?method=doUpdate&rowNum=<c:out value="${listItem_rowNum}" />';">
                                </acl:checkButton>
                                <acl:checkButton buttonName="btnDelete">
                                	...
                                	<c:if test='${listItem.deleteUpdateStr ne "Y"}'>
                                        <input type="button" class="button" name="btnDelete" value="刪　除" onclick="if(confirm('是否確定刪除?')){location.href='<c:url value="/disabledReplacementRatioMaintList.do"/>?method=doDelete&rowNum=<c:out value="${listItem_rowNum}" />'};">                                                                                             
                                    </c:if>  
                                </acl:checkButton>                                
                            </display:column>
                        </display:table>
