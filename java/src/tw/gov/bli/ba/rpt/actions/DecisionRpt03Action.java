 ...
 * @author Noctis
 */
public class DecisionRpt03Action extends BaseDispatchAction {
    private static Log log = LogFactory.getLog(DecisionRpt03Action.class);

    private RptService rptService;

    /**
     * 列印作業 - 決行作業相關報表 - 歸檔清單點交清冊 - 查詢頁面 (balp0d230l.jsp) <br>
     * 按下「列印」的動作
     * 
     * @param mapping
     * @param form
    ...
    public ActionForward doReport(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        ...
        DecisionRpt03Form queryForm = (DecisionRpt03Form) form;

        try {
            // 抓報表資料
            List<DecisionRpt03Case> caseList = rptService.selectDataForDecisionRpt03(queryForm.getPayCode(), queryForm.getDecisionDate());
            ...
            if (caseList.size() == 0) {
                ...
                DecisionRpt03Report report = new DecisionRpt03Report();
                baoOutput = report.doReport(userData, caseList, queryForm.getDecisionDate());

                String printDate = DateUtility.getNowChineseDate();

                // 設定回傳回 Client 端之檔案大小, 若不設定,
                // 若 User 使用的瀏覽器為 IE 則可能收到空白的畫面
                response.setHeader("Content-disposition", "attachment; filename=\"DecisionRpt03_" + printDate + ".pdf" + "\"");
