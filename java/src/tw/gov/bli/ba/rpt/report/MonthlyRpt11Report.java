 ...
 * @author Rickychi
 */
public class MonthlyRpt11Report extends ReportBase {

    private RptService rptService;

    public MonthlyRpt11Report() throws Exception {
        super();
    }
    ...
    public Table getHeader(boolean bPrintMainHeader, String payCode, String payTypStr, String chkDate, String pageNum, String cPrnDate, String naChgMk, String nlWkMk, String adWkMk, String isNaChgMk) throws Exception {
        ...
        if (bPrintMainHeader) {
            ...
            addColumn(table, 99, 1, "退匯核定清單", fontCh10, 0, CENTER);
            if (!("").equals(payTypStr)) {
    ...
    public ByteArrayOutputStream doReport(UserBean userData, List<MonthlyRpt11Case> caseData, List<MonthlyRpt11Case> caseDataFooter, HashMap<String, Object> map) throws Exception {
        try {
            ...
            Map<String, List<MonthlyRpt11Case>> dataMap = new TreeMap<String, List<MonthlyRpt11Case>>();
            Map<String, List<MonthlyRpt11Case>> dataMapFooter = new TreeMap<String, List<MonthlyRpt11Case>>(); // 20120406
            for (int i = 0; i < caseData.size(); i++) {
                MonthlyRpt11Case obj = caseData.get(i);
                dataMap.put(obj.getPayTyp(), new ArrayList<MonthlyRpt11Case>());
            }
            ...
            for (int i = 0; i < caseData.size(); i++) { // 20120416
                MonthlyRpt11Case obj = caseData.get(i);
                dataMapFooter.put(obj.getPayTyp(), new ArrayList<MonthlyRpt11Case>());
            }

            for (int i = 0; i < caseDataFooter.size(); i++) { // 20120416
                MonthlyRpt11Case obj = caseDataFooter.get(i);
            ...
            for (Iterator it = dataMap.keySet().iterator(); it.hasNext();) {
                ...
                List<MonthlyRpt11Case> caseList = dataMap.get(key);
                MonthlyRpt11Case tmpCaseObj = caseList.get(0);

                // 拿取頁次 20130731
                List<MonthlyRpt11Case> caseListPageNum = dataMapFooter.get(key);
                ...
                for (MonthlyRpt11Case caseObj : caseList) {
                ...
                table.setBorderWidth(1);
                int rowCount = 0;
                List<MonthlyRpt11Case> caseList1 = dataMapFooter.get(key);
                for (MonthlyRpt11Case caseObj1 : caseList1) {
            ...
            printToDisk((PropertyHelper.getProperty("rpt.path") + Encode.forJava(payCode) + "_MonthlyRpt11_" + Encode.forJava(printDate) + ".pdf"), bao);
        }
        finally {
            document.close();
        }
