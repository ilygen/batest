...
<html:html lang="true">
<head>
    <acl:setProgId progId="BACO0D111A" />
    ...
    <div id="main" class="mainBody">
        ...
        <fieldset>
            ...
            <table width="98%" border="0" align="center" cellpadding="2" cellspacing="2" class="px13">
                <tr>
                    <td colspan="2" align="right">
                        <acl:checkButton buttonName="btnConfirm">
                            <input type="button" name="btnConfirm" class="button" value="確　認" onclick="$('page').value='1'; $('method').value='doSaveDataForAll'; if (checkFields()){document.DisabledPaymentReviewForm.submit();}else{return false;}" />&nbsp;
                        </acl:checkButton>
                        <acl:checkButton buttonName="btnBack">
                            <input type="button" name="btnBack" class="button" value="返　回" onclick="$('page').value='1'; $('method').value='doBack'; document.DisabledPaymentReviewForm.submit();" />&nbsp;
                        </acl:checkButton>                                             
                    </td>
                </tr>    
                <tr>
                    <td colspan="11" class="issuetitle_L">
                        <bean:define id="payTitle" name="<%=ConstantKey.DISABLED_PAYMENT_REVIEW_CASE_FOR_TITLE%>" />                        
                        <table width="100%" border="0" cellpadding="2" cellspacing="2" class="px13">
                            <tr>
                                <td width="25%">
                                    <c:if test="${payTitle.payKind eq '36'}">
                                        <span class="issuetitle_L_down">給付種類：</span>國併勞
                                    </c:if>
                                    <c:if test="${payTitle.payKind eq '38'}">
                                        <span class="issuetitle_L_down">給付種類：</span>勞併國
                                    </c:if>
                                    <c:if test="${(payTitle.payKind ne '36') and (payTitle.payKind ne '38')}">
                ...
                <tr align="center">
                    <td>
                        <bean:define id="list" name="<%=ConstantKey.DISABLED_PAYMENT_REVIEW_LIST%>" />
                        <display:table name="pageScope.list" id="listItem" pagesize="<%=ConstantKey.LIST_PAGE_SIZE%>">
                            <display:column title="<input type='checkbox' name='chkReview' onclick='selectAll(this, \"indexOfConfirm\");'>全選<br>審　核" style="width:7%; text-align:center;">
                                <c:if test="${(listItem.flag1 eq 'Y')}">
                                    <input type='checkbox' name='indexOfConfirm' value='<%=(listItem_rowNum.intValue()-1)%>'>
                                </c:if>
                                <c:if test="${(listItem.flag1 eq 'N')}">
                            ...
                            <display:column title="給付年月起迄" style="width:9%; text-align:center;">
                            ...
                            <display:column title="版 次" style="width:6%; text-align:center;">
                                <c:out value="${listItem.veriSeq}" />&nbsp;
                            </display:column>
                            <display:column title="資料別" style="width:6%; text-align:left;">
                            ...
                            <display:column title="資料明細" style="width:8%; text-align:center;">
                                <acl:checkButton buttonName="btnReview">
                                    <input type="button" name="btnReview" class="button" value="個案審核" onclick="$('page').value='1'; $('method').value='doSingleReview'; $('apNo').value='<c:out value="${listItem.apNo}" />'; document.DisabledPaymentReviewForm.submit();" />&nbsp;
                                </acl:checkButton>                            
                            </display:column>
                        </display:table>
                    </td>
