...
<html:html lang="true">
<head>
    <acl:setProgId progId="BAPA0X022C" />
                	...
                	<td>
                        ...
                        <display:column title="資料明細" style="width:14%; text-align:center;" >
                            <acl:checkButton buttonName="btnUpdate">
                                <input type="button" class="button" name="btnUpdate" value="修改" onclick="location.href='<c:url value="/adviceMaintList.do"/>?method=doUpdate&payCode=<c:out value="${listItem.payCode}" />&authTyp=<c:out value="${listItem.authTyp}" />';">
                            </acl:checkButton>
                        </display:column>
                        </display:table>
                    </td>
