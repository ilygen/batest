 ...
 * @author Noctis
 */
public class MonthCheckLAction extends BaseDispatchAction {
    private static Log log = LogFactory.getLog(MonthCheckLAction.class);
    ...
    // 給付別
    private static final String payCode = ConstantKey.BAAPPBASE_PAGE_PAYKIND_L;

    /**
     * 批次處理 - 線上月試核作業 - 老年年金線上月試核作業 - 查詢頁面 (baba0m080x.jsp) 按下 確認
     * 
     * @param mapping
     * @param form
     * @param request
    ...
    public ActionForward doQuery(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 批次處理 - 線上月試核作業 - 老年年金線上月試核作業 - 查詢頁面  MonthCheckLAction.doQuery() 開始 ... ");

        HttpSession session = request.getSession();
        UserBean userData = (UserBean) UserSessionHelper.getUserData(request);
        MonthCheckLForm iform = (MonthCheckLForm) form;

        try {

            // 查詢每個受理案件及處理狀態，如有符合訊息區顯示受理編號及處理狀態不可編審，並停留於原畫面。
            List<MonthCheckLCase> chkDataList = bjService.selectChkMonthLDataBy(iform.getApNoStr(), iform.getApNoAStr(), iform.getApNoBStr(), iform.getApNoCStr(), iform.getApNoDStr(), iform.getApNoEStr(), iform.getApNoFStr(), iform.getApNoGStr(), iform.getApNoHStr(), iform.getApNoIStr());

            if (chkDataList.size() > 0) {
                StringBuffer chkDataMsg = new StringBuffer();
                for (MonthCheckLCase caseData : chkDataList) {
            ...
            // 取得資料
            List<MonthCheckLCase> dataList = bjService.selectMonthCheckLDataListBy(payCode, iform.getApNoStr(), iform.getApNoAStr(), iform.getApNoBStr(), iform.getApNoCStr(), iform.getApNoDStr(), iform.getApNoEStr(), iform.getApNoFStr(), iform.getApNoGStr(), iform.getApNoHStr(), iform.getApNoIStr());
            ...
            if (dataList.size() < 1) {
                ...
                CaseSessionHelper.setMonthCheckLCaseList(dataList, request);
            ...
            // 設定查詢失敗訊息
            log.error("MonthCheckLAction.doQuery() 發生錯誤:" + ExceptionUtility.getStackTrace(e));
    ...
    /**
     * 批次處理 - 線上月試核作業 - 老年年金線上月試核作業 - 查詢頁面 (baba0m080x.jsp) 按下 查詢
     * 
     * @param mapping
     * @param form
     * @param request
    ...
    public ActionForward doQueryList(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 批次處理 - 線上月試核作業 - 老年年金線上月試核作業 - 查詢頁面  MonthCheckLAction.doQueryList() 開始 ... ");
        ...
        try {
            ...
            List<MonthCheckLCase> dataList = bjService.selectMonthCheckLQueryDataListBy(payCode, DateUtility.changeChineseYearMonthType(iform.getIssuYm()));

            if (dataList.size() < 1) {

                saveMessages(session, DatabaseMessageHelper.getNoResultMessage());
                log.debug("批次處理 - 線上月試核作業 - 老年年金線上月試核作業 - 查詢頁面  MonthCheckLAction.doQueryList() 完成 ... ");
            ...
            // 設定查詢失敗訊息
            log.error("MonthCheckLAction.doQueryList() 發生錯誤:" + ExceptionUtility.getStackTrace(e));
            saveMessages(session, DatabaseMessageHelper.getQueryFailMessage());
            return mapping.findForward(ConstantKey.FORWARD_FAIL);
        }
    }
    ...
    /**
     * 批次處理 - 線上月試核作業 - 老年年金線上月試核作業 - 清單頁面 (baba0m081q.jsp) 按下 確認
     * 
     * @param mapping
     * @param form
     * @param request
    ...
    public ActionForward doConfirm(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 批次處理 - 線上月試核作業 - 老年年金線上月試核作業 - 清單頁面  MonthCheckLAction.doConfirm() 開始 ... ");
        ...
        try {
            ...
            // 取得清單資料
            List<MonthCheckLCase> dataList = CaseSessionHelper.getMonthCheckLCaseList(request);
            ...
            for (int i = 0; i < tmpIndexList.length; i++) {
                ...
                if (StringUtils.isNotBlank(dataIndex)) {
                    ...
                    MonthCheckLCase caseObj = (MonthCheckLCase) dataList.get(Integer.parseInt(dataIndex));
                    ...
                    if (checkCount1 > 0 || checkCount2 > 0) {
                        ...
                        // 更新 勞保年金線上作業檔
                        bjService.insertBaonlinejobDataForMonthCheckL(caseObj, issuYm, procMsg, userData);
                    }
            ...
            // 更新頁面資料
            List<MonthCheckLCase> dataListNew = new ArrayList<MonthCheckLCase>();

            for (int i = 0; i < apNoList.size(); i++) {
                List<MonthCheckLCase> caseData = bjService.selectMonthCheckLApprovedDataListBy(apNoList.get(i), DateUtility.changeChineseYearMonthType(iform.getIssuYm()));
            ...
            //
            // saveMessages(session, DatabaseMessageHelper.getNoResultMessage());
            // log.debug("批次處理 - 線上月試核作業 - 老年年金線上月試核作業 - 清單頁面  MonthCheckLAction.doConfirm() 完成 ... ");
            // return mapping.findForward(FORWARD_QUERY_LIST_FAIL);
            //
            // }else{
            ...
            // 將 查詢結果清單 存到 Request Scope
            CaseSessionHelper.setMonthQueryCaseForTitle(caseData, request);
            CaseSessionHelper.setMonthCheckLCaseList(dataListNew, request);
            ...
            // 設定查詢失敗訊息
            log.error("MonthCheckLAction.doConfirm() 發生錯誤:" + ExceptionUtility.getStackTrace(e));
            saveMessages(session, DatabaseMessageHelper.getQueryFailMessage());
            return mapping.findForward(ConstantKey.FORWARD_FAIL);
        }
    }
    ...
    /**
     * 批次處理 - 線上月試核作業 - 老年年金線上月試核作業 - 清單頁面 (baba0m082a.jsp) 按下 確認
     * 
     * @param mapping
     * @param form
     * @param request
    ...
    public ActionForward doConfirmList(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 批次處理 - 線上月試核作業 - 老年年金線上月試核作業 - 清單頁面  MonthCheckLAction.doConfirmList() 開始 ... ");
        ...
        try {
            ...
            for (int i = 0; i < tmpIndexList.length; i++) {
                ...
                if (StringUtils.isNotBlank(dataIndex)) {
                    ...
                    if (checkCount > 0) {
                        ...
                        // 更新 勞保年金線上作業檔
                        bjService.updateBaonlinejobDataForMonthCheckL(caseObj, issuYm, procMsg, userData);
                    }
            ...
            // 更新頁面資料
            // List<MonthCheckLCase> dataListNew = bjService.selectMonthCheckLQueryDataListBy(payCode, DateUtility.changeChineseYearMonthType(iform.getIssuYm()));

            // if(dataListNew.size() < 1){
            //
            // saveMessages(session, DatabaseMessageHelper.getNoResultMessage());
            // log.debug("批次處理 - 線上月試核作業 - 老年年金線上月試核作業 - 清單頁面  MonthCheckLAction.doConfirmList() 完成 ... ");
            // return mapping.findForward(FORWARD_QUERY_LIST_FAIL);
            //
            // }else{
            ...
            // 設定查詢失敗訊息
            log.error("MonthCheckLAction.doConfirmList() 發生錯誤:" + ExceptionUtility.getStackTrace(e));
            saveMessages(session, DatabaseMessageHelper.getQueryFailMessage());
            return mapping.findForward(ConstantKey.FORWARD_FAIL);
        }
    }
    ...
    /**
     * 批次處理 - 線上月試核作業 - 老年年金線上月試核作業 - 清單頁面 (baba0m082a.jsp) 按下 刪除
     * 
     * @param mapping
     * @param form
     * @param request
    ...
    public ActionForward doDelete(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 批次處理 - 線上月核定作業 - 老年年金線上月核定作業 - 清單頁面  MonthCheckLAction.doDelete() 開始 ... ");
        ...
        try {
            ...
            for (int i = 0; i < tmpIndexList.length; i++) {
                ...
                if (StringUtils.isNotBlank(dataIndex)) {
                    ...
                    bjService.updateBaonlinejobDataForMonthCheckLDelete(caseObj, userData);
            ...
            for (int i = 0; i < dataList.size(); i++) {
                for (String apNo : apNoList) {
                    if (dataList.get(i).getApNo().equals(apNo)) {
            ...
            log.debug("批次處理 - 線上月核定作業 - 老年年金線上月核定作業 - 清單頁面  MonthCheckLAction.doDelete() 完成 ... ");
            return mapping.findForward(FORWARD_QUERY_LIST_SUCCESS);

        }
        catch (Exception e) {
            // 設定查詢失敗訊息
            log.error("MonthCheckLAction.doDelete() 發生錯誤:" + ExceptionUtility.getStackTrace(e));
            saveMessages(session, DatabaseMessageHelper.getQueryFailMessage());
            return mapping.findForward(ConstantKey.FORWARD_FAIL);
        }
    }
    ...
    /**
     * 批次處理 - 線上月試核作業 - 老年年金線上月試核作業 - 資料頁面 (baba0m081q.jsp,baba0m082a.jsp) 按下 返回
     * 
     * @param mapping
     * @param form
     * @param request
    ...
    public ActionForward doBack(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 批次處理 - 線上月試核作業 - 老年年金線上月試核作業 - 返回 MonthCheckLAction.doBack() 開始 ... ");
        ...
        CaseSessionHelper.removeMonthCheckLCaseList(request);

        log.debug("執行批次處理 - 線上月試核作業 - 老年年金線上月試核作業 - 返回 MonthCheckLAction.doBack() 完成 ... ");
