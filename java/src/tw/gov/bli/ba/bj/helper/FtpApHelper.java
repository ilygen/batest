...
public class FtpApHelper {
    private static Log log = LogFactory.getLog(FtpApHelper.class);
    ...
    public ByteArrayOutputStream getZipFile(String fileName, String dirForDownload) {
        ...
        try {
            ...
            if (!FTPReply.isPositiveCompletion(reply)) {
                ftp.disconnect();
                log.error("FTP " + serverAddress + "連線失敗...");
                throw new RuntimeException("FTP 連線失敗...");
            }

            Boolean checkLogin = ftp.login(userId, DbToolsUtils.decrypt(userPass));
            if (!checkLogin) {
                log.error("FTP " + serverAddress + ":帳號密碼錯誤...");
            }
            else {
                log.error("FTP " + serverAddress + ":登入成功...");
            }
            ...
            if (!checkDir) {
                log.error("FTP " + serverAddress + ":資料夾切換失敗或無此資料夾...");
            }
            else {
                log.error("FTP " + serverAddress + ":進入目標資料夾...");
            }
            ftp.setFileType(FTP.BINARY_FILE_TYPE);

            Boolean success = ftp.retrieveFile(fileName, baoOutput);
            if (!success) {
                log.error("FTP " + serverAddress + ":" + Encode.forJava(fileName) + "無檔案");
            }
            else {
                log.error("FTP " + serverAddress + ":寫檔成功，回傳zip檔案...");
            }
            ...
            log.error("FtpHelper.getZipFile()" + serverAddress + " 發生錯誤:" + ExceptionUtility.getStackTrace(e));
            throw new RuntimeException("FTP 發生錯誤:" + StringUtils.defaultString(e.getMessage()));
        }
        finally {
            if (ftp.isConnected()) {
