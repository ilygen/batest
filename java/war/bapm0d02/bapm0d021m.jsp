...
<html:html lang="true">
<head>
    <acl:setProgId progId="BAPM0D021M" />
            ...
            <bean:define id="payTitle" name="<%=ConstantKey.PAYMENT_REPRINT_FORM%>" />
                <tr>
                    <td colspan="2" align="right">
                        ...
                        <c:if test="${payTitle.printDt ne 'show'}">
                    	<acl:checkButton buttonName="btnPrint">
                            <input name="btnConfirm" type="button" class="button" value="確　認" onclick="$('page').value='1'; $('method').value='doPrintFirst'; if (checkedFields()){document.PaymentReprintForm.submit();}else{return false;}"/>&nbsp;
                        </acl:checkButton>
                        </c:if>
                        <c:if test="${payTitle.printDt eq 'show'}">
                    	<acl:checkButton buttonName="btnComp">
                            <input name="btnComp" type="button" class="button" value="補　印" onclick="$('page').value='1'; $('method').value='doCompList'; if (checkedFields()){document.PaymentReprintForm.submit();}else{return false;}"/>&nbsp;
                        </acl:checkButton>
                        </c:if>
 				...
 				<tr align="center">
	                    <td>
	                        ...
	                        <display:table name="pageScope.list" id="listItem" >
	                            <display:column title="選取" style="width:5%; text-align:center;">
                                	<input type='checkbox' id="reChkCheckbox" name='reChkCheckbox' onclick="chooseChange(this)"  value='<c:out value="${listItem_rowNum-1}"/>' />
                            	</display:column>
	                            ...
	                            <display:column title="期數" style="width:8%; text-align:center;">
	                                <c:out value="${listItem.maxInterest}" />&nbsp;
	                            </display:column>
	                            <display:column title="列印日期" style="width:8%; text-align:center;">
