 ...
 * @author zehua
 */
public class OtherRpt06Report extends ReportBase {

    private RptService rptService;

    public OtherRpt06Report() throws Exception {
        super();
    }
    ...
    public Table getHeader(boolean bPrintMainHeader, String payCode, String chkDate, String pageNum, String naChgMk, String nlWkMk, String adWkMk, String isNaChgMk, String cPrnDate) throws Exception {
        ...
        if (bPrintMainHeader) {
            ...
            addColumn(table, 35, 1, "轉催收核定清單", fontCh12, 0, CENTER);
    ...
    public ByteArrayOutputStream doReport(UserBean userData, List<OtherRpt06Case> caseData, List<OtherRpt06Case> caseData1, List<OtherRpt06Case> caseDataFooter, List<OtherRpt06Case> caseDataFooter1, HashMap<String, Object> map) throws Exception {
        try {
            RptService rptService = (RptService) SpringHelper.getBeanById("rptService");

            String payCode = (String) map.get("payCode");
            String procYm = (String) map.get("procYm");

            String printDate = DateUtility.getNowChineseDate();
            // FileOutputStream bao = new FileOutputStream(PropertyHelper.getProperty("rpt.path") + Encode.forJava(payCode) + "_OtherRpt06_" + Encode.forJava(printDate) + ".pdf");
            ...
            Map<String, List<OtherRpt06Case>> dataMap = new TreeMap<String, List<OtherRpt06Case>>();
            Map<String, List<OtherRpt06Case>> dataMapFooter = new TreeMap<String, List<OtherRpt06Case>>(); // 20120406
            for (int i = 0; i < caseData.size(); i++) {
                OtherRpt06Case obj = caseData.get(i);
                dataMap.put(obj.getNaChgMk() + obj.getNlWkMk() + obj.getAdWkMk(), new ArrayList<OtherRpt06Case>());
            }
            ...
            for (int i = 0; i < caseData.size(); i++) { // 20120416
                OtherRpt06Case obj = caseData.get(i);
                dataMapFooter.put(obj.getNaChgMk() + obj.getNlWkMk() + obj.getAdWkMk(), new ArrayList<OtherRpt06Case>());
            }

            for (int i = 0; i < caseDataFooter.size(); i++) { // 20120416
                OtherRpt06Case obj = caseDataFooter.get(i);
            ...
            for (Iterator it = dataMap.keySet().iterator(); it.hasNext();) {
                ...
                List<OtherRpt06Case> caseList = dataMap.get(key);

                OtherRpt06Case tmpCaseObj = caseList.get(0);

                // 頁次
                // List<OtherRpt06Case> caseListPageNum = dataMapFooter.get(key);
                ...
                headtable = getHeader(true, payCode, tmpCaseObj.getChkDateStr(), String.valueOf(pageNumNoPf), "", tmpCaseObj.getNlWkMk(), tmpCaseObj.getAdWkMk(), "N", tmpCaseObj.getcPrnDateStr());
                ...
                for (OtherRpt06Case caseObj : caseList) {
                ...
                int rowCount = 0;
                List<OtherRpt06Case> caseList1 = dataMapFooter.get(key);
                for (OtherRpt06Case caseObj1 : caseList1) {
            ...
            // Map的Key 為 issuYm+payYm
            // Value則是同為該 payTyp + nlWkMk + adWkMk 下的所有資料
            Map<String, List<OtherRpt06Case>> dataMap1 = new TreeMap<String, List<OtherRpt06Case>>();
            Map<String, List<OtherRpt06Case>> dataMapFooter1 = new TreeMap<String, List<OtherRpt06Case>>(); // 20120406
            for (int i = 0; i < caseData1.size(); i++) {
                OtherRpt06Case obj = caseData1.get(i);
                dataMap1.put(obj.getNaChgMk() + obj.getNlWkMk() + obj.getAdWkMk(), new ArrayList<OtherRpt06Case>());
            }
            ...
            for (int i = 0; i < caseData1.size(); i++) { // 20120416
                OtherRpt06Case obj = caseData1.get(i);
                dataMapFooter1.put(obj.getNaChgMk() + obj.getNlWkMk() + obj.getAdWkMk(), new ArrayList<OtherRpt06Case>());
            }

            for (int i = 0; i < caseDataFooter1.size(); i++) { // 20120416
                OtherRpt06Case obj = caseDataFooter1.get(i);
            ...
            for (Iterator it = dataMap1.keySet().iterator(); it.hasNext();) {
                ...
                String key = (String) it.next();
                List<OtherRpt06Case> caseList = dataMap1.get(key);

                OtherRpt06Case tmpCaseObj = caseList.get(0);

                // 拿取頁次 20130731
                List<OtherRpt06Case> caseListPageNumSub = dataMapFooter1.get(key);
                ...
                table.setBorderWidth(1);
                int rowCount = 0;
                List<OtherRpt06Case> caseList1 = dataMapFooter1.get(key);
                for (OtherRpt06Case caseObj1 : caseList1) {
            ...
            printToDisk((PropertyHelper.getProperty("rpt.path") + Encode.forJava(payCode) + "_OtherRpt06_" + Encode.forJava(printDate) + ".pdf"), bao);
        }
        finally {
            document.close();
        }
