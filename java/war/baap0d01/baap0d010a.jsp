...
<%@ page import="tw.gov.bli.ba.receipt.forms.OldAgeAnnuityReceiptForm" %>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">
<html:html lang="true">
<head>
    <acl:setProgId progId="BAAP0D010A" />
    ...
    <html:javascript formName="OldAgeAnnuityReceiptQueryForm" page="1" />        
    <script language="javascript" type="text/javascript">
    <!--
    //頁面欄位值初始化       
    function initAll(){  
        <%if(request.getSession().getAttribute(ConstantKey.OLDAGE_ANNUITY_RECEIPT_QUERY_FORM) == null){%>     
            $("apNo1").value='L';
            disableInsertButton(false);
        <%}else{%>
            disableInsertButton(false);
            var apNo1 = '<c:out value ="<%=((OldAgeAnnuityReceiptForm)request.getSession().getAttribute(ConstantKey.OLDAGE_ANNUITY_RECEIPT_QUERY_FORM)).getApNo1()%>" />'     
            var apNo2 = '<c:out value ="<%=((OldAgeAnnuityReceiptForm)request.getSession().getAttribute(ConstantKey.OLDAGE_ANNUITY_RECEIPT_QUERY_FORM)).getApNo2()%>" />'     
            var apNo3 = '<c:out value ="<%=((OldAgeAnnuityReceiptForm)request.getSession().getAttribute(ConstantKey.OLDAGE_ANNUITY_RECEIPT_QUERY_FORM)).getApNo3()%>" />'     
            var apNo4 = '<c:out value ="<%=((OldAgeAnnuityReceiptForm)request.getSession().getAttribute(ConstantKey.OLDAGE_ANNUITY_RECEIPT_QUERY_FORM)).getApNo4()%>" />'     
            var evtIdnNo = '<c:out value ="<%=((OldAgeAnnuityReceiptForm)request.getSession().getAttribute(ConstantKey.OLDAGE_ANNUITY_RECEIPT_QUERY_FORM)).getEvtIdnNo()%>" />'     
            if(apNo1==""||apNo1=="null"){
                $("apNo1").value='L';
            }else{
                $("apNo1").value=apNo1;
...
<div id="mainContent">
    ...
    <div id="main" class="mainBody">
        <html:form action="/oldAgeAnnuityReceipt" method="post" onsubmit="return validateOldAgeAnnuityReceiptQueryForm(this);">
                ...
                <tr>
                    <td colspan="2" align="right">
                        <acl:checkButton buttonName="btnInsert">
                            <input type="button" id="btnInsert" name="btnInsert" tabindex="60" class="button" value="新　增" onclick="$('page').value='1'; $('method').value='prepareInsert'; document.OldAgeAnnuityReceiptQueryForm.submit();" disabled="true"/>&nbsp;
                        </acl:checkButton>
                        <acl:checkButton buttonName="btnModify">
                            <input type="button" id="btnModify" name="btnModify" tabindex="70" class="button" value="修　改" onclick="$('page').value='1'; $('method').value='prepareModify'; if (document.OldAgeAnnuityReceiptQueryForm.onsubmit() && checkFields()){document.OldAgeAnnuityReceiptQueryForm.submit();}else{return false;}" />&nbsp;
                        </acl:checkButton>
