...
<html:html lang="true">
<head>
    <acl:setProgId progId="BAMO0D082C" />
...
<body scroll="no">
    ...
    <div id="main" class="mainBody">
        <html:form action="/payeeDataUpdateList" method="post">
            ...
            <table width="98%" border="0" align="center" cellpadding="2" cellspacing="2" class="px13">
                <tr>
                    <td align="right" colspan="8">
                        ...
                        <acl:checkButton buttonName="btnQuery">
                            <input name="btnQuery" type="button" class="button" value="可領金額" onclick="location.href='<c:url value="/payeeDataUpdateList.do"/>?method=doQuery';">&nbsp;
                        </acl:checkButton>
                        <acl:checkButton buttonName="btnPrint">
                            <input name="btnPrint" type="button" class="button_120" value="檢視受理/審核清單" onclick="location.href='<c:url value="/printOldAgeReviewRpt01.do?method=doReport&apNo1Begin=${PayeeDataUpdateQueryCase[0].apNo1Str}&apNo2Begin=${PayeeDataUpdateQueryCase[0].apNo2Str}&apNo3Begin=${PayeeDataUpdateQueryCase[0].apNo3Str}&apNo4Begin=${PayeeDataUpdateQueryCase[0].apNo4Str}&apNo1End=${PayeeDataUpdateQueryCase[0].apNo1Str}&apNo2End=${PayeeDataUpdateQueryCase[0].apNo2Str}&apNo3End=${PayeeDataUpdateQueryCase[0].apNo3Str}&apNo4End=${PayeeDataUpdateQueryCase[0].apNo4Str}&action=single" />'">&nbsp;
                        </acl:checkButton>
                        <acl:checkButton buttonName="btnBack">
                            <input name="btnBack" type="button" class="button" value="返　回" onclick="$('page').value='1'; $('method').value='doBack'; document.PayeeDataUpdateListForm.submit();" />
                        </acl:checkButton>
                    </td>
                </tr>
                <tr>
                ...
                <tr align="center">
                    <td class="issuetitle_L">
                        ...
                        <display:column property="benName" title="受款人姓名" headerClass="issuetitle_L" style="width:26%; text-align:left;" />
                        <display:column property="benIdnNo" title="受款人身分證號" headerClass="issuetitle_L" style="width:15%; text-align:left;" />
                        <display:column property="benEvtRelName" title="關 係" headerClass="issuetitle_L" style="width:22%; text-align:left;" />
                        <display:column title="受款人可領金額" headerClass="issuetitle_L" style="width:9%; text-align:right;">
                        ...
                        <display:column title="資料明細" headerClass="issuetitle_L" style="width:21%; text-align:center;" >
                            <acl:checkButton buttonName="btnUpdate">
                                <input type="button" class="button" name="btnUpdate" value="修　改" onclick="location.href='<c:url value="/payeeDataUpdateList.do"/>?method=doUpdate&baappbaseId=<c:out value="${listItem.baappbaseId}" />';">
                            </acl:checkButton>
                            <acl:checkButton buttonName="btnDelete">
                                ...
                                <c:if test='${listItem.delButtonQ4 eq "0"}'>
                                    <input type="button" class="button" name="btnDelete" value="刪　除" onclick="if (confirm('<bean:message bundle="<%=Global.BA_MSG%>" key="msg.delete.confirm"/>')){location.href='<c:url value="/payeeDataUpdateList.do"/>?method=doDelete&baappbaseId=<c:out value="${listItem.baappbaseId}" />';}">
                                </c:if>
                            </acl:checkButton>
                            ...
                            <c:choose>
                                ...
                                <input type="button" class="button" name="btnCancel" value="改匯註銷" onclick="location.href='<c:url value="/payeeDataUpdateList.do"/>?method=doCancel&baappbaseId=<c:out value="${listItem.baappbaseId}" />';">
                                </c:when>
                                <c:otherwise>
