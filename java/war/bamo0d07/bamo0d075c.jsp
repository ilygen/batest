...
<html:html lang="true">
<head>
  <acl:setProgId progId="BAMO0D075C" />
    ...
    <script type="text/javascript">
        ...
        function resetForm() {
            document.DependantDataUpdateCompelDataForm.reset();
            initAll();
        }
...
<body scroll="no">
    ...
    <html:form action="/dependantDataUpdateCompelData" method="post">
      <fieldset>
              ...
              <tr>
                  <td align="right">
                      ...
                      <acl:checkButton buttonName="btnSave">
                          <input name="btnSave" type="button" class="button" value="新　增" onclick="$('page').value='1'; $('method').value='doSave'; if(checkRequireFields()){document.DependantDataUpdateCompelDataForm.submit();} else {return false;} "/>&nbsp;
                      </acl:checkButton>
                      <acl:checkButton buttonName="btnUpdate">
                          <input name="btnUpdate" type="button" class="button" value="確　定" onclick="$('page').value='1'; $('method').value='doUpdate'; if(checkRequireFields()){document.DependantDataUpdateCompelDataForm.submit();} else {return false;} " style="display: none;"/>&nbsp;
                      </acl:checkButton>
                      <acl:checkButton buttonName="btnClear">
                          <input name="btnClear" type="button" class="button" value="清　除" onclick="resetForm();">&nbsp;
                      </acl:checkButton>
                      <acl:checkButton buttonName="btnBack">
                          <input name="btnBack" type="button" class="button" value="返　回" onclick="$('page').value='1'; $('method').value='doBack'; document.DependantDataUpdateCompelDataForm.submit();"/>
                      </acl:checkButton>
                  </td>
              </tr>
              <tr>
                  <td>
                      <table width="100%" border="0" cellpadding="2" cellspacing="2" class="px13">
                          ...
                          <tr>
                          	<td colspan="2"><span class="issuetitle_L_down">眷屬姓名：</span><c:out value="${DependantDataUpdateDetailForm.famName}"/></td>
                            <td><span class="issuetitle_L_down">眷屬身分證號：</span><c:out value="${DependantDataUpdateDetailForm.famIdnNo}"/></td>
                            <td><span class="issuetitle_L_down">眷屬出生日期：</span><c:out value="${DependantDataUpdateDetailForm.famBrDate}"/></td>
                          </tr>
                      </table>
                  </td>
              </tr>
                      ...
                      <table width="98%" cellpadding="3" cellspacing="4" class="px13">
                          ...
                          <tr align="center">
                              <td colspan="4" class="issuetitle_L">
                                  <bean:define id="list" name="<%=ConstantKey.DEPENDANT_DATA_UPDATE_COMPELDATA_LIST_CASE%>"/>
                                  <display:table name="pageScope.list" id="listItem" pagesize="999">
                                      ...
                                      <display:column title="操作區" headerClass="issuetitle_L" class="issueinfo_C" style="width: 30%; text-align: center;">
                                        <input type="button" class="button_90" name="btnUpdate" value="修　改" onclick="$('compelNo').value='<c:out value="${listItem.compelNo}"/>'; $('compelDesc').value='<c:out value="${listItem.compelDesc}"/>'; toggleUpdateMode('<c:out value="${listItem_rowNum}"/>'); $('compelSdate').focus(); ">
                                              <input type="button" class="button_90" name="btnDelete" value="刪　除" onclick="$('method').value='doDelete'; $('compelNo').value='<c:out value="${listItem.compelNo}"/>'; document.DependantDataUpdateCompelDataForm.submit();">
                                      </display:column>
                                  </display:table>
                              </td>
                          </tr>
