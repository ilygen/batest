...
/**
 * 更正作業 - 死亡改匯處理作業 - 繼承人維護 (BAMO0D093A,BAMO0D094C)
 * 
 * @author Noctis
 */
public class PayeeDataUpdateDetailForOldAgeDeathRepayAction extends BaseDispatchAction {
    private static Log log = LogFactory.getLog(PayeeDataUpdateDetailForOldAgeDeathRepayAction.class);
    ...
    /**
     * 更正作業 - 死亡改匯處理作業 - 繼承人維護 - 新增頁面 (bamo0d093a.jsp) <br>
     * 按下「確認」的動作
     * 
     * @param mapping
     * @param form
    ...
    public ActionForward doSave(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 更正作業 - 死亡改匯處理作業 - 繼承人維護 - 新增頁面 PayeeDataUpdateDetailForOldAgeDeathRepayAction.doSave() 開始 ... ");
        ...
        // 取得查詢List
        List<BaappbaseDataUpdateCase> queryList = CaseSessionHelper.getPayeeDataUpdateQueryCaseList(request);
        BaappbaseDataUpdateCase queryData = (BaappbaseDataUpdateCase) queryList.get(0);
        //取得事故者資料
        BaappbaseDataUpdateCase caseData = updateService.selectDataForPayeeDataUpdateDetail(queryData.getApNo());
        ...
        try {
            ...
            if (detailForm != null) {
                ...
                // 存檔
                updateService.insertBaappbaseDataForOldAgeDeathRepayPayeeData(userData, caseData, queryData, detailData, sInterValMonth);
...
//                String returnCode = binding.doCheckMark(detailData.getApNo());
                ...
                List<BaappbaseDataUpdateCase> applyList = updateService.selectPayeeDataListForOldAgeDeathRepayBy(queryForm.getApNo1() + queryForm.getApNo2() + queryForm.getApNo3() + queryForm.getApNo4(),heirSeqNo,deathRepayqueryForm.getSeqNo());
                //把原受款人自List中移除
...
//                }
            ...
            log.debug("執行更正作業 - 死亡改匯處理作業 - 繼承人維護 - 新增頁面 PayeeDataUpdateDetailForOldAgeDeathRepayAction.doSave() 完成 ... ");

            return mapping.findForward(ConstantKey.FORWARD_SAVE_SUCCESS);
        }
        catch (Exception e) {
            // 設定查詢失敗訊息
            log.error("PayeeDataUpdateDetailForOldAgeDeathRepayAction.doSave() 發生錯誤:" + ExceptionUtility.getStackTrace(e));
    ...
    /**
     * 更正作業 - 死亡改匯處理作業 - 繼承人維護 - 修改頁面 (bamo0d094c.jsp) <br>
     * 按下「確認」的動作
     * 
     * @param mapping
     * @param form
    ...
    public ActionForward doConfirm(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 更正作業 - 死亡改匯處理作業 - 繼承人維護 - 修改頁面 PayeeDataUpdateDetailForOldAgeDeathRepayAction.doConfirm() 開始 ... ");
        ...
        try {
            ...
            if (detailForm != null) {
                ...
                if (caseData.getPayTyp().equals("5") || caseData.getPayTyp().equals("6")) {
                    if (!detailForm.getPayTyp().equals("5") && !detailForm.getPayTyp().equals("6")) {
                        // updateService.updateInterValMonth(queryData.getApNo(), "0");
                        interValMonth = "0";
                    }
                ...
                // 存檔
                updateService.updateBaappbaseDataForOldAgeDeathRepayPayeeData(userData, detailData, caseData, interValMonth);
...
//                    request.getSession().setAttribute(ConstantKey.PAYEE_RELATIION_OPTION_LIST, selectOptionService.getPayeeRelationOptionList2("typeOne", null, null, null, relationStatus));
//                }
//                else if (relationQ1 > 0 && relationQ2 <= 0) {
//                    request.getSession().setAttribute(ConstantKey.PAYEE_RELATIION_OPTION_LIST, selectOptionService.getPayeeRelationOptionList2(null, "typeTwo", null, null, relationStatus));
//                }
//                else if (relationQ1 <= 0 && relationQ2 <= 0) {
//                    request.getSession().setAttribute(ConstantKey.PAYEE_RELATIION_OPTION_LIST, selectOptionService.getPayeeRelationOptionList2(null, null, "typeThree", null, relationStatus));
//                }
//                else if (relationQ1 <= 0 && relationQ2 > 0) {
//                    request.getSession().setAttribute(ConstantKey.PAYEE_RELATIION_OPTION_LIST, selectOptionService.getPayeeRelationOptionList2(null, null, null, "typefour", relationStatus));
//                }
...
//                }
            ...
            log.debug("更正作業 - 死亡改匯處理作業 - 繼承人維護 - 修改頁面 PayeeDataUpdateDetailForOldAgeDeathRepayAction.doConfirm() 完成 ... ");

            return mapping.findForward(ConstantKey.FORWARD_UPDATE_SUCCESS);
        }
        catch (Exception e) {
            // 設定查詢失敗訊息
            log.error("PayeeDataUpdateDetailForOldAgeDeathRepayAction.doConfirm() 發生錯誤:" + ExceptionUtility.getStackTrace(e));
    ...
    /**
     * 更正作業 - 死亡改匯處理作業 - 繼承人維護 - 修改頁面 新增頁面 (bamo0d093a.jsp,bamo0d094c.jsp) <br>
     * 按下「返回」的動作
     * 
     * @param mapping
     * @param form
    ...
    public ActionForward doBackList(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 更正作業 - 死亡改匯處理作業 - 繼承人維護 - 修改頁面 PayeeDataUpdateDetailForOldAgeDeathRepayAction.doBackList() 開始 ... ");
        ...
        try {
            ...
            // 設定失敗訊息
            log.error("PayeeDataUpdateDetailForOldAgeDeathRepayAction.doBackList() 發生錯誤:" + ExceptionUtility.getStackTrace(e));
