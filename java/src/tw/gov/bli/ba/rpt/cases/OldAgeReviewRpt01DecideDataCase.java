...
/**
 * Case for 勞保老年年金給付受理編審清單 - 核定資料
 * 
 * @author Goston
 */
public class OldAgeReviewRpt01DecideDataCase implements Serializable {
    ...
    public String getOldRateSdateString() {
        if (StringUtils.length(oldRateSdate) == 6)
            return DateUtility.formatChineseDateString(DateUtility.changeDateType(oldRateSdate), false);
        else
    ...
    public String getOldRateEdateString() {
        if (StringUtils.length(oldRateEdate) == 6)
            return DateUtility.formatChineseDateString(DateUtility.changeDateType(oldRateEdate), false);
        else
            return StringUtils.defaultString(oldRateEdate);
    }

    public OldAgeReviewRpt01DecideDataCase() {
    }
