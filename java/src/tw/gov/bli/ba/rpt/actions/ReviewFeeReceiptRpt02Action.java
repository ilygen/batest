...
import tw.gov.bli.ba.rpt.forms.ReviewFeeReceiptRpt02Form;
import tw.gov.bli.ba.rpt.report.ReviewFeeReceiptRpt02Report;
...
/**
 * 列印作業 - 複檢費用申請作業 - 勞保失能年金給付複檢費用核定通知書 - 查詢頁面 (bare0d041l.jsp)
 * 
 * @author Evelyn Hsu
 */

public class ReviewFeeReceiptRpt02Action extends BaseDispatchAction {
    
    private static Log log = LogFactory.getLog(ReviewFeeReceiptRpt02Action.class);
    ...
    public ActionForward doReport(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        ...
        ReviewFeeReceiptRpt02Form queryForm = (ReviewFeeReceiptRpt02Form) form;
        ...
        try {
            ...
            List<ReviewFeeReceiptRpt02DataCase> caseList = rptService.getReviewFeeReceiptRpt02ListDataBy(apNo, queryForm.getApSeq());
            ...
            if (caseList.size() == 0) {
                ...
                ReviewFeeReceiptRpt02Report report = new ReviewFeeReceiptRpt02Report();
                baoOutput = report.doReport(userData, caseList);
                
                // 設定回傳回 Client 端之檔案大小, 若不設定,
                // 若 User 使用的瀏覽器為 IE 則可能收到空白的畫面
                response.setHeader("Content-disposition", "attachment; filename=\"BARE0D041L_" + printDate + ".pdf" + "\"");
