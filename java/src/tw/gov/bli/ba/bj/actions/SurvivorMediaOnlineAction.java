...
import tw.gov.bli.ba.bj.forms.SurvivorMediaOnlineForm;
 ...
 * @author ZeHua
 */
public class SurvivorMediaOnlineAction extends BaseDispatchAction {
    private static Log log = LogFactory.getLog(SurvivorMediaOnlineAction.class);
    ...
    public ActionForward doProduct(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            if (qryCheckBatch.compareTo(BigDecimal.ZERO) == 0) {
                int batchNum = bjService.doQueryBatchJobStatus(DateUtility.changeChineseYearMonthType(queryForm.getIssuYm()), DateUtility.changeDateType(queryForm.getChkDate()), PAYCODE_SURVIVOR, PROCTYPE_ONLINE_MEDIA);
                if (batchNum <= 0) {
                    String jobId = ScheduleHelper.scheduleNow(MediaOnlineJob.class, null, userData);
                    bjService.doScheduleBatchJob(jobId, DateUtility.changeChineseYearMonthType(queryForm.getIssuYm()), DateUtility.changeDateType(queryForm.getChkDate(), false), PAYCODE_SURVIVOR, userData.getEmpNo(), userData.getDeptId(),
                    ...
                    log.debug("批次處理 - 遺屬年金線上批次產製媒體檔作業-產製媒體檔-排程作業SurvivorMediaOnlineAction.doProduce() 完成 ... ");
                    ...
                    log.debug("批次處理 - 遺屬年金線上批次產製媒體檔作業-產製媒體檔-排程作業 SurvivorMediaOnlineAction.doProduce() 完成 ...(排程中已有相同資料) ");
                    return mapping.findForward(ConstantKey.FORWARD_SCHEDULE_SUCCESS);
                }
            ...
            // 設定查詢失敗訊息
            log.error("SurvivorMediaOnlineAction.doProduct() 發生錯誤:" + ExceptionUtility.getStackTrace(e));
    ...
    public ActionForward doDownloadFile(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            if (caseData == null) {
                ...
                log.debug("批次處理 -遺屬年金線上批次產製媒體檔作業-下載媒體檔- SurvivorMediaOnlineAction.doDownloadFile() 完成 ...(查無資料) ");
                return mapping.findForward(ConstantKey.FORWARD_SCHEDULE_SUCCESS);
            }
            else {
                CaseSessionHelper.setSurvivorMediaOnlineCaseList(caseData, request);
                log.debug("批次處理 -遺屬年金線上批次產製媒體檔作業-下載媒體檔- SurvivorMediaOnlineAction.doDownloadFile() 完成 ...");
                return mapping.findForward(ConstantKey.FOWARD_DOWN_FILE);
            }
        }
        catch (Exception e) {
            // 設定查詢失敗訊息
            log.error("SurvivorMediaOnlineAction.doDownloadFile() 發生錯誤:" + ExceptionUtility.getStackTrace(e));
            saveMessages(session, DatabaseMessageHelper.getQueryFailMessage());
            return mapping.findForward(ConstantKey.FOWARD_DOWN_FILE);
        }
    }
    ...
    public ActionForward doDownload(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            log.debug("執行 SurvivorMediaOnlineAction.doDownload() 完成 ...");

            return null;
        }
        catch (Exception e) {
            // 設定查詢失敗訊息
            log.error("SurvivorMediaOnlineAction.doDownload() 發生錯誤:" + ExceptionUtility.getStackTrace(e));
    ...
    public ActionForward doQueryProcess(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        ...
        SurvivorMediaOnlineForm queryForm = (SurvivorMediaOnlineForm) form;
        try {
            List<MediaOnlineProgressCase> caseData = bjService.getProgressStep(PROCTYPE_ONLINE_MEDIA, DateUtility.changeChineseYearMonthType(queryForm.getIssuYm()), DateUtility.changeDateType(queryForm.getChkDate()), PAYCODE_SURVIVOR,
                            ConstantKey.PAYSEQNO_1);
            if (caseData == null) {
                saveMessages(session, DatabaseMessageHelper.getNoDataMessage());
                log.debug("批次處理 - 遺屬年金線上批次產製媒體檔作業-進度查詢 - SurvivorMediaOnlineAction.doQueryProcess() 完成 ...(查無資料) ");
                return mapping.findForward(ConstantKey.FORWARD_SCHEDULE_SUCCESS);
            }
            else {
                CaseSessionHelper.setSurvivorMediaOnlineProgressList(caseData, request);
                log.debug("批次處理 - 遺屬年金線上批次產製媒體檔作業-進度查詢 - SurvivorMediaOnlineAction.doQueryProcess() 完成 ...");
                return mapping.findForward(ConstantKey.FOWARD_QRY_PROGRESS);
            }
        }
        catch (Exception e) {
            // 設定查詢失敗訊息
            log.error("SurvivorMediaOnlineAction.doQueryProcess() 發生錯誤:" + ExceptionUtility.getStackTrace(e));
            saveMessages(session, DatabaseMessageHelper.getQueryFailMessage());
            return mapping.findForward(ConstantKey.FORWARD_SCHEDULE_FAIL);
        }
    }
    ...
    public ActionForward doQryProgressDtl(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            if (caseData == null) {
                saveMessages(session, DatabaseMessageHelper.getNoDataMessage());
                log.debug("批次處理 - 遺屬年金線上批次處理狀況明細查詢  - SurvivorMediaOnlineAction.doQryProgressDtl() 完成 ...(查無資料) ");
                return mapping.findForward(ConstantKey.FOWARD_QRY_PROGRESS);
            }
            else {
                CaseSessionHelper.setSurvivorMediaOnlineProgressDtlList(caseData, request);
                log.debug("批次處理 - 遺屬年金線上批次處理狀況明細查詢  - SurvivorMediaOnlineAction.doQryProgressDtl() 完成 ...");
                return mapping.findForward(ConstantKey.FOWARD_QRY_PROGRESS_DTL);
            }
        }
        catch (Exception e) {
            // 設定查詢失敗訊息
            log.error("SurvivorMediaOnlineAction.doQryProgressDtl() 發生錯誤:" + ExceptionUtility.getStackTrace(e));
            saveMessages(session, DatabaseMessageHelper.getQueryFailMessage());
            return mapping.findForward(ConstantKey.FOWARD_QRY_PROGRESS);
        }
    }
    ...
    /**
     * 批次處理 - 失能年金線上批次產製媒體檔作業 - 媒體檔下載 - 傳送作業 (BABA0M151Q)
     *
     * @param mapping
     * @param form
     * @param request
    ...
    public ActionForward doUploadFile(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            if (result) {
                ...
                log.debug("批次處理 - 遺屬年金線上產製媒體檔作業 - 媒體檔下載 - 傳送作業 SurvivorMediaOnlineAction.doUploadFile() 完成 ...");
                return mapping.findForward(ConstantKey.FORWARD_SUCCESS_UPLOAD);
            } else {
                saveMessages(session, DatabaseMessageHelper.failUpload2());
                log.debug("批次處理 - 失能年金線上產製媒體檔作業 - 媒體檔下載 - 傳送作業 SurvivorMediaOnlineAction.doUploadFile() 失敗 ...");
                return mapping.findForward(ConstantKey.FORWARD_FAIL_UPLOAD);
            }
        } catch (Exception e) {
            log.error("批次處理 - 失能年金線上產製媒體檔作業 - 媒體檔下載 - 傳送作業 SurvivorMediaOnlineAction.doUploadFile() 失敗 ..." + ExceptionUtility.getStackTrace(e));
            saveMessages(session, DatabaseMessageHelper.failUpload2());
            return mapping.findForward(ConstantKey.FORWARD_FAIL_UPLOAD);
        }
    }
