 ...
 * @author frank
 */
public class OldAgePaymentRpt01Report extends ReportBase {
    ...
    public OldAgePaymentRpt01Report() throws Exception {
        super();
        nPage = 0;
...
//                }
            ...
            addColumn(table, 15, 1, "累計已領年金金額："+DecimalFormat.getNumberInstance().format(caseData.getAnnuAmt()).toString() , fontCh12, 0, LEFT);
            addColumn(table, 15, 1, "試算老年差額金："+DecimalFormat.getNumberInstance().format(caseData.getMarginAmt()).toString() , fontCh12, 0, LEFT);
            ...
            if(caseData.getBmChkAmt().compareTo(new BigDecimal(0)) == 0){
            	addColumn(table, 15, 1, "差額金金額：", fontCh12, 0, LEFT);// 差額金金額	
            }else{
