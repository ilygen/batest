 ...
 * @author Goston
 */
public class SurvivorCheckMarkLevelAdjustAction extends DispatchAction {
    private static Log log = LogFactory.getLog(SurvivorCheckMarkLevelAdjustAction.class);
    ...
    /**
     * 更正作業 - 編審註記程度調整 - 遺屬年金編審註記程度調整 - 查詢頁面 (bamo0d260c.jsp) <br>
     * 按下「確認」的動作
     * 
     * @param mapping
     * @param form
    ...
    public ActionForward doQuery(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 更正作業 - 編審註記程度調整 - 遺屬年金編審註記程度調整 - 查詢頁面 SurvivorCheckMarkLevelAdjustAction.doQuery() 開始 ... ");

        HttpSession session = request.getSession();
        UserBean userData = (UserBean) UserSessionHelper.getUserData(request);

        SurvivorCheckMarkLevelAdjustForm queryForm = (SurvivorCheckMarkLevelAdjustForm) form;

        try {
            // 將先前的 Case 先清除
            CaseSessionHelper.removeSurvivorCheckMarkLevelAdjustCase(request);
            ...
            // 取得 案件 資料
            SurvivorCheckMarkLevelAdjustCase caseData = updateService.getSurvivorCheckMarkLevelAdjustData(apNo);
            this.setOptionList(request, caseData);
            ...
            log.debug("執行 更正作業 - 編審註記程度調整 - 遺屬年金編審註記程度調整 - 查詢頁面 SurvivorCheckMarkLevelAdjustAction.doQuery() 完成 ... ");

            return mapping.findForward(ConstantKey.FORWARD_QUERY_SUCCESS);
        }
        catch (Exception e) {
            // 設定查詢失敗訊息
            log.error("SurvivorCheckMarkLevelAdjustAction.doQuery() 發生錯誤:" + ExceptionUtility.getStackTrace(e));
    ...
    /**
     * 更正作業 - 編審註記程度調整 - 遺屬年金編審註記程度調整 - 修改頁面 (bamo0d261c.jsp) <br>
     * 按下「確認」的動作
     * 
     * @param mapping
     * @param form
    ...
    public ActionForward doSave(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 更正作業 - 編審註記程度調整 - 遺屬年金編審註記程度調整 - 修改頁面 SurvivorCheckMarkLevelAdjustAction.doSave() 開始 ... ");
        ...
        try {
                ...
                if (StringUtils.startsWith(parameterName, ADJ_LEVEL_FIELD_PREFIX)) { // begin ... [
                    ...
                    if (parameterValue != null && parameterValue.length > 0) { // begin ... [
                        ...
                        if (StringUtils.equals(parameterValue[0], ADJ_LEVEL_MUST_HANDLE)) {
                            SurvivorCheckMarkLevelAdjustBenDetailDataCase benDetailCase = caseData.searchBenDetailByKey(key);
            ...
            // 存檔
            updateService.saveSurvivorCheckMarkLevelAdjustData(caseData.getApNo(), caseData.getIssuYm(), saveList, caseData.getRegetCipbMk());
            ...
            try {
                ...
                log.error("SurvivorCheckMarkLevelAdjustAction.doSave() 即時編審發生錯誤:" + ExceptionUtility.getStackTrace(e));
            }
            ...
            // 重新取得更新後的案件資料
            caseData = updateService.getSurvivorCheckMarkLevelAdjustData(apNo);

            // 將 CaseData 塞到 Session 中
            CaseSessionHelper.setSurvivorCheckMarkLevelAdjustCase(caseData, request);
            
            log.debug("執行 更正作業 - 編審註記程度調整 - 遺屬年金編審註記程度調整 - 修改頁面 SurvivorCheckMarkLevelAdjustAction.doSave() 完成 ... ");

            return mapping.findForward(ConstantKey.FORWARD_SAVE_SUCCESS);
        }
        catch (Exception e) {
            // 設定查詢失敗訊息
            log.error("SurvivorCheckMarkLevelAdjustAction.doSave() 發生錯誤:" + ExceptionUtility.getStackTrace(e));
            saveMessages(session, DatabaseMessageHelper.getCheckMarkLevelAdjustSaveFailMessage());
            return mapping.findForward(ConstantKey.FORWARD_SAVE_FAIL);
        }
    }
    ...
    public ActionForward doPrint(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 更正作業 - 編審註記程度調整 - 遺屬年金編審註記程度調整 - 修改頁面 SurvivorCheckMarkLevelAdjustAction.doPrint() 開始 ... ");
        ...
        try {
            ...
            log.debug("執行 更正作業 - 編審註記程度調整 - 遺屬年金編審註記程度調整 - 修改頁面 SurvivorCheckMarkLevelAdjustAction.doPrint() 完成 ... ");

            return null;
        }
        catch (Exception e) {
            // 設定報表產製失敗訊息
            log.error("SurvivorCheckMarkLevelAdjustAction.doPrint() 發生錯誤:" + ExceptionUtility.getStackTrace(e));
            saveMessages(session, CustomMessageHelper.getReportGenerateFailMessage());
            return mapping.findForward(FORWARD_DETAIL_PAGE);
        }
    }

    // 設定 明細頁面 給付年月 以及 受款人下拉選單
    private void setOptionList(HttpServletRequest request, SurvivorCheckMarkLevelAdjustCase caseData) {
        if (caseData == null || !caseData.hasCheckFileData()) {
            // 取得 給付年月下拉選單
            request.getSession().setAttribute(ConstantKey.SURVIVOR_CHECK_MARK_LEVEL_ADJUST_PAYYM_OPTION_LIST, new TreeMap<String, String>());
            // 取得 受款人下拉選單
            request.getSession().setAttribute(ConstantKey.SURVIVOR_CHECK_MARK_LEVEL_ADJUST_SEQNO_OPTION_LIST, new TreeMap<String, String>());
        }
        else {
            TreeMap<String, String> payYmMap = new TreeMap<String, String>();
            TreeMap<String, String> seqNoMap = new TreeMap<String, String>();
            ...
            for (SurvivorCheckMarkLevelAdjustBenDataCase benData : benDataList) {
                if (!seqNoMap.containsKey(benData.getSeqNo())) {
                    seqNoMap.put(benData.getSeqNo(), benData.getBenName());
                }
                List<SurvivorCheckMarkLevelAdjustBenDetailDataCase> benDetailDataList = benData.getDetailList();
                for (SurvivorCheckMarkLevelAdjustBenDetailDataCase benDetailData : benDetailDataList) {
                    if (!payYmMap.containsKey(benDetailData.getPayYm())) {
                        payYmMap.put(benDetailData.getPayYm(), benDetailData.getPayYm());
                    }
                }
            }
            // 取得 給付年月下拉選單
            request.getSession().setAttribute(ConstantKey.SURVIVOR_CHECK_MARK_LEVEL_ADJUST_PAYYM_OPTION_LIST, payYmMap);
            // 取得 受款人下拉選單
            request.getSession().setAttribute(ConstantKey.SURVIVOR_CHECK_MARK_LEVEL_ADJUST_SEQNO_OPTION_LIST, seqNoMap);
        }
    }
