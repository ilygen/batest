...
import tw.gov.bli.ba.bj.forms.MonthBatchKForm;
 ...
 * @author Noctis
 */
public class MonthBatchKAction extends BaseDispatchAction {
    private static Log log = LogFactory.getLog(MonthBatchKAction.class);
    ...
    /**
     * 批次處理 - 批次月處理作業 - 失能年金批次月處理作業 - 查詢頁面 (baba0m060x.jsp) 按下 確認
     * 
     * @param mapping
     * @param form
     * @param request
    ...
    public ActionForward doConfirm(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 批次處理 - 批次月處理作業 - 失能年金批次月處理作業 - 查詢頁面  MonthBatchKAction.doQuery() 開始 ... ");

        HttpSession session = request.getSession();
        UserBean userData = (UserBean) UserSessionHelper.getUserData(request);
        MonthBatchKForm iform = (MonthBatchKForm) form;
        ...
        try {
            ...
            // log.debug("批次處理 - 批次月處理作業 - 失能年金批次月處理作業 - 查詢頁面 MonthBatchKAction.doConfirm() 完成 ... ");
            // return mapping.findForward(ConstantKey.FORWARD_QUERY_FAIL);

            // }else{
            // 新增一筆批次作業紀錄於「勞保年金批次作業檔_BABATCHJOB」中
            // bjService.insertBabatchjobDataForMonthBatchK(DateUtility.changeChineseYearMonthType(iform.getIssuYm()) , payCode, iform.getProcType(), iform.getPaySeqNo(), userData);
            ...
            if (iform.getProcType().equals("2")) {
                ...
                returnCode = (String) call.invoke(new Object[] { payCode, dateTime, userId, deptId, loginIP, iform.getPaySeqNo(), iform.getCaseTyp(), SecureToken.getInstance().getToken() });
                log.debug("執行 批次月處理作業 - 失能年金批次月處理作業 - 呼叫批次排程編審結果... " + returnCode);
                ...
                if (iform.getProcType().equals("3")) {
                    returnCode = (String) call.invoke(new Object[] { payCode, iform.getProcType(), dateTime, userId, deptId, loginIP, iform.getPaySeqNo(), iform.getCaseTyp(), SecureToken.getInstance().getToken() });
                }
                else {
                    returnCode = (String) call.invoke(new Object[] { payCode, iform.getProcType(), dateTime, userId, deptId, loginIP, iform.getPaySeqNo(), null, SecureToken.getInstance().getToken() });
                }
                log.debug("執行  批次月處理作業 - 失能年金批次月處理作業 - 呼叫批次排程編審結果... " + returnCode);
            }
            ...
            if (returnCode.equals("0")) {
                ...
                scheduleMsg = "失能年金" + iform.getIssuYm() + "核定年月批次月處理作業(月處理類型(" + procTypeStr + "))";

                // 批次作業完成後，直接將批次月處理條件帶入[進度查詢_BABA0M050X]頁面查明，
                List<MonthBatchKCase> dataList = bjService.selectMonthBatchKQueryDataListBy(payCode, null, DateUtility.getNowWestDate(), iform.getProcType(), iform.getPaySeqNo());

                // 將 查詢結果清單 存到 Request Scope
                CaseSessionHelper.setMonthBatchKCaseList(dataList, request);
                ...
                saveMessages(session, CustomMessageHelper.getMonthBatchPerformedErrorMessage(procTypeStr));
                log.debug("批次處理 - 批次月處理作業 - 失能年金批次月處理作業 - 查詢頁面  MonthBatchLAction.doConfirm() 完成 ... ");
                return mapping.findForward(ConstantKey.FORWARD_QUERY_FAIL);
            ...
            // 設定查詢失敗訊息
            log.error("MonthBatchKAction.doConfirm() 發生錯誤:" + ExceptionUtility.getStackTrace(e));
    ...
    /**
     * 批次處理 - 批次月處理作業 - 失能年金批次月處理作業 - 查詢頁面 (baba0m050x.jsp) 按下 進度查詢
     * 
     * @param mapping
     * @param form
     * @param request
    ...
    public ActionForward doQuery(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            List<MonthBatchKCase> dataList = bjService.selectMonthBatchKQueryDataListBy(payCode, DateUtility.changeChineseYearMonthType(iform.getIssuYm()), null, iform.getProcType(), iform.getPaySeqNo());

            if (dataList.size() < 1) {

                saveMessages(session, DatabaseMessageHelper.getNoResultMessage());
                log.debug("批次處理 - 批次月處理作業 - 失能年金批次月處理作業 - 查詢頁面  MonthBatchKAction.doQuery() 完成 ... ");
                return mapping.findForward(ConstantKey.FORWARD_QUERY_FAIL);
            ...
            // 設定查詢失敗訊息
            log.error("MonthBatchKAction.doQuery() 發生錯誤:" + ExceptionUtility.getStackTrace(e));
            saveMessages(session, DatabaseMessageHelper.getQueryFailMessage());
            return mapping.findForward(ConstantKey.FORWARD_FAIL);
        }
    }
    ...
    /**
     * 批次處理 - 批次月處理作業 - 失能年金批次月處理作業 - 查詢頁面 (baba0m051q.jsp) 按下 檔案下載
     * 
     * @param mapping
     * @param form
     * @param request
    ...
    public ActionForward doDownLoad(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 批次處理 - 批次月處理作業 - 失能年金批次月處理作業 - 查詢頁面  MonthBatchKAction.doDownLoad() 開始 ... ");
        ...
        try {
            ...
            if (StringUtils.isBlank(fileName)) {
                ...
                saveMessages(session, DatabaseMessageHelper.getNoResultMessage());
                log.debug("批次處理 - 批次月處理作業 - 失能年金批次月處理作業 - 資料頁面  MonthBatchKAction.doDownLoad() 完成 ... ");
            ...
            // 設定查詢失敗訊息
            log.error("MonthBatchKAction.doDownLoad() 發生錯誤:" + ExceptionUtility.getStackTrace(e));
            saveMessages(session, DatabaseMessageHelper.getQueryFailMessage());
            return mapping.findForward(ConstantKey.FORWARD_FAIL);
        }
    }
    ...
    /**
     * 批次處理 - 批次月處理作業 - 失能年金批次月處理作業 - 查詢頁面 (baba0m051q.jsp) 按下 狀態明細
     * 
     * @param mapping
     * @param form
     * @param request
    ...
    public ActionForward doDetail(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 批次處理 - 批次月處理作業 - 失能年金批次月處理作業 - 資料頁面  MonthBatchKAction.doDetail() 開始 ... ");
        ...
        try {
            ...
            if (dataList.size() < 1) {
                ...
                saveMessages(session, DatabaseMessageHelper.getNoResultMessage());
                log.debug("批次處理 - 批次月處理作業 - 失能年金批次月處理作業 - 資料頁面  MonthBatchKAction.doDetail() 完成 ... ");
                return mapping.findForward(FORWARD_QUERY_LIST_FAIL);
            ...
            // 設定查詢失敗訊息
            log.error("MonthBatchKAction.doDetail() 發生錯誤:" + ExceptionUtility.getStackTrace(e));
            saveMessages(session, DatabaseMessageHelper.getQueryFailMessage());
            return mapping.findForward(ConstantKey.FORWARD_FAIL);
        }
    }
    ...
    /**
     * 批次處理 - 批次月處理作業 - 失能年金批次月處理作業 - 資料頁面 (baba0m111q.jsp,baba0m112a.jsp) 按下 返回
     * 
     * @param mapping
     * @param form
     * @param request
    ...
    public ActionForward doBack(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 批次處理 - 批次月處理作業 - 失能年金批次月處理作業 - 返回 MonthBatchKAction.doBack() 開始 ... ");
        ...
        iform.setProcType("");
        iform.setPaySeqNo("");
        CaseSessionHelper.removeMonthBatchKCaseList(request);

        log.debug("執行批次處理 - 批次月處理作業 - 失能年金批次月處理作業 - 返回 MonthBatchKAction.doBack() 完成 ... ");
