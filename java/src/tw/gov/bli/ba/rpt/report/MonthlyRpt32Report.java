 ...
 * @author KIYOMI
 */
public class MonthlyRpt32Report extends ReportBase {

    private RptService rptService;

    public MonthlyRpt32Report() throws Exception {
        super();
    }
    ...
    public Table getHeader(boolean bPrintMainHeader, String payCode, String chkDate, String pageNum, String naChgMk, String nlWkMk, String adWkMk, String isNaChgMk, String cPrnDate) throws Exception {
        ...
        if (bPrintMainHeader) {
            ...
            addColumn(table, 99, 1, "保留遺屬年金計息存儲核定清單", fontCh10, 0, CENTER);
    ...
    public ByteArrayOutputStream doReport(UserBean userData, List<MonthlyRpt32Case> caseData, List<MonthlyRpt32Case> caseDataFooter, List<MonthlyRpt32Case> caseData1, List<MonthlyRpt32Case> caseDataFooter1, HashMap<String, Object> map)
                    throws Exception {
        try {
            ...
            // FileOutputStream bao = new FileOutputStream(PropertyHelper.getProperty("rpt.path") + Encode.forJava(payCode) + "_MonthlyRpt32_" + Encode.forJava(printDate) + ".pdf");
            // FileOutputStream bao = new FileOutputStream("F:\\"+"MonthlyRpt32_" + printDate + ".pdf");
            ...
            if (!(caseData == null) && caseData.size() > 0) {
                ...
                Map<String, List<MonthlyRpt32Case>> dataMap = new TreeMap<String, List<MonthlyRpt32Case>>();
                Map<String, List<MonthlyRpt32Case>> dataMapFooter = new TreeMap<String, List<MonthlyRpt32Case>>(); // 20120406
                for (int i = 0; i < caseData.size(); i++) {
                    MonthlyRpt32Case obj = caseData.get(i);
                    dataMap.put(obj.getPayTyp() + obj.getNlWkMk() + obj.getAdWkMk() + obj.getIssuTyp(), new ArrayList<MonthlyRpt32Case>());
                }
                ...
                for (int i = 0; i < caseData.size(); i++) { // 20120416
                    MonthlyRpt32Case obj = caseData.get(i);
                    dataMapFooter.put(obj.getPayTyp() + obj.getNlWkMk() + obj.getAdWkMk() + obj.getIssuTyp(), new ArrayList<MonthlyRpt32Case>());
                }

                for (int i = 0; i < caseDataFooter.size(); i++) { // 20120416
                    MonthlyRpt32Case obj = caseDataFooter.get(i);
                ...
                for (Iterator it = dataMap.keySet().iterator(); it.hasNext();) {
                    ...
                    List<MonthlyRpt32Case> caseList = dataMap.get(key);
                    MonthlyRpt32Case tmpCaseObj = caseList.get(0);

                    // 拿取頁次 20130731
                    List<MonthlyRpt32Case> caseListPageNum = dataMapFooter.get(key);
                    ...
                    table = getHeader(true, payCode, tmpCaseObj.getChkDateStr(), pageNum, "", tmpCaseObj.getNlWkMk(), tmpCaseObj.getAdWkMk(), "N", tmpCaseObj.getcPrnDateStr());
                    for (MonthlyRpt32Case caseObj : caseList) {
                    ...
                    table.setBorderWidth(1);
                    int rowCount = 0;
                    List<MonthlyRpt32Case> caseList1 = dataMapFooter.get(key);
                    for (MonthlyRpt32Case caseObj1 : caseList1) {
            ...
            if (!(caseData1 == null) && caseData1.size() > 0) {
                ...
                // Map的Key 為 issuYm+payYm
                // Value則是同為該 payTyp 下的所有資料
                Map<String, List<MonthlyRpt32Case>> dataMap1 = new TreeMap<String, List<MonthlyRpt32Case>>();
                Map<String, List<MonthlyRpt32Case>> dataMapFooter1 = new TreeMap<String, List<MonthlyRpt32Case>>(); // 20120406
                for (int i = 0; i < caseData1.size(); i++) {
                    MonthlyRpt32Case obj = caseData1.get(i);
                    dataMap1.put(obj.getPayTyp() + obj.getNlWkMk() + obj.getAdWkMk() + obj.getIssuTyp(), new ArrayList<MonthlyRpt32Case>());
                }
                ...
                for (int i = 0; i < caseData1.size(); i++) { // 20120416
                    MonthlyRpt32Case obj = caseData1.get(i);
                    dataMapFooter1.put(obj.getPayTyp() + obj.getNlWkMk() + obj.getAdWkMk() + obj.getIssuTyp(), new ArrayList<MonthlyRpt32Case>());
                }

                for (int i = 0; i < caseDataFooter1.size(); i++) { // 20120416
                    MonthlyRpt32Case obj = caseDataFooter1.get(i);
                ...
                for (Iterator it = dataMap1.keySet().iterator(); it.hasNext();) {
                    ...
                    String key = (String) it.next();
                    List<MonthlyRpt32Case> caseList = dataMap1.get(key);
                    MonthlyRpt32Case tmpCaseObj = caseList.get(0);

                    // 拿取頁次 20130731
                    List<MonthlyRpt32Case> caseListPageNumSub = dataMapFooter1.get(key);
                    ...
                    table = getHeader(true, payCode, tmpCaseObj.getChkDateStr(), pageNumSub, "1", tmpCaseObj.getNlWkMk(), tmpCaseObj.getAdWkMk(), "Y", tmpCaseObj.getcPrnDateStr());
                    for (MonthlyRpt32Case caseObj : caseList) {
                        BigDecimal payAmt = new BigDecimal(0);// 應付金額歸0
                        // 計算應付金額
                        payAmt = caseObj.getIssueAmt();// 應付金額 = 核定金額
                    ...
                    table.setBorderWidth(1);
                    int rowCount = 0;
                    List<MonthlyRpt32Case> caseList1 = dataMapFooter1.get(key);
                    for (MonthlyRpt32Case caseObj1 : caseList1) {
            ...
            printToDisk((PropertyHelper.getProperty("rpt.path") + Encode.forJava(payCode) + "_MonthlyRpt32_" + Encode.forJava(printDate) + ".pdf"), bao);
        }
        finally {
            document.close();
        }
