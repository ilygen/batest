 ...
 * @author Goston
 */
public class PreviewConditionMaintForOldAgeAnnuityAction extends BaseDispatchAction {
    private static Log log = LogFactory.getLog(PreviewConditionMaintForOldAgeAnnuityAction.class);

    private MaintService maintService;

    /**
     * 維護作業 - 先抽對象維護作業 - 老年年金維護作業 (bapa0x042c.jsp) <br>
     * 按下「確認」的動作
     * 
     * @param mapping
     * @param form
    ...
    public ActionForward doSave(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 維護作業 - 先抽對象維護作業 - 老年年金維護作業 PreviewConditionMaintForOldAgeAnnuityAction.doSave() 開始 ... ");

        HttpSession session = request.getSession();
        UserBean userData = (UserBean) UserSessionHelper.getUserData(request);

        PreviewConditionMaintDetailForOldAgeAnnuityForm detailForm = (PreviewConditionMaintDetailForOldAgeAnnuityForm) form;

        try {
            if (detailForm != null) {
                PreviewConditionMaintCase caseData = new PreviewConditionMaintCase();
                BeanUtility.copyProperties(caseData, detailForm);

                // 存檔
                maintService.savePreviewConditionMaintData(caseData, userData);

                // 設定存檔成功訊息
                saveMessages(session, DatabaseMessageHelper.getSaveSuccessMessage());
            }

            log.debug("執行 維護作業 - 先抽對象維護作業 - 老年年金維護作業 PreviewConditionMaintForOldAgeAnnuityAction.doSave() 完成 ... ");

            return mapping.findForward(ConstantKey.FORWARD_SAVE_SUCCESS);
        }
        catch (Exception e) {
            // 設定存檔失敗訊息
            log.error("PreviewConditionMaintForOldAgeAnnuityAction.doSave() 發生錯誤:" + ExceptionUtility.getStackTrace(e));
