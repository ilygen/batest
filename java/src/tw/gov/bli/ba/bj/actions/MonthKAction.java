...
import tw.gov.bli.ba.bj.forms.MonthKForm;
...
/**
 * 批次處理 - 線上月核定作業 - 失能年金線上月核定作業 - 查詢頁面 (baba0m110x.jsp)
 * 
 * @author Noctis
 */
public class MonthKAction extends BaseDispatchAction {
    private static Log log = LogFactory.getLog(MonthKAction.class);
    ...
    /**
     * 批次處理 - 線上月核定作業 - 失能年金線上月核定作業 - 查詢頁面 (baba0m110x.jsp) 按下 確認
     * 
     * @param mapping
     * @param form
     * @param request
    ...
    public ActionForward doQuery(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 批次處理 - 線上月核定作業 - 失能年金線上月核定作業 - 查詢頁面  MonthKAction.doQuery() 開始 ... ");

        HttpSession session = request.getSession();
        UserBean userData = (UserBean) UserSessionHelper.getUserData(request);
        MonthKForm iform = (MonthKForm) form;
        ...
        try {
            ...
            if (checkCount > 0) {
                ...
                log.debug("批次處理 - 線上月核定作業 - 失能年金線上月核定作業 - 查詢頁面  MonthKAction.doQuery() 完成 ... ");
                return mapping.findForward(ConstantKey.FORWARD_QUERY_FAIL);

            }
            else {

                List<MonthKCase> dataList = bjService.selectMonthKDataListBy(payCode, DateUtility.changeChineseYearMonthType(iform.getIssuYm()));
                ...
                if (dataList.size() < 1) {
                    ...
                    CaseSessionHelper.setMonthKCaseList(dataList, request);
            ...
            // 設定查詢失敗訊息
            log.error("MonthKAction.doQuery() 發生錯誤:" + ExceptionUtility.getStackTrace(e));
    ...
    /**
     * 批次處理 - 線上月核定作業 - 失能年金線上月核定作業 - 查詢頁面 (baba0m110x.jsp) 按下 查詢
     * 
     * @param mapping
     * @param form
     * @param request
    ...
    public ActionForward doQueryList(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 批次處理 - 線上月核定作業 - 失能年金線上月核定作業 - 查詢頁面  MonthKAction.doQueryList() 開始 ... ");
        ...
        try {
            ...
            List<MonthKCase> dataList = bjService.selectMonthKQueryDataListBy(payCode, DateUtility.changeChineseYearMonthType(iform.getIssuYm()), DateUtility.changeDateType(iform.getChkDate(), false));

            if (dataList.size() < 1) {

                saveMessages(session, DatabaseMessageHelper.getNoResultMessage());
                log.debug("批次處理 - 線上月核定作業 - 失能年金線上月核定作業 - 查詢頁面  MonthKAction.doQueryList() 完成 ... ");
            ...
            // 設定查詢失敗訊息
            log.error("MonthKAction.doQueryList() 發生錯誤:" + ExceptionUtility.getStackTrace(e));
            saveMessages(session, DatabaseMessageHelper.getQueryFailMessage());
            return mapping.findForward(ConstantKey.FORWARD_FAIL);
        }
    }
    ...
    /**
     * 批次處理 - 線上月核定作業 - 失能年金線上月核定作業 - 清單頁面 (baba0m111q.jsp) 按下 確認
     * 
     * @param mapping
     * @param form
     * @param request
    ...
    public ActionForward doConfirm(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 批次處理 - 線上月核定作業 - 失能年金線上月核定作業 - 清單頁面  MonthKAction.doConfirm() 開始 ... ");
        ...
        try {
            ...
            // 取得清單資料
            List<MonthKCase> dataList = CaseSessionHelper.getMonthKCaseList(request);
            ...
            for (int i = 0; i < tmpIndexList.length; i++) {
                ...
                if (StringUtils.isNotBlank(dataIndex)) {
                    ...
                    MonthKCase caseObj = (MonthKCase) dataList.get(Integer.parseInt(dataIndex));
                    ...
                    if (checkCount > 0) {
                        ...
                        // 更新 勞保年金線上作業檔
                        bjService.updateBaonlinejobDataForMonthK(caseObj, issuYm, procMsg, userData);
            ...
            // 更新頁面資料
            List<MonthKCase> dataListNew = new ArrayList<MonthKCase>();

            for (int i = 0; i < apNoList.size(); i++) {
                List<MonthKCase> caseData = bjService.selectMonthKApprovedDataListBy(apNoList.get(i), DateUtility.changeChineseYearMonthType(iform.getIssuYm()));
            ...
            if (dataListNew.size() < 1) {
                ...
                saveMessages(session, DatabaseMessageHelper.getNoResultMessage());
                log.debug("批次處理 - 線上月核定作業 - 失能年金線上月核定作業 - 清單頁面  MonthKAction.doConfirm() 完成 ... ");
                return mapping.findForward(FORWARD_QUERY_LIST_FAIL);
                ...
                // 將 查詢結果清單 存到 Request Scope
                CaseSessionHelper.setMonthQueryCaseForTitle(caseData, request);
                CaseSessionHelper.setMonthKCaseList(dataListNew, request);
            ...
            // 設定查詢失敗訊息
            log.error("MonthKAction.doConfirm() 發生錯誤:" + ExceptionUtility.getStackTrace(e));
            saveMessages(session, DatabaseMessageHelper.getQueryFailMessage());
            return mapping.findForward(ConstantKey.FORWARD_FAIL);
        }
    }
    ...
    /**
     * 批次處理 - 線上月核定作業 - 失能年金線上月核定作業 - 清單頁面 (baba0m111q.jsp) 按下返回
     * 
     * @param mapping
     * @param form
     * @param request
    ...
    public ActionForward doDelete(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 批次處理 - 線上月核定作業 - 失能年金線上月核定作業 - 清單頁面  MonthKAction.doDelete() 開始 ... ");
        ...
        try {
            ...
            for (int i = 0; i < tmpIndexList.length; i++) {
                ...
                if (StringUtils.isNotBlank(dataIndex)) {
                    ...
                    bjService.updateBaonlinejobDataForMonthKDelete(caseObj, userData);

                }
            }

            // 更新頁面資料
            List<MonthKCase> dataListNew = bjService.selectMonthKDataListBy(payCode, DateUtility.changeChineseYearMonthType(iform.getIssuYm()));
            ...
            log.debug("批次處理 - 線上月核定作業 - 失能年金線上月核定作業 - 清單頁面  MonthKAction.doDelete() 完成 ... ");
            return mapping.findForward(ConstantKey.FORWARD_QUERY_SUCCESS);

        }
        catch (Exception e) {
            // 設定查詢失敗訊息
            log.error("MonthKAction.doDelete() 發生錯誤:" + ExceptionUtility.getStackTrace(e));
            saveMessages(session, DatabaseMessageHelper.getQueryFailMessage());
            return mapping.findForward(ConstantKey.FORWARD_FAIL);
        }
    }
    ...
    /**
     * 批次處理 - 線上月核定作業 - 失能年金線上月核定作業 - 資料頁面 (baba0m111q.jsp,baba0m112a.jsp) 按下 返回
     * 
     * @param mapping
     * @param form
     * @param request
    ...
    public ActionForward doBack(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 批次處理 - 線上月核定作業 - 失能年金線上月核定作業 - 返回 MonthKAction.doBack() 開始 ... ");
        ...
        CaseSessionHelper.removeMonthKCaseList(request);

        log.debug("執行批次處理 - 線上月核定作業 - 失能年金線上月核定作業 - 返回 MonthKAction.doBack() 完成 ... ");
