...
<html:html lang="true">
<head>
    <acl:setProgId progId="BARC0D211A" />
    ...
    <div id="main" class="mainBody">
        ...
        <fieldset>
            ...
            <table width="98%" border="0" align="center" cellpadding="2" cellspacing="2" class="px13">
                <tr>
                    <td colspan="2" align="right">
                        <acl:checkButton buttonName="btnConfirm">
                            <input type="button" name="btnConfirm" class="button" value="確　認" onclick="$('page').value='1'; $('method').value='doSaveData'; if (checkFields()){document.SurvivorPaymentDecisionForm.submit();}else{return false;}" />&nbsp;
                        </acl:checkButton>
                        <acl:checkButton buttonName="btnBack">
                            <input type="button" name="btnBack" class="button" value="返　回" onclick="$('page').value='1'; $('method').value='doBack'; document.SurvivorPaymentDecisionForm.submit();" />&nbsp;
                        </acl:checkButton>                                             
                    </td>
                </tr>    
                <tr>
                    <td colspan="2" class="issuetitle_L">
                        <bean:define id="payTitle" name="<%=ConstantKey.SURVIVOR_PAYMENT_DECISION_CASE_FOR_TITLE%>" />                        
                ...
                <tr align="center">
                    <td colspan="2" >
                        <bean:define id="list" name="<%=ConstantKey.SURVIVOR_PAYMENT_DECISION_LIST%>" />
                        <display:table name="pageScope.list" id="listItem" pagesize="<%=ConstantKey.DECISION_LIST_PAGE_SIZE%>">
                            <display:column title="<input type='checkbox' name='radioAll' onclick='chooseOne(this);selectAllPlus(this, \"reChkCheckbox\");'>全選<br>複 核" style="width:6%; text-align:center;">                                
                                <c:if test="${listItem.isShowReChk eq 'Y'}">
                                    <c:if test="${listItem.reBox eq 'Y'}">
                                        <input type="checkbox" id="reChkCheckbox" name='checkbox<c:out value="${listItem_rowNum}" />' value="<c:out value="${listItem.apNo}" />;<c:out value="${listItem.balp0d020Id}" />;30" onclick='chooseOne(this);' checked="true">
                                    </c:if>
                                    <c:if test="${listItem.reBox ne'Y'}">
                                        <input type="checkbox" id="reChkCheckbox" name='checkbox<c:out value="${listItem_rowNum}" />' value="<c:out value="${listItem.apNo}" />;<c:out value="${listItem.balp0d020Id}" />;30" onclick='chooseOne(this);'>                                    
                                    </c:if>
                                </c:if>
                                <c:if test="${listItem.isShowReChk eq 'N'}">
                                    <input type="checkbox" id="reChkCheckbox" name='checkbox<c:out value="${listItem_rowNum}" />' value="<c:out value="${listItem.apNo}" />;<c:out value="${listItem.balp0d020Id}" />;30" disabled="disabled">
                                </c:if>                                
                            </display:column>
                            ...
                            <display:column title="資料明細" style="width:6%; text-align:center;">
                                <acl:checkButton buttonName="btnDetail">
                                    <input type="button" name="btnDetail" class="button" value="明細資料" onclick="$('page').value='1'; $('method').value='doSingleReview'; $('apNo').value='<c:out value="${listItem.apNo}" />'; document.SurvivorPaymentDecisionForm.submit();" />&nbsp;
                                </acl:checkButton>                            
                            </display:column>
                        </display:table>
                    </td>
