...
<html:html lang="true">
<head>
<acl:setProgId progId="BAMO0D311C" />
...
<body scroll="no">
	...
	<div id="mainContent">
		...
		<div id="main" class="mainBody">
			<html:form action="/survivorCloseStatusAlteration" method="post">
				...
				<fieldset>
					...
					<table width="98%" border="0" align="center" cellpadding="2" cellspacing="2" class="px13">
						...
						<tr align="center">
							<td><bean:define id="list" name="<%=ConstantKey.CLOSE_STATUS_ALTERATION_CASE_PAYEE_LIST%>" /> 
								<display:table name="pageScope.list" id="listItem">
									<display:column title="勾選" style="width:10%; text-align:center;">
										<c:choose>
											<c:when test="${listItem.idForConfirm eq 'Y'}">
									...
									<display:column title="受款人序號" style="width:10%; text-align:center;">
										<c:out value="${listItem.seqNo}" />&nbsp;
                            		</display:column>
									<display:column title="遺屬姓名" style="width:35%;">
										<c:out value="${listItem.benName}" />&nbsp;
                            		</display:column>
									<display:column title="身分證號碼" style="width:15%;">
										<c:out value="${listItem.benIdnNo}" />&nbsp;
                            		</display:column>
									<display:column title="關係" style="width:15%;">
										<c:out value="${listItem.benEvtRelStr}" />&nbsp;
                            		</display:column>
									<display:column title="狀態" style="width:15%;">
										<c:if test="${listItem.idForConfirm eq 'Y'}"> 
											結案&nbsp; 
										</c:if> 
                            		</display:column>
