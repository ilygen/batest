...
<html:html lang="true">
<head>
<acl:setProgId progId="BAMO0D105C" />
						...
						<tr>
							<td colspan="14">
								<table border="0" cellpadding="0" cellspacing="0"
									...
									<tr>
										<td><a
											href='<c:url value="/oldAgePaymentReceive.do?method=doQuery" />'
											title="連結至：退現資料" target="_self" class="needtxt"><span>退現資料</span></a>
											<h5>
												<a>應收資料</a>
											</h5></td>
									</tr>
								...
								<table width="98%" cellpadding="3" cellspacing="3" class="px13">
									<tr>
										<td colspan="8" class="issuetitle_L"><span
											class="systemMsg">▲</span>&nbsp;應收資料</td>
									</tr>
									<tr>
										<td colspan="8" align="center"><bean:define id="list"
												name="<%=ConstantKey.ACCOUNTS_RECEIVABLE_DATA_CASE_LIST%>" />
											<display:table name="pageScope.list" id="listItem">
												...
												<display:column title="資料狀態" headerClass="issuetitle_L"
													class="issueinfo_C" style="width:10%; text-align:center;">
													<c:out value="${listItem.stsStr}" />&nbsp;
                                                </display:column>
												<display:column title="核定年月" headerClass="issuetitle_L"
													class="issueinfo_C" style="width:10%; text-align:center;">
													<c:out value="${listItem.issuYmStr}" />&nbsp;
                                                </display:column>
												<display:column title="給付年月" headerClass="issuetitle_L"
													class="issueinfo_C" style="width:10%; text-align:center;">
													<c:out value="${listItem.payYmStr}" />&nbsp;
                                                </display:column>
												<display:column title="受款人序" headerClass="issuetitle_L"
													class="issueinfo_C" style="width:10%; text-align:center;">
													<c:out value="${listItem.seqNo}" />&nbsp;
                                                </display:column>
												<display:column title="立帳日期" headerClass="issuetitle_L"
													class="issueinfo_C" style="width:10%; text-align:center;">
													<c:out value="${listItem.unacpDateStr}" />&nbsp;
                                                </display:column>
												<display:column title="應收金額" headerClass="issuetitle_L"
													class="issueinfo_C" style="width:20%; text-align:right;">
													<fmt:formatNumber value="${listItem.recAmt}" />
												</display:column>
												<display:column title="應收未收餘額" headerClass="issuetitle_L"
													class="issueinfo_C" style="width:20%; text-align:right;">
