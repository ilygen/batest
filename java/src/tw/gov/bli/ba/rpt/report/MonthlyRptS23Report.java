...
public class MonthlyRptS23Report extends ReportBase {

    private int nPage; // 每筆資料的頁數

    private static final String TEMPLATE_FILE = "/pdf/MonthlyRpt23.pdf";

    public MonthlyRptS23Report() throws Exception {
        super();
        nPage = 0;
    }
    ...
    public ByteArrayOutputStream doReport(InputStream inFile, UserBean userData, List<MonthlyRpt23Case> caseList, HashMap<String, Object> map) throws Exception {
        ...
        try {
            ...
            if (inFile == null) {
                printToDisk((PropertyHelper.getProperty("rpt.path") + Encode.forJava(payCode) + "_MonthlyRptS23_" + Encode.forJava(printDate) + ".pdf"), out);
            }
            else {
                printToDisk((PropertyHelper.getProperty("rpt.path") + Encode.forJava(payCode) + "_MonthlyRptS23_" + Encode.forJava(printDate) + ".pdf"), bao);
            }
        }
        finally {
            document.close();
