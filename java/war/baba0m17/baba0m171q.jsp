...
<html:html lang="true">
<head>
    <acl:setProgId progId="BABA0M171Q" />
    ...
    <div id="main" class="mainBody">
        ...
        <fieldset>
            ...
            <table width="100%" border="0" align="center" cellpadding="2" cellspacing="2" class="px13">
                ...
                <bean:define id="qryForm" name="<%=ConstantKey.OLD_MEDIA_ONLINE_FORM %>" />
                <tr>
                ...
                <tr align="center">                   
                    <td colspan="4">
                        <bean:define id="list" name="<%=ConstantKey.OLD_MEDIA_ONLINE_CASE_LIST%>" />
                        <display:table name="pageScope.list" id="listItem" pagesize="<%=ConstantKey.LIST_PAGE_SIZE%>">                            
                            ...
                            <display:column title="媒體檔案名稱" headerClass="issuetitle_L" class="issueinfo" style="width:20%; text-align:center;">
                                <c:out value="${listItem.mfileName}" />&nbsp;
                            </display:column>
                            <display:column title="媒體檔案日期" headerClass="issuetitle_L" class="issueinfo" style="width:20%; text-align:center;">
                                <c:out value="${listItem.mfileDate}" />&nbsp;
                            </display:column>
                            <display:column title="檔案下載" headerClass="issuetitle_L" class="issueinfo_C" style="width:30%; text-align:center;">
                                <acl:checkButton buttonName="btnDetail">
                                    <input type="button" name="btnDetail" class="button" value="檔案下載" onclick="$('page').value='1'; $('method').value='doDownload';$('mfileName').value='<c:out value="${listItem.mfileName}" />'; document.OldMediaOnlineForm.submit();" />&nbsp;
                                </acl:checkButton>    
                            </display:column>
                            <display:column title="傳送" headerClass="issuetitle_L" class="issueinfo_C" style="width:10%; text-align:center;">
                                <acl:checkButton buttonName="btnDetail">
                                    <input type="button" name="btnDetail" class="button" value="傳送" onclick="$('page').value='1'; $('method').value='doUploadFile';$('mfileName').value='<c:out value="${listItem.mfileName}" />';$('mfileDate').value='<c:out value="${listItem.mfileDate}" />';$('ftpSeq').value='<c:out value="${listItem.ftpSeq}" />'; document.OldMediaOnlineForm.submit();" />&nbsp;
                                </acl:checkButton>
                            </display:column>
