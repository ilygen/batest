 ...
 * @author Goston
 */
public class ReviewFeeDecisionCase implements Serializable {
    // Header
    // [
    // BABCML7
    ...
    private ReviewFeeDecisionDetailCase detailData; // Detail 頁面之該筆明細資料

    // BABCML7 - 複檢費用主檔明細資料
    private List<ReviewFeeDecisionDetailCase> detailList; // List 頁面之清單明細資料
     ...
     * @param detailData
     */
    public void addDetailData(ReviewFeeDecisionDetailCase detailData) {
        if (detailList == null)
            detailList = new ArrayList<ReviewFeeDecisionDetailCase>();
     ...
     * @return
     */
    public ReviewFeeDecisionDetailCase getDetailDataByApSeq(BigDecimal apSeq) {
        if (apSeq == null)
            return null;

        for (ReviewFeeDecisionDetailCase detailData : detailList) {
     ...
     * 是否顯示 複核
     * 
     * @return <code>true</code> 顯示; <code>false</code> 不顯示
     */
    public boolean isShowReCheck() {
        if (StringUtils.equalsIgnoreCase(reChk, "Y"))
            return true;
        else
            return false;
    }
     ...
     * @return <code>true</code> 顯示; <code>false</code> 不顯示
     */
    public boolean isShowDecision() {
        if (StringUtils.isBlank(exeMk) || StringUtils.isBlank(mexcLvl))
            return false;

        try {
            if (Integer.parseInt(exeMk) >= Integer.parseInt(mexcLvl))
                return true;
            else
                return false;
        }
    ...
    public ReviewFeeDecisionDetailCase getDetailData() {
        return detailData;
    }

    public void setDetailData(ReviewFeeDecisionDetailCase detailData) {
        this.detailData = detailData;
    }

    public List<ReviewFeeDecisionDetailCase> getDetailList() {
        return detailList;
    }

    public void setDetailList(List<ReviewFeeDecisionDetailCase> detailList) {
