...
public class MonthlyBatchRptReport extends ReportBase {

    private RptService rptService;

    public MonthlyBatchRptReport() throws Exception {
        super();
    }
    ...
    public ByteArrayOutputStream doReport(UserBean userData, List<MonthlyBatchRptCase> caseData, HashMap<String, Object> map) throws Exception {
        try {
            ...
            if (caseData == null || caseData.isEmpty()) {
                ...
                boolean isPrintHead = true;// 是否需要列印表頭
                ...
                for (int i = 0; i < caseData.size(); i++) { // 受理案件資料
                    MonthlyBatchRptCase monthlyBatchRptCase = caseData.get(i);
                    ...
                    // 受理編號
                    addColumn(table, 7, 1, monthlyBatchRptCase.getApNoStr(), fontCh10, 1, CENTER);
                    // 給付年月
                    addColumn(table, 4, 1, monthlyBatchRptCase.getPaysYmStr() + "\n" + monthlyBatchRptCase.getPayeYmStr(), fontCh10, 1, CENTER);
                    // 姓名
                    addColumn(table, 4, 1, monthlyBatchRptCase.getEvtName(), fontCh10, 1, CENTER);
                    // 異動狀況
                    addColumn(table, 13, 1, monthlyBatchRptCase.getUpCauseCodeStr(), fontCh10, 1, CENTER);
                    // 印表日期
                    addColumn(table, 4, 1, monthlyBatchRptCase.getCprnDateStr(), fontCh10, 1, RIGHT);
                    // 清單頁次
                    addColumn(table, 4, 1, monthlyBatchRptCase.getCprnPage(), fontCh10, 1, RIGHT);
                    // 種類
                    addColumn(table, 4, 1, monthlyBatchRptCase.getCaseTypStr(), fontCh10, 1, CENTER);
