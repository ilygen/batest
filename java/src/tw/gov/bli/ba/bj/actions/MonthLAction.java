 ...
 * @author Noctis
 */
public class MonthLAction extends BaseDispatchAction {
    private static Log log = LogFactory.getLog(MonthLAction.class);
    ...
    /**
     * 批次處理 - 線上月核定作業 - 老年年金線上月核定作業 - 查詢頁面 (baba0m110x.jsp) 按下 確認
     * 
     * @param mapping
     * @param form
     * @param request
    ...
    public ActionForward doQuery(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 批次處理 - 線上月核定作業 - 老年年金線上月核定作業 - 查詢頁面  MonthLAction.doQuery() 開始 ... ");

        HttpSession session = request.getSession();
        UserBean userData = (UserBean) UserSessionHelper.getUserData(request);
        MonthLForm iform = (MonthLForm) form;
        ...
        try {
            ...
            if (checkCount > 0) {
                ...
                List<MonthLCase> dataList = bjService.selectMonthLDataListBy(payCode, DateUtility.changeChineseYearMonthType(iform.getIssuYm()));
                ...
                if (dataList.size() < 1) {
                    ...
                    CaseSessionHelper.setMonthLCaseList(dataList, request);
            ...
            // 設定查詢失敗訊息
            log.error("MonthLAction.doQuery() 發生錯誤:" + ExceptionUtility.getStackTrace(e));
    ...
    /**
     * 批次處理 - 線上月核定作業 - 老年年金線上月核定作業 - 查詢頁面 (baba0m110x.jsp) 按下 查詢
     * 
     * @param mapping
     * @param form
     * @param request
    ...
    public ActionForward doQueryList(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 批次處理 - 線上月核定作業 - 老年年金線上月核定作業 - 查詢頁面  MonthLAction.doQueryList() 開始 ... ");
        ...
        try {
            ...
            List<MonthLCase> dataList = bjService.selectMonthLQueryDataListBy(payCode, DateUtility.changeChineseYearMonthType(iform.getIssuYm()), DateUtility.changeDateType(iform.getChkDate(), false));

            if (dataList.size() < 1) {

                saveMessages(session, DatabaseMessageHelper.getNoResultMessage());
                log.debug("批次處理 - 線上月核定作業 - 老年年金線上月核定作業 - 查詢頁面  MonthLAction.doQueryList() 完成 ... ");
            ...
            // 設定查詢失敗訊息
            log.error("MonthLAction.doQueryList() 發生錯誤:" + ExceptionUtility.getStackTrace(e));
            saveMessages(session, DatabaseMessageHelper.getQueryFailMessage());
            return mapping.findForward(ConstantKey.FORWARD_FAIL);
        }
    }
    ...
    /**
     * 批次處理 - 線上月核定作業 - 老年年金線上月核定作業 - 清單頁面 (baba0m111q.jsp) 按下 確認
     * 
     * @param mapping
     * @param form
     * @param request
    ...
    public ActionForward doConfirm(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 批次處理 - 線上月核定作業 - 老年年金線上月核定作業 - 清單頁面  MonthLAction.doConfirm() 開始 ... ");
        ...
        try {
            ...
            // 取得清單資料
            List<MonthLCase> dataList = CaseSessionHelper.getMonthLCaseList(request);
            ...
            for (int i = 0; i < tmpIndexList.length; i++) {
                ...
                if (StringUtils.isNotBlank(dataIndex)) {
                    ...
                    MonthLCase caseObj = (MonthLCase) dataList.get(Integer.parseInt(dataIndex));
                    ...
                    if (checkCount > 0) {
                        ...
                        // 更新 勞保年金線上作業檔
                        bjService.updateBaonlinejobDataForMonthL(caseObj, issuYm, procMsg, userData);
            ...
            // 更新頁面資料
            List<MonthLCase> dataListNew = new ArrayList<MonthLCase>();

            for (int i = 0; i < apNoList.size(); i++) {
                List<MonthLCase> caseData = bjService.selectMonthLApprovedDataListBy(apNoList.get(i), DateUtility.changeChineseYearMonthType(iform.getIssuYm()));
            ...
            if (dataListNew.size() < 1) {
                ...
                saveMessages(session, DatabaseMessageHelper.getNoResultMessage());
                log.debug("批次處理 - 線上月核定作業 - 老年年金線上月核定作業 - 清單頁面  MonthLAction.doConfirm() 完成 ... ");
                return mapping.findForward(FORWARD_QUERY_LIST_FAIL);
                ...
                // 將 查詢結果清單 存到 Request Scope
                CaseSessionHelper.setMonthQueryCaseForTitle(caseData, request);
                CaseSessionHelper.setMonthLCaseList(dataListNew, request);
            ...
            // 設定查詢失敗訊息
            log.error("MonthLAction.doConfirm() 發生錯誤:" + ExceptionUtility.getStackTrace(e));
            saveMessages(session, DatabaseMessageHelper.getQueryFailMessage());
            return mapping.findForward(ConstantKey.FORWARD_FAIL);
        }
    }
    ...
    /**
     * 批次處理 - 線上月核定作業 - 老年年金線上月核定作業 - 清單頁面 (baba0m111q.jsp) 按下返回
     * 
     * @param mapping
     * @param form
     * @param request
    ...
    public ActionForward doDelete(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 批次處理 - 線上月核定作業 - 老年年金線上月核定作業 - 清單頁面  MonthLAction.doDelete() 開始 ... ");
        ...
        try {
            ...
            for (int i = 0; i < tmpIndexList.length; i++) {
                ...
                if (StringUtils.isNotBlank(dataIndex)) {
                    ...
                    bjService.updateBaonlinejobDataForMonthLDelete(caseObj, userData);

                }
            }

            // 更新頁面資料
            List<MonthLCase> dataListNew = bjService.selectMonthLDataListBy(payCode, DateUtility.changeChineseYearMonthType(iform.getIssuYm()));
            ...
            log.debug("批次處理 - 線上月核定作業 - 老年年金線上月核定作業 - 清單頁面  MonthLAction.doDelete() 完成 ... ");
            return mapping.findForward(ConstantKey.FORWARD_QUERY_SUCCESS);

        }
        catch (Exception e) {
            // 設定查詢失敗訊息
            log.error("MonthLAction.doDelete() 發生錯誤:" + ExceptionUtility.getStackTrace(e));
            saveMessages(session, DatabaseMessageHelper.getQueryFailMessage());
            return mapping.findForward(ConstantKey.FORWARD_FAIL);
        }
    }
    ...
    /**
     * 批次處理 - 線上月核定作業 - 老年年金線上月核定作業 - 資料頁面 (baba0m111q.jsp,baba0m112a.jsp) 按下 返回
     * 
     * @param mapping
     * @param form
     * @param request
    ...
    public ActionForward doBack(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 批次處理 - 線上月核定作業 - 老年年金線上月核定作業 - 返回 MonthLAction.doBack() 開始 ... ");
        ...
        CaseSessionHelper.removeMonthLCaseList(request);

        log.debug("執行批次處理 - 線上月核定作業 - 老年年金線上月核定作業 - 返回 MonthLAction.doBack() 完成 ... ");
