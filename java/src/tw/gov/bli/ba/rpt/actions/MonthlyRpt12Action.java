...
import tw.gov.bli.ba.rpt.forms.MonthlyRpt12Form;
 ...
 * @author swim
 */
public class MonthlyRpt12Action extends BaseDispatchAction {
    private static Log log = LogFactory.getLog(MonthlyRpt12Action.class);

    private RptService rptService;

    /**
     * 列印作業 - 月核定處理相關報表 - 退匯清冊 - 查詢頁面 (balp0d3c0l.jsp) <br>
     * 按下「列印」的動作
     * 
     * @param mapping
     * @param form
    ...
    public ActionForward doReport(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 列印作業 - 月核定處理相關報表 - 退匯清冊 - 查詢頁面 MonthlyRpt12Action.doReport() 開始 ... ");

        HttpSession session = request.getSession();
        UserBean userData = (UserBean) UserSessionHelper.getUserData(request);

        MonthlyRpt12Form queryForm = (MonthlyRpt12Form) form;
        ...
        try {
            ...
            // 抓報表資料
            List<MonthlyRpt12Case> caseDataList = rptService.getMonthlyRpt12DataBy(payCode, DateUtility.changeChineseYearMonthType(issuYm), DateUtility.changeDateType(chkDate));
            // 報表列印資料
            List<MonthlyRpt12Case> reportData = new ArrayList<MonthlyRpt12Case>();

            if (caseDataList.size() > 0) {

                // 【NACHGMK is null】
                for (MonthlyRpt12Case caseData : caseDataList) {
            ...
            if (caseDataList.size() == 0) {
                ...
                response.setHeader("Content-disposition", "attachment; filename=\"" + Encode.forJava(payCode) + "_MonthlyRpt12_" + Encode.forJava(printDate) + ".pdf" + "\"");
            ...
            // 設定查詢失敗訊息
            log.error("產製 退匯清冊 發生錯誤:" + ExceptionUtility.getStackTrace(e));
