...
/**
 * Case for 勞保遺屬年金給付受理編審清單 - 核定資料
 * 
 * @author Evelyn Hsu
 */

public class SurvivorReviewRpt01DecideDataCase implements Serializable {
    ...
    private BigDecimal oldExtraRate; // 遺屬展延
    ...
    public String getOldRateSdateString() {
        ...
        if (StringUtils.length(oldRateSdate) == 6)
            return DateUtility.formatChineseYearMonthString(DateUtility.changeWestYearMonthType(oldRateSdate), false);
        else
            return StringUtils.defaultString(oldRateSdate);
    }
    ...
    public String getOldRateEdateString() {
        ...
        if (StringUtils.length(oldRateEdate) == 6)
            return DateUtility.formatChineseYearMonthString(DateUtility.changeWestYearMonthType(oldRateEdate), false);
        else
            return StringUtils.defaultString(oldRateEdate);
    }
