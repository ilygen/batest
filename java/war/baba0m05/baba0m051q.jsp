...
<html:html lang="true">
<head>
    <acl:setProgId progId="BABA0M051Q" />
    ...
    <html:javascript formName="MonthBatchLForm" page="1" />	       
...
<div id="mainContent">
    ...
    <div id="main" class="mainBody">
        <html:form action="/monthBatchL" method="post" onsubmit="">
        <html:hidden styleId="page" property="page" value="1" />
        <html:hidden styleId="method" property="method" value="" />
        
        <fieldset>
            <legend>&nbsp;老年年金批次月處理作業&nbsp;</legend>
                ...
                <tr>
                    <td colspan="2" align="right">
                        <acl:checkButton buttonName="btnBack">
                            <input type="button" name="btnBack" class="button" value="返　回" onclick="$('page').value='1'; $('method').value='doBack'; document.MonthBatchLForm.submit();" />&nbsp;
                        </acl:checkButton>                                             
                    </td>
                </tr>    
                <tr align="center">
                    <td>
                        <bean:define id="list" name="<%=ConstantKey.MONTH_BATCH_L_CASE_LIST%>" />
                        <display:table name="pageScope.list" id="listItem" pagesize="<%=ConstantKey.LIST_PAGE_SIZE%>">
                            ...
                            <display:column title="月處理檔案" style="width:14%; text-align:center;" class="issueinfo_C" headerClass="issuetitle_L">
                                <acl:checkButton buttonName="btnUpdate">
                                    <c:if test='${(listItem.fileName ne "")}'>
                                      <input type="button" class="button" name="btnDownLoad" value="檔案下載" onclick="location.href='<c:url value="/monthBatchL.do"/>?method=doDownLoad&fileName=<c:out value="${listItem.fileName}" />';">
                                    </c:if>
                            ...
                            <display:column title="處理狀態明細" style="width:14%; text-align:center;" class="issueinfo_C" headerClass="issuetitle_L">
                                <acl:checkButton buttonName="btnDetail">
                                    <input type="button" class="button" name="btnDetail" value="狀態明細" onclick="location.href='<c:url value="/monthBatchL.do"/>?method=doDetail&baJobId=<c:out value="${listItem.baJobId}" />';">
                                </acl:checkButton>
                            </display:column>
                        </display:table>
                    </td>
