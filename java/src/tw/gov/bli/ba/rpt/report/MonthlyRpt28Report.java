 ...
 * @author Noctis
 */
public class MonthlyRpt28Report extends ReportBase {

    public MonthlyRpt28Report() throws Exception {
        super();
    }
    ...
    public Table getHeader(boolean bPrintMainHeader, String payCode, String mchkTypStr, String payTypStr, String payYm, String issuYm, String payTyp, HashMap<String, Object> map, String cPrnDate, String naChgMk, String nlWkMk, String adWkMk,
        ...
        if (bPrintMainHeader) {
            ...
            if (receiveType.equals("1")) {
                addColumn(table, 38, 1, "退匯應收已收註銷清冊", fontCh18, 0, CENTER);
            }
            else {
                addColumn(table, 38, 1, "退現應收已收註銷清冊", fontCh18, 0, CENTER);
            }
            addColumn(table, 14, 1, "", fontCh10, 0, CENTER);
    ...
    public void addFooter(Table table, String DeptName, List<MonthlyRpt28Case> amtList, Integer rowCount, List<MonthlyRpt28Case> caseList, String issuTyp, String payTyp, Integer rowBeg, Integer rowEnd) throws Exception {
        ...
        for (int i = 0; i < rowCount; i++) {
            MonthlyRpt28Case amtData = amtList.get(i);
        ...
        for (int j = rowBeg; j < rowEnd; j++) {
            MonthlyRpt28Case caseData = caseList.get(j);
    ...
    public ByteArrayOutputStream doReport(UserBean userData, List<MonthlyRpt28Case> caseData, HashMap<String, Object> map) throws Exception {
        try {
            ...
            // FileOutputStream bao = new FileOutputStream(PropertyHelper.getProperty("rpt.path") + Encode.forJava(payCode) + "_MonthlyRpt28_" + Encode.forJava(printDate) + ".pdf");
            // FileOutputStream bao = new FileOutputStream("C:\\" + "MonthlyRpt28_" + printDate + ".pdf");
            ...
            List<MonthlyRpt28Case> amtList = new ArrayList<MonthlyRpt28Case>();
            ...
            for (int i = 0; i < caseData.size(); i++) { // 受理案件資料
                MonthlyRpt28Case monthlyRpt28Case = caseData.get(i);
                BigDecimal rptPage = monthlyRpt28Case.getRptPage();
                if (i == 0) {
                    // 取得表頭
                    table = getHeader(true, payCode, monthlyRpt28Case.getIssuTypStr(), monthlyRpt28Case.getPayTypStr(), monthlyRpt28Case.getPayYm(), monthlyRpt28Case.getIssuYm(), monthlyRpt28Case.getPayTyp(), map, monthlyRpt28Case.getcPrnDateStr(),
                                    monthlyRpt28Case.getNaChgMk(), monthlyRpt28Case.getNlWkMk(), monthlyRpt28Case.getAdWkMk(), monthlyRpt28Case.getIsNaChgMk(), receiveType, rptPage);
                }
                else {
                    // if(((rowCount) % 20 == 0) || !(monthlyRpt28Case.getIssuTyp()).equals(issuTyp) || !(monthlyRpt28Case.getPayTyp()).equals(payTyp)){
                    if (((rowCount) % 20 == 0) || !(monthlyRpt28Case.getApNoLastDigit()).equals(apNoLastDigit) || !(monthlyRpt28Case.getPayTyp()).equals(payTyp)) {
                        // 不滿20筆補空白行
                        if (rowCount % 20 > 0)
                            addEmptyRow(table, 20 - (rowCount % 20));

                        // 加入表尾
                        addFooter(table, monthlyRpt28Case.getDeptNameString(), amtList, rowCount, caseData, issuTyp, payTyp, rowBeg, i);
                        document.add(table);

                        // 取得表頭
                        table = getHeader(true, payCode, monthlyRpt28Case.getIssuTypStr(), monthlyRpt28Case.getPayTypStr(), monthlyRpt28Case.getPayYm(), monthlyRpt28Case.getIssuYm(), monthlyRpt28Case.getPayTyp(), map,
                                        monthlyRpt28Case.getcPrnDateStr(), monthlyRpt28Case.getNaChgMk(), monthlyRpt28Case.getNlWkMk(), monthlyRpt28Case.getAdWkMk(), monthlyRpt28Case.getIsNaChgMk(), receiveType, rptPage);

                        // if (!(monthlyRpt28Case.getIssuTyp()).equals(issuTyp) || !(monthlyRpt28Case.getPayTyp()).equals(payTyp)) {
                        if (!(monthlyRpt28Case.getApNoLastDigit()).equals(apNoLastDigit) || !(monthlyRpt28Case.getPayTyp()).equals(payTyp)) {
                            // 序號歸零
                            seqNo = 1;
                            rowBeg = i;
                        }
                ...
                // 案件類別
                issuTyp = monthlyRpt28Case.getIssuTyp();
                // 給付方式
                payTyp = monthlyRpt28Case.getPayTyp();
                // 部門名稱
                deptName = monthlyRpt28Case.getDeptNameString();
                // 受理編號的最後一碼
                apNoLastDigit = monthlyRpt28Case.getApNoLastDigit();

                // 序號
                addColumn(table, 3, 1, Integer.toString(seqNo), fontCh9, 1, CENTER);
                // 受理編號
                addColumn(table, 7, 1, monthlyRpt28Case.getApNoStr(), fontCh9, 1, CENTER);
                // 給付年月
                addColumn(table, 3, 1, DateUtility.changeWestYearMonthType(monthlyRpt28Case.getPayYm()), fontCh9, 1, CENTER);
                // 金融機構轉帳帳號
                if (payTyp.equals(ConstantKey.BAAPPBASE_PAYTYP_STR_A))
                    addColumn(table, 11, 1, monthlyRpt28Case.getBenIdn(), fontCh9, 1, CENTER);
                else
                    addColumn(table, 11, 1, monthlyRpt28Case.getPayBankIdStr(), fontCh9, 1, CENTER);
                // 受款人姓名
                addColumn(table, 5, 1, monthlyRpt28Case.getBenName(), fontCh9, 1, CENTER);
                // 核定金額(A)
                addColumn(table, 5, 1, ReportBase.formatNumber(monthlyRpt28Case.getIssueAmt().longValue()), fontCh9, 1, RIGHT);
                // 另案扣減同保險(B)
                addColumn(table, 4, 1, ReportBase.formatNumber(monthlyRpt28Case.getOtheraAmt().longValue()), fontCh9, 1, RIGHT);
                // 另案扣減他保險(C)
                addColumn(table, 4, 1, ReportBase.formatNumber(monthlyRpt28Case.getOtherbAmt().longValue()), fontCh9, 1, RIGHT);
                // 匯費(D)
                addColumn(table, 4, 1, ReportBase.formatNumber(monthlyRpt28Case.getMitRate().longValue()), fontCh9, 1, RIGHT);
                // 抵銷紓困(E)
                addColumn(table, 4, 1, ReportBase.formatNumber(monthlyRpt28Case.getOffsetAmt().longValue()), fontCh9, 1, RIGHT);
                // 代扣補償金(F)
                addColumn(table, 4, 1, ReportBase.formatNumber(monthlyRpt28Case.getCompenAmt().longValue()), fontCh9, 1, RIGHT);
                // 保留遺屬津貼(G)
                addColumn(table, 4, 1, ReportBase.formatNumber(monthlyRpt28Case.getInherItorAmt().longValue()), fontCh9, 1, RIGHT);
                // 代扣利息所得稅(H)
                addColumn(table, 4, 1, ReportBase.formatNumber(monthlyRpt28Case.getItrtTax().longValue()), fontCh9, 1, RIGHT);
                // 其他(I)
                addColumn(table, 4, 1, ReportBase.formatNumber(monthlyRpt28Case.getOtherAmt().longValue()), fontCh9, 1, RIGHT);

                // 計算本頁小計用
                amtList.add(monthlyRpt28Case);
                 ...
                 * monthlyRpt28Case.getPayCodeStr(), monthlyRpt28Case.getIssuTypStr(), monthlyRpt28Case.getPayTypStr(), monthlyRpt28Case.getPayYm(), monthlyRpt28Case.getPayTyp(), map); rowCount = 0; } else { // 刪除造成換頁的該筆資料 (一筆有一行)
            ...
            printToDisk((PropertyHelper.getProperty("rpt.path") + Encode.forJava(payCode) + "_MonthlyRpt28_" + Encode.forJava(printDate) + ".pdf"), bao);
        }
        finally {
            document.close();
        }
