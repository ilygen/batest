...
<html:html lang="true">
<head>
    <acl:setProgId progId="BASU0D011A" />
    ...
    <html:javascript formName="ExecutiveSupportMaintAddForm" page="1" />
    ...
    function changeOption(obj){
        ...
        if($("letterType").value == "31"){
            ...
            $("injserContent").style.display="inline";
            
        }else if($("letterType").value == "17"){
        	$("reliefKindContent").style.display="none";
            $("reliefStatContent").style.display="none";
            $("reliefTypContent").style.display="none";
            $("injserContent").style.display="none";
        	$("paymkContent").style.display="inline";
    ...
    function initAll(){
        $("proDate").value = '<%=DateUtility.getNowChineseDate()%>';
        $("promoteUser").value = '<c:out value ="<%=userData.getEmpNo()%>" />';
        // 當「處理函別」=31時,才要顯示欄位
        $("reliefKindContent").style.display="none";
        $("reliefStatContent").style.display="none";
        $("reliefTypContent").style.display="none";
        $("injserContent").style.display="inline";
        $("paymkContent").style.display="none";
        // 銷案日期
        $("closDate").disabled = true;
        // 管制註記
        $("delMk").disabled = true;
    ...
    function checkRequireFields(q2) {
        ...
        //處理函別欄位值為： 21不給付函時，則承辦日期不可小於系統日期
        if($("letterType").value == 21 && Trim($("proDate").value) < nowDate){
            msg += '「21-不給付函」承辦日期不可小於系統日期。\r\n';
        }
        ...
        if (msg != "") {
            ...
            document.ExecutiveSupportMaintAddForm.submit();
        }      
    }
...
<body scroll="no"  onload="checkClosDate()">
...
<div id="mainContent">
    ...
    <div id="main" class="mainBody">
        <html:form action="/executiveSupportMaintAdd" method="post" onsubmit="return validateExecutiveSupportMaintAddForm(this)">
            ...
            <tr>
                <td align="right">
                    ...
                    <acl:checkButton buttonName="btnSave">
                        <input type="button" name="btnSave" id="btnSave" class="button" value="確 認" onclick="$('page').value='1'; $('method').value='doSave'; if (document.ExecutiveSupportMaintAddForm.onsubmit() && checkType21_2()){getStatus();}else{return false;}" />
                    </acl:checkButton>
                    &nbsp;
                    ...
                    &nbsp;
                    <acl:checkButton buttonName="btnReset">
                        <input type="button" name="btnBack" class="button" value="返  回" onclick="$('page').value='1'; $('method').value='doBack'; document.ExecutiveSupportMaintAddForm.submit();" />
                    </acl:checkButton>
                </td>
            </tr>
            <tr>
                    ...
                    <table width="98%" cellpadding="3" cellspacing="4" class="px13">
                        <tr>
                            ...
                            <td id="iss" width="50%">
                                ...
                                <html:text styleId="proDate" property="proDate" size="10" maxlength="7" styleClass="textinput" />
                            </td>
                        </tr>
                        <tr>
                            <td id="iss" colspan="2">
