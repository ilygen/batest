 ...
 * @author Noctis
 */
public class MonthlyRpt27Report extends ReportBase {

    private RptService rptService;

    public MonthlyRpt27Report() throws Exception {
        super();
    }
    ...
    public Table getHeader(boolean bPrintMainHeader, String payCode, String payTypStr, String chkDate, String pageNum, String cPrnDate, String naChgMk, String nlWkMk, String adWkMk, String isNaChgMk, String receiveType) throws Exception {
        ...
        if (bPrintMainHeader) {
            ...
            if (receiveType.equals("1")) {
                addColumn(table, 99, 1, "退匯應收已收註銷核定清單", fontCh10, 0, CENTER);
            }
            else {
                addColumn(table, 99, 1, "退現應收已收註銷核定清單", fontCh10, 0, CENTER);
            }
            if (!("").equals(payTypStr)) {
    ...
    public ByteArrayOutputStream doReport(UserBean userData, List<MonthlyRpt27Case> caseData, List<MonthlyRpt27Case> caseDataFooter, HashMap<String, Object> map) throws Exception {
        try {
            ...
            Map<String, List<MonthlyRpt27Case>> dataMap = new TreeMap<String, List<MonthlyRpt27Case>>();
            Map<String, List<MonthlyRpt27Case>> dataMapFooter = new TreeMap<String, List<MonthlyRpt27Case>>(); // 20120406
            for (int i = 0; i < caseData.size(); i++) {
                MonthlyRpt27Case obj = caseData.get(i);
                dataMap.put(obj.getPayTyp(), new ArrayList<MonthlyRpt27Case>());
            }
            ...
            for (int i = 0; i < caseData.size(); i++) { // 20120416
                MonthlyRpt27Case obj = caseData.get(i);
                dataMapFooter.put(obj.getPayTyp(), new ArrayList<MonthlyRpt27Case>());
            }

            for (int i = 0; i < caseDataFooter.size(); i++) { // 20120416
                MonthlyRpt27Case obj = caseDataFooter.get(i);
            ...
            for (Iterator it = dataMap.keySet().iterator(); it.hasNext();) {
                ...
                List<MonthlyRpt27Case> caseList = dataMap.get(key);
                MonthlyRpt27Case tmpCaseObj = caseList.get(0);

                // 拿取頁次 20130731
                List<MonthlyRpt27Case> caseListPageNum = dataMapFooter.get(key);
                ...
                table = getHeader(true, payCode, tmpCaseObj.getPayTypStr(), tmpCaseObj.getChkDateStr(), pageNum, tmpCaseObj.getcPrnDateStr(), tmpCaseObj.getNaChgMk(), tmpCaseObj.getNlWkMk(), tmpCaseObj.getAdWkMk(), tmpCaseObj.getIsNaChgMk(),
                                receiveType);
                for (MonthlyRpt27Case caseObj : caseList) {
                ...
                table.setBorderWidth(1);
                int rowCount = 0;
                List<MonthlyRpt27Case> caseList1 = dataMapFooter.get(key);
                for (MonthlyRpt27Case caseObj1 : caseList1) {
            ...
            printToDisk((PropertyHelper.getProperty("rpt.path") + Encode.forJava(payCode) + "_MonthlyRpt27_" + Encode.forJava(printDate) + ".pdf"), bao);
        }
        finally {
            document.close();
        }
