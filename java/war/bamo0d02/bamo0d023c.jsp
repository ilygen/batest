...
<html:html lang="true">
<head>
    <acl:setProgId progId="BAMO0D023C" />
            ...
            <table width="98%" border="0" align="center" cellpadding="2" cellspacing="2" class="px13">
                <tr>
                    <td colspan="7" align="right">
                        <acl:checkButton buttonName="btnBack">
                            <input type="button" name="btnBack" class="button" value="返　回" onclick="$('page').value='1'; $('method').value='doBack'; document.PaymentDataUpdateForm.submit();"/>
                        </acl:checkButton>                        
                    </td>
                </tr> 
                <tr>
                ...
                <tr align="center">
                    <td colspan="6" class="issuetitle_L">
                        <bean:define id="list" name="<%=ConstantKey.PAYMENT_DATA_UPDATE_LIST%>" />
                        <display:table name="pageScope.list" id="listItem" pagesize="<%=ConstantKey.LIST_PAGE_SIZE%>">
                            ...
                            <display:column title="受款人姓名" headerClass="issuetitle_L" class="issueinfo" style="width:16%; text-align:left;">
                                <c:out value="${listItem.benName}" />&nbsp;
                            </display:column>
                            <display:column title="受款人身分證號" headerClass="issuetitle_L" class="issueinfo" style="width:15%; text-align:left;">
                                <c:out value="${listItem.benIdnNo}" />&nbsp;
                            </display:column>
                            <display:column title="關　係" headerClass="issuetitle_L" class="issueinfo" style="width:14%; text-align:left;">
                            ...
                            <display:column title="給付方式" headerClass="issuetitle_L" class="issueinfo" style="width:16%; text-align:left;">
                                <c:if test='${(listItem.payTyp eq "1")}'>
                                    <c:out value="<%=ConstantKey.BAAPPBASE_PAYTYP_CODESTR_1%>" />
                                </c:if>
                                <c:if test='${(listItem.payTyp eq "2")}'>
                                    <c:out value="<%=ConstantKey.BAAPPBASE_PAYTYP_CODESTR_2%>" />
                                </c:if>
                                <c:if test='${(listItem.payTyp eq "3")}'>
                                    <c:out value="<%=ConstantKey.BAAPPBASE_PAYTYP_CODESTR_3%>" />
                                </c:if>
                                <c:if test='${(listItem.payTyp eq "4")}'>
                                    <c:out value="<%=ConstantKey.BAAPPBASE_PAYTYP_CODESTR_4%>" />
                                </c:if>
                                <c:if test='${(listItem.payTyp eq "5")}'>
                                    <c:out value="<%=ConstantKey.BAAPPBASE_PAYTYP_CODESTR_5%>" />
                                </c:if>
                                <c:if test='${(listItem.payTyp eq "6")}'>
                                    <c:out value="<%=ConstantKey.BAAPPBASE_PAYTYP_CODESTR_6%>" />
                                </c:if>
                                <c:if test='${(listItem.payTyp eq "7")}'>
                                    <c:out value="<%=ConstantKey.BAAPPBASE_PAYTYP_CODESTR_7%>" />
                                </c:if>
                                <c:if test='${(listItem.payTyp eq "8")}'>
                                    <c:out value="<%=ConstantKey.BAAPPBASE_PAYTYP_CODESTR_8%>" />
                                </c:if>
                                <c:if test='${(listItem.payTyp eq "9")}'>
                                    <c:out value="<%=ConstantKey.BAAPPBASE_PAYTYP_CODESTR_9%>" />
                                </c:if>
                                <c:if test='${(listItem.payTyp eq "A")}'>
                                    <c:out value="<%=ConstantKey.BAAPPBASE_PAYTYP_CODESTR_A%>" />
                                </c:if>&nbsp;
                            </display:column>
                            <display:column title="帳號" headerClass="issuetitle_L" class="issueinfo" style="width:19%; text-align:left;">
                                <c:choose>
                                        <c:when test="${(listItem.payTyp eq '1')and(listItem.payBankId ne '700')}">
                                                <c:out value="${listItem.payBankId}" />-0000-<c:out value="${listItem.payEeacc}" />&nbsp;
                                        </c:when>
                                        <c:otherwise>
                                                <c:out value="${listItem.payBankId}" />-<c:out value="${listItem.branchId}" />-<c:out value="${listItem.payEeacc}" />&nbsp;
                                        </c:otherwise>
                                </c:choose>
                            </display:column>
                            <display:column title="資料明細" headerClass="issuetitle_L" class="issueinfo_C" style="width:10%; text-align:center;">
                                <acl:checkButton buttonName="btnModify">
                                    ...
                                    <c:if test="${(listItem.pagePayKind ne 'K')or((listItem.pagePayKind eq 'K')and(listItem.benDieDate eq ''))}">
                                        <input type="button" name="btnModify" class="button" value="修　改" onclick="$('page').value='1'; $('method').value='doModifyDetail'; $('baappbaseId').value='<c:out value="${listItem.baappbaseId}" />'; document.PaymentDataUpdateForm.submit();" />&nbsp;
                                    </c:if>
                                    --%>
