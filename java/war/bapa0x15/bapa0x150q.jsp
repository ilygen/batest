...
<html:html lang="true">
<head>
<acl:setProgId progId="BAPA0X150Q" />
...
<script language="javascript" type="text/javascript">
	...
	function checkFields() {
		if (Trim($F("apNo1")).length == 0 || Trim($F("apNo2")).length == 0
				|| Trim($F("apNo3")).length == 0
				|| Trim($F("apNo4")).length == 0) {
			alert("請輸入受理編號");
			return false;
		}
		var msg = "";
...
<%-- begin ... [ --%>
	function resetForm() {
		document.ResetPaymentParameterForm.reset();
		initAll();
	}
<%-- ] ... end --%>
...
<body scroll="no">
	...
	<div id="mainContent">
		...
		<div id="main" class="mainBody">
			<html:form action="/resetPaymentParameter" method="post" onsubmit="">
						...
						<tr>
							<td colspan="2" align="right">
								<acl:checkButton buttonName="btnQuery">
									<input type="button" name="btnQuery" class="button" value="確　認" onclick="$('page').value='1'; $('method').value='doQuery'; if(checkFields()){document.ResetPaymentParameterForm.submit();}else{return false;}" />&nbsp;
                        		</acl:checkButton> 
