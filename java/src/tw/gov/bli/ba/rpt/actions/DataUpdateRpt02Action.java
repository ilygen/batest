...
import tw.gov.bli.ba.rpt.forms.DataUpdateRpt02Form;
import tw.gov.bli.ba.rpt.report.DataUpdateRpt02Report;
 ...
 * @author Goston
 */
public class DataUpdateRpt02Action extends BaseDispatchAction {
    private static Log log = LogFactory.getLog(DataUpdateRpt02Action.class);

    private RptService rptService;

    /**
     * 列印作業 - 更正作業相關報表 - 紓困貸款撥款情形查詢清單 - 查詢頁面 (balp0d120l.jsp) <br>
     * 按下「列印」的動作
     * 
     * @param mapping
     * @param form
    ...
    public ActionForward doReport(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 列印作業 - 更正作業相關報表 - 紓困貸款撥款情形查詢清單 - 查詢頁面 DataUpdateRpt02Action.doReport() 開始 ... ");

        HttpSession session = request.getSession();
        UserBean userData = (UserBean) UserSessionHelper.getUserData(request);

        DataUpdateRpt02Form queryForm = (DataUpdateRpt02Form) form;
        ...
        try {
            ...
            // 抓報表資料
            DataUpdateRpt02Case caseData = rptService.getDataUpdateRpt02DataBy(apNo);
            ...
            if (caseData == null) {
                ...
                DataUpdateRpt02Report report = new DataUpdateRpt02Report();
                ...
                response.setHeader("Content-disposition", "attachment; filename=\"DataUpdateRpt02_" + Encode.forJava(apNo) + ".pdf" + "\"");
            ...
            // 設定查詢失敗訊息
            log.error("產製 紓困貸款撥款情形查詢清單 發生錯誤:" + ExceptionUtility.getStackTrace(e));
