...
/**
 * 列印作業 - 其他類報表 - 已退匯止付尚未應收款立帳案件清單
 * 
 * @author Noctis
 */
public class OtherRpt03Report extends ReportBase {
    private String printDate = ""; // 印表日期
    private static final int COUNT = 20; // 報表每頁筆數

    public OtherRpt03Report() throws Exception {
        super();
    ...
    public PdfPTable doReportHeader(String payCode) throws Exception {
        ...
        addPdfPCell(table, 100, "已退匯止付尚未應收款立帳案件清單", fontCh14b, 0, 1, CENTER);
        addPdfPCell(table, 100, "　", fontCh14b, 0, 1, CENTER);

        if(payCode.equals("L")){
        	addPdfPCell(table, 15, "給付別：老年年金", fontCh12, 0, 1, LEFT);
        }else if(payCode.equals("K")){
        	addPdfPCell(table, 15, "給付別：失能年金" + StringUtils.defaultString(payCode), fontCh12, 0, 1, LEFT);
        }else if(payCode.equals("S")){
        	addPdfPCell(table, 15, "給付別：遺屬年金" + StringUtils.defaultString(payCode), fontCh12, 0, 1, LEFT);
        }else{
        	addPdfPCell(table, 15, "給付別：", fontCh12, 0, 1, LEFT);
        }
        addPdfPCell(table, 70, "", fontCh12, 0, 1, LEFT);
        addPdfPCell(table, 15, "列印日期：" + StringUtils.defaultString(printDate), fontCh12, 0, 1, LEFT);
        
        addPdfPCell(table, 7, "序號", fontCh12, 1, 15, CENTER);
        addPdfPCell(table, 15, "受理編號", fontCh12, 1, 15, CENTER);
        addPdfPCell(table, 16, "核定年月 ", fontCh12, 1, 15, CENTER);
        addPdfPCell(table, 16, "給付年月", fontCh12, 1, 15, CENTER);
        addPdfPCell(table, 10, "受款人序", fontCh12, 1, 15, CENTER);
        addPdfPCell(table, 18, "退匯(退現)日期", fontCh12, 1, 15, CENTER);
        addPdfPCell(table, 18, "退匯(退現)金額", fontCh12, 1, 15, CENTER);
        
        return table;
    }
    
    public ByteArrayOutputStream doReport(String payCode, List<OtherRpt03Case> caseList) throws Exception {
        try {
            document.open();
            document.newPage();
            ...
            for ( int i=0; i < caseList.size(); i++){
                addPdfPCell(table, 7, String.valueOf(i+1), fontCh12, 1,15, CENTER);
                addPdfPCell(table, 15, caseList.get(i).getApNo(), fontCh12, 1,15, CENTER);
                String issuYm = DateUtility.changeWestYearMonthType(caseList.get(i).getOriIssuYm()); //核定年月
                addPdfPCell(table, 16, DateUtility.formatChineseYearMonthString(issuYm,false), fontCh12, 1,15, CENTER);
                String payYm = DateUtility.changeWestYearMonthType(caseList.get(i).getPayYm()); //給付年月
                addPdfPCell(table, 16, DateUtility.formatChineseYearMonthString(payYm,false), fontCh12, 1,15, CENTER);
                addPdfPCell(table, 10, caseList.get(i).getSeqNo(), fontCh12, 1,15, CENTER);
                String brChkDate =  DateUtility.changeDateType(caseList.get(i).getBrChkDate()); //退匯（退現）日期
                addPdfPCell(table, 18,DateUtility.formatChineseDateString(brChkDate,false), fontCh12, 1,15, CENTER);
                addPdfPCell(table, 18, formatBigDecimalToInteger(caseList.get(i).getRemitAmt()), fontCh12, 1,15, RIGHT);
                
                
                if ((i+1)%COUNT == 0) { // 每20筆換頁一次
                	document.add(table);
                	document.newPage();
