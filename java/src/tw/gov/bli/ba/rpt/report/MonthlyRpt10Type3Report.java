...
public class MonthlyRpt10Type3Report extends ReportBase {

    private String printDate = ""; // 印表日期

    private HashMap<String, MonthlyRpt10Type1PayAmtCase> payAmtMap; // 代扣補償金

    public MonthlyRpt10Type3Report() throws Exception {
        super();
     ...
     * @throws Exception
     */
    public Table addHeader(MonthlyRpt10Type3RptCase caseData) throws Exception {
        document.newPage();
        ...
        addColumn(table, 15, 1, "報表編號：BALP0D3A0L-3", fontCh10, 0, LEFT);
        ...
        addColumn(table, 24, 1, "郵政匯票　核付清單", fontCh16, 0, CENTER);
    ...
    public ByteArrayOutputStream doReport(UserBean userData, List<MonthlyRpt10Type3RptCase> caseList, HashMap<String, MonthlyRpt10Type1PayAmtCase> payAmtMap, List<MonthlyRpt10Type3RptCase> subCaseList, HashMap<String, Object> map) throws Exception {
        try {
            String payCode = (String) map.get("payCode");

            // 2009.2.20 EvelynHsu add 輸出報表檔至主機上
            String printDate = DateUtility.getNowChineseDate();
            // FileOutputStream bao = new FileOutputStream(PropertyHelper.getProperty("rpt.path") + Encode.forJava(payCode) + "_MonthlyRpt10Type3_" + Encode.forJava(printDate) + ".pdf");
            // FileOutputStream bao = new FileOutputStream("F:\\"+"MonthlyRpt10Type3_" + printDate + ".pdf");
            ...
            for (int i = 0; i < caseList.size(); i++) { // ... [
                MonthlyRpt10Type3RptCase caseData = caseList.get(i);
                ...
                addColumn(table, 9, 1, caseData.getApNoString(), fontCh10, 1, nLine1BorderStyle, LEFT); // 受理編號
                ...
                addColumn(table, 23, 1, caseData.getBenName(), fontCh10, 1, nLine1BorderStyle, LEFT); // 受款人抬頭
                ...
                // ---
                addColumn(table, 9, 1, " ", fontCh10, 1, nLine2BorderStyle, LEFT); // 支票號碼
                addColumn(table, 11, 1, caseData.getBenIdn(), fontCh10, 1, nLine2BorderStyle, LEFT); // 身分證號
                addColumn(table, 23, 1, caseData.getCommZip() + " " + caseData.getCommAddr(), fontCh10, 1, nLine2BorderStyle, LEFT); // 受款人地址
                ...
                // if (i == caseList.size() - 1 || !StringUtils.equals(StringUtils.trimToEmpty(caseData.getPayTyp()) + StringUtils.trimToEmpty(caseData.getBenEvtRel()) + StringUtils.trimToEmpty(caseData.getCompName1()),
                // StringUtils.trimToEmpty(caseList.get(i + 1).getPayTyp()) + StringUtils.trimToEmpty(caseList.get(i + 1).getBenEvtRel() + StringUtils.trimToEmpty(caseList.get(i + 1).getCompName1())))) {
                if (i == caseList.size() - 1 || !StringUtils.equals(StringUtils.trimToEmpty(caseData.getPayTyp()), StringUtils.trimToEmpty(caseList.get(i + 1).getPayTyp()))) {
                    ...
                    while (writer.fitsPage(table)) {
                        ...
                        addColumn(table, 23, 1, "　", fontCh10, 1, nLine1BorderStyle, LEFT);
                        ...
                        addColumn(table, 23, 1, "　", fontCh10, 1, nLine2BorderStyle, LEFT);
                    ...
                    addColumn(table, 9, 1, caseList.get(i + 1).getApNoString(), fontCh10, 1, nLine1BorderStyle, LEFT); // 受理編號
                    ...
                    addColumn(table, 23, 1, caseList.get(i + 1).getBenName(), fontCh10, 1, nLine1BorderStyle, LEFT); // 受款人抬頭
                    ...
                    // ---
                    addColumn(table, 9, 1, caseList.get(i + 1).getAccountNo(), fontCh10, 1, nLine2BorderStyle, LEFT); // 帳 號
                    addColumn(table, 11, 1, caseList.get(i + 1).getBenIdn(), fontCh10, 1, nLine2BorderStyle, LEFT); // 身分證號
                    addColumn(table, 23, 1, caseList.get(i + 1).getCommZip() + " " + caseList.get(i + 1).getCommAddr(), fontCh10, 1, nLine2BorderStyle, LEFT); // 受款人地址
                    addColumn(table, 7, 1, "　", fontCh10, 1, nLine2BorderStyle, RIGHT); // 新台幣：元
            ...
            for (int i = 0; i < subCaseList.size(); i++) { // ... [
                MonthlyRpt10Type3RptCase caseData = subCaseList.get(i);
                ...
                if (i == subCaseList.size() - 1 || !StringUtils.equals(StringUtils.trimToEmpty(caseData.getPayTyp()) + StringUtils.trimToEmpty(caseData.getBenEvtRel()) + StringUtils.trimToEmpty(caseData.getCompName1()),
                    ...
                    // 測試塞下一筆 + 表尾時是否需要換頁
                    addColumnAssignVAlignment(table, 4, 2, String.valueOf(recNo + 2), fontCh10, 1, CENTER, MIDDLE); // 編號
                    addColumn(table, 9, 1, subCaseList.get(i + 1).getApNoString(), fontCh10, 1, nLine1BorderStyle, LEFT); // 受理編號
                    ...
                    addColumn(table, 23, 1, subCaseList.get(i + 1).getBenName(), fontCh10, 1, nLine1BorderStyle, LEFT); // 受款人抬頭
                    ...
                    // ---
                    addColumn(table, 9, 1, subCaseList.get(i + 1).getAccountNo(), fontCh10, 1, nLine2BorderStyle, LEFT); // 帳 號
                    addColumn(table, 11, 1, subCaseList.get(i + 1).getBenIdn(), fontCh10, 1, nLine2BorderStyle, LEFT); // 身分證號
                    addColumn(table, 23, 1, subCaseList.get(i + 1).getCommZip() + " " + subCaseList.get(i + 1).getCommAddr(), fontCh10, 1, nLine2BorderStyle, LEFT); // 受款人地址
                    addColumn(table, 7, 1, "　", fontCh10, 1, nLine2BorderStyle, RIGHT); // 新台幣：元
            ...
            printToDisk((PropertyHelper.getProperty("rpt.path") + Encode.forJava(payCode) + "_MonthlyRpt10Type3_" + Encode.forJava(printDate) + ".pdf"), bao);
        }
        finally {
            document.close();
        }
