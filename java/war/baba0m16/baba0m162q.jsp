...
<html:html lang="true">
<head>
    <acl:setProgId progId="BABA0M162Q" />
                ...
                <tr>
                    <td colspan="8" align="right">
                        <acl:checkButton buttonName="btnBack">
                          <input type="button" name="btnBack" class="button" value="返　回" onclick="$('page').value='1'; $('method').value='doBackFirst';  document.SurvivorMediaBatchForm.submit();" />&nbsp;
                         </acl:checkButton>                                               
                    </td>
                </tr>                
                        ...
                        <display:table name="pageScope.list" id="listItem" pagesize="<%=ConstantKey.LIST_PAGE_SIZE%>">                            
                            ...
                            <display:column title="處理狀態明細" headerClass="issuetitle_L" class="issueinfo_C" style="width:8%; text-align:center;">
                                ...
                                <acl:checkButton buttonName="btnDetail">
                                    <input type="button" name="btnDetail" class="button" value="狀態明細" onclick="$('page').value='1'; $('method').value='doQryProgressDtl'; $('baJobId').value='<c:out value="${listItem.baJobId}" />'; document.SurvivorMediaBatchForm.submit();" />&nbsp;
                                </acl:checkButton>
                                </c:if>                 
                                <c:if test="${listItem.status eq '等待中'}">
                                <acl:checkButton buttonName="btnDetail">
                                    <input type="button" name="btnDetail" class="button" value="狀態明細" disabled="disabled" onclick="$('page').value='1'; $('method').value='doQryProgressDtl'; $('baJobId').value='<c:out value="${listItem.baJobId}" />'; document.SurvivorMediaBatchForm.submit();" />&nbsp;
                                </acl:checkButton>
                                </c:if>               
