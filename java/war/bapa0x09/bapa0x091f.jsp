...
<html:html lang="true">
<head>
    <acl:setProgId progId="BAPA0X091F" />
...
<div id="mainContent">
    ...
    <div id="main" class="mainBody">
        <html:form action="/disabledBasicAmtMaintList" method="post" onsubmit="return validateBasicAmtMaintListForm(this);">
                        ...
                        <display:table name="pageScope.list" id="listItem" pagesize="<%=ConstantKey.LIST_PAGE_SIZE%>" class="px13">                            
                            ...
                            <display:column title="資料明細" style="width:14%; text-align:center;" class="issueinfo_C" headerClass="issuetitle_L">
                                <acl:checkButton buttonName="btnUpdate">
                                    <input type="button" class="button" name="btnUpdate" value="修　改" onclick="location.href='<c:url value="/disabledBasicAmtMaintList.do"/>?method=doUpdate&rowNum=<c:out value="${listItem_rowNum}" />';">
                                </acl:checkButton>
                                <acl:checkButton buttonName="btnDelete">
									...
									<c:otherwise>
										<input type="button" class="button" name="btnDelete" value="刪　除" onclick="if(confirm('是否確定刪除?')){location.href='<c:url value="/disabledBasicAmtMaintList.do"/>?method=doDelete&rowNum=<c:out value="${listItem_rowNum}" />'};">
									</c:otherwise>
									</c:choose>                                                                                                                       
                                </acl:checkButton>                                                                
                            </display:column>
