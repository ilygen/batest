...
<html:html lang="true">
<head>
    <acl:setProgId progId="BAAP0D033C" />
    ...
    function initAll(){
        if('<c:out value="${SurvivorAnnuityReceiptBenForm.benOptionMode}" />'=="updateMode"){
            ...
            $("benBaappbaseId").value='<c:out value="${SurvivorAnnuityReceiptBenForm.baappbaseId}" />';
        ...
        if('<c:out value="${SurvivorAnnuityReceiptBenForm.benNationTyp}" />'!=""){
            if('<c:out value="${SurvivorAnnuityReceiptBenForm.benNationTyp}" />'=="1"){
                ...
                if('<c:out value="${SurvivorAnnuityReceiptBenForm.benNationCodeOption}" />'!=""){
                    $("benNationCodeOption").value = '<c:out value="${SurvivorAnnuityReceiptForm.benNationCodeOption}" />';
                }else{
                    $("benNationCodeOption").value = '<c:out value="${SurvivorAnnuityReceiptForm.benNationCode}" />';
                }
            }            
        }else{
        ...
        if('<c:out value="${SurvivorAnnuityReceiptBenForm.payCategory}" />' == "1"){
            ...
            $("accData").value='<c:out value="${SurvivorAnnuityReceiptBenForm.accData}" />'; 
        }else{
            $("payCategory1").checked=true;
            $("accData").value="";
            $("accData").disabled = true;
    ...
    //取得欲修改之眷屬資料
    function getBenDetailData(baappbaseId){
        receiptCommonAjaxDo.getSurvivorBenDetailData(baappbaseId, setBenData);  
    }
    
    function setBenData(benCase){
        $("benBaappbaseId").value = benCase.baappbaseId;
        $("baappexpandId").value = benCase.baappexpandId;  
        $("benSeqNo").value = benCase.seqNo;
        $("benNationTyp").value = benCase.benNationTyp;     
        if(benCase.benNationTyp == '2'){                                  
            $("benSex").value = benCase.benSex;                                       
            $("benNationCode").value = benCase.benNationCode;
            $("benNationCodeOption").value = benCase.benNationCode;
        }                                       
        $("benName").value = benCase.benName;                                       
        $("benAppDate").value = benCase.benAppDate;                                       
        $("benIdnNo").value = benCase.benIdnNo;                                       
        $("benBrDate").value = benCase.benBrDate;
        $("benEvtRel").value = benCase.benEvtRel;
        ...
        if(benCase.accSeqNoAmt == "1"){
            ...
            getAccDataOptionList(benCase.baappbaseId, benCase.apNo);
        }      
        
        if(benCase.benEvtRel == '2'){
    ...
    function getAccDataOptionList(baappbaseId, apNo) {
        receiptCommonAjaxDo.getBenOptionListForSurvivor(baappbaseId, apNo, setAccDataOptionList);       
    }

    function setAccDataOptionList(accDataOption) {
        DWRUtil.removeAllOptions("accData");
        DWRUtil.addOptions("accData", {'':'請選擇'});
        DWRUtil.addOptions("accData", accDataOption ,'seqNo','benName');
        ...
        for(i=0; i<accDataOption.length; i++){
            strs = (accDataOption[i].seqNo).split(";");
            if($("accSeqNo").value == strs[1]){
                $("accData").value = accDataOption[i].seqNo;
                break;
            }
        }
    }  
    ...
    function autoForeignEvtSex(){
    	var secondText = $("evtIdnNo").value.substring(1,2);
		if($("evtIdnNo").value.length==10){
    	if($("evtNationTpe").value=="2" && $("evtIdnNo").value.length==10 && Trim($("evtSex").value)==""){
    		if(secondText=="A" || secondText=="a" || secondText=="C" || secondText=="c" || secondText=="8"){
    			$("evtSex").value = "1";	
    ...
    <div id="main" class="mainBody">
        ...
        <fieldset>
            ...
            <table width="98%" border="0" align="center" cellpadding="2" cellspacing="2" class="px13">
                <html:form action="/survivorAnnuityReceiptModify" method="post" onsubmit="return validateSurvivorAnnuityReceiptForm(this);">
                    <html:hidden styleId="page" property="page" value="1" />
                    <html:hidden styleId="method" property="method" value="" />
                    <html:hidden styleId="apNo" property="apNo"/>
                    <html:hidden styleId="evtBaappbaseId" property="baappbaseId"/>
                    <html:hidden styleId="accSeqNo" property="accSeqNo"  value=""/>
                    <html:hidden styleId="actionTyp" property="actionTyp" value="updateMode"/>
                
                    <tr>
                        <td align="right" colspan="8">
                            <acl:checkButton buttonName="btnConfirm">
                                <input tabindex="450" type="button" name="btnConfirm" class="button" value="確　認" onclick="$('page').value='1'; $('method').value='doUpdate'; if(document.SurvivorAnnuityReceiptForm.onsubmit() && checkFields()){document.SurvivorAnnuityReceiptForm.submit();}else{return false;}" />&nbsp;
                            </acl:checkButton>
                            <acl:checkButton buttonName="btnDelete">
                                <input tabindex="460" type="button" name="btnDelete" class="button" value="刪　除" onclick="$('page').value='1'; $('method').value='doDelete'; if(confirm('<bean:message bundle="<%=Global.BA_MSG%>" key="msg.delete.confirm"/>')){document.SurvivorAnnuityReceiptForm.submit();}else{return false;}" />&nbsp;
                            </acl:checkButton>
                            <acl:checkButton buttonName="btnBack">
                                <input tabindex="470" type="button" name="btnBack" class="button" value="返　回" onclick="$('page').value='1'; $('method').value='doBackModifyList'; document.SurvivorAnnuityReceiptForm.submit();"/>
                            </acl:checkButton>                        
                        </td>
                    </tr>
                    <tr>
                            ...
                            <table width="100%" border="0" cellpadding="2" cellspacing="2" class="px13">
                                <tr>
                                    <td>
                                        ...
                                        <c:out value="${SurvivorAnnuityReceiptForm.apNoStrDisplay}"/>
                                    </td>
                                    <td>
                    ...
                    <tr>
                        <td align="center" class="table1" colspan="8">
                            <table width="98%" cellpadding="3" cellspacing="4" class="px13">
                                ...
                                <tr>
                                    <td id="iss"><span class="needtxt">＊</span><span class="issuetitle_L_down">申請傷病分類：</span>
                                        <html:text tabindex="110" property="evAppTyp" styleId="evAppTyp" styleClass="textinput"  size="1" maxlength="1" onblur="this.value=asc(this.value);"/>
                ...
                <html:form action="/survivorAnnuityReceiptModifyBen" method="post" onsubmit="return validateSurvivorAnnuityReceiptBenForm(this);">
                    <html:hidden styleId="benPage" property="page" value="1" />
                    <html:hidden styleId="benMethod" property="method" value="" />
                    <html:hidden styleId="benBaappbaseId" property="baappbaseId" />
                    <html:hidden styleId="benBaappexpandId" property="baappexpandId" />
                    <html:hidden styleId="benApNo" property="apNo" value="${SurvivorAnnuityReceiptForm.apNo}"/>
                    <html:hidden styleId="benSeqNo" property="seqNo"/>
                    <html:hidden styleId="accSeqNo" property="accSeqNo"/>
                    <html:hidden styleId="benActionTyp" property="actionTyp" value="updateMode"/>
                            ...
                            <display:table name="pageScope.list" id="listItem" pagesize="<%=ConstantKey.LIST_PAGE_SIZE%>">
                                ...
                                <display:column title="操作區" style="width:16%; text-align:center;">
                                    <acl:checkButton buttonName="btnUpdate">                                                                                                                                                                                                                                                                                       
                                        <input type="button" name="btnUpdate" class="button" value="修　改" onclick="getBenDetailData('<c:out value="${listItem.baappbaseId}" />');$('dataIdx').value=<c:out value="${listItem_rowNum}" />;$('dataIndex').innerHTML=<c:out value="${listItem_rowNum}" />;$('benOptionMode').value='updateMode';" onblur="tabChange();"/>&nbsp;
                                    </acl:checkButton>
                                    <acl:checkButton buttonName="btnDelete">                                                                                                                                                                                                                                                                                       
                                        <input type="button" name="btnDelete" class="button" value="刪　除" onclick="$('benPage').value='1'; $('benMethod').value='doDeleteBenData'; $('benBaappbaseId').value='<c:out value="${listItem.baappbaseId}" />'; $('benSeqNo').value='<c:out value="${listItem.seqNo}" />'; if (confirm('<bean:message bundle="<%=Global.BA_MSG%>" key="msg.delete.confirm"/>')){document.SurvivorAnnuityReceiptBenForm.submit();}else{return false;}" />
                                    </acl:checkButton>
                                </display:column>
                            </display:table>
                        </td>                                                       
