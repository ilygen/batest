 ...
 * @author Goston
 */
public class MonthlyRpt10Type2Report extends ReportBase {
    private String printDate = ""; // 印表日期

    private HashMap<String, MonthlyRpt10Type2PayAmtCase> payAmtMap; // 代扣補償金

    public MonthlyRpt10Type2Report() throws Exception {
        super();
    ...
    public ByteArrayOutputStream doReport(UserBean userData, List<MonthlyRpt10Type2RptCase> caseList, HashMap<String, MonthlyRpt10Type2PayAmtCase> payAmtMap, List<MonthlyRpt10Type2RptCase> subCaseList, HashMap<String, Object> map) throws Exception {
        try {
            String payCode = (String) map.get("payCode");
            String chkDate = (String) map.get("chkDate");
            // 2009.2.20 EvelynHsu add 輸出報表檔至主機上
            String printDate = DateUtility.getNowChineseDate();
            // FileOutputStream bao = new FileOutputStream(PropertyHelper.getProperty("rpt.path") + Encode.forJava(payCode) + "_" + Encode.forJava(chkDate) + "_MonthlyRpt10Type2_" + Encode.forJava(printDate) + ".pdf");
            ...
            for (int i = 0; i < subCaseList.size(); i++) { // ... [
                MonthlyRpt10Type2RptCase caseData = subCaseList.get(i);
                ...
                if (i == subCaseList.size() - 1 || !StringUtils.equals(StringUtils.trimToEmpty(caseData.getBenEvtRel()) + StringUtils.trimToEmpty(caseData.getCompName1()),
                                StringUtils.trimToEmpty(subCaseList.get(i + 1).getBenEvtRel() + StringUtils.trimToEmpty(subCaseList.get(i + 1).getCompName1())))) {
                    // 塞滿空白行, 以讓表格美觀
                    // [
                    while (writer.fitsPage(table)) {
                        addEmptyRow(table, 1);
                    ...
                    addColumn(table, 30, 1, "本頁小計金額：" + formatBigDecimalToFloat(pageAmountAmt), fontCh12, 0, LEFT);
                    // ---
                    if (!StringUtils.equalsIgnoreCase(caseData.getBenEvtRel(), "Z") || i == subCaseList.size() - 1) {
                        addColumn(table, 15, 1, "　", fontCh12, 0, LEFT);
                        addColumn(table, 15, 1, "　　　總筆數：" + formatBigDecimalToInteger(allRec), fontCh12, 0, LEFT);
                        addColumn(table, 30, 1, "　　　總金額：" + formatBigDecimalToFloat(allAmountAmt), fontCh12, 0, LEFT);
                    }
                    ...
                    // 測試塞下一筆 + 表尾時是否需要換頁
                    addColumn(table, 5, 1, String.valueOf(recNo + 2), fontCh12, 0, CENTER); // 序號
                    addColumn(table, 6, 1, subCaseList.get(i + 1).getBankString(), fontCh12, 0, LEFT); // 入帳銀行
                    addColumn(table, 6, 1, subCaseList.get(i + 1).getPayEeacc(), fontCh12, 0, RIGHT); // 入帳帳號
                    addColumn(table, 6, 1, subCaseList.get(i + 1).getBenName(), fontCh12, 0, LEFT); // 姓名
                    addColumn(table, 5, 1, formatBigDecimalToFloat(((subCaseList.get(i + 1).getPayAmt() == null) ? new BigDecimal(0) : subCaseList.get(i + 1).getPayAmt())), fontCh12, 0, RIGHT); // 交易金額
                    addColumn(table, 6, 1, subCaseList.get(i + 1).getApNoString(), fontCh12, 0, LEFT); // 受理編號
                    addColumn(table, 5, 1, subCaseList.get(i + 1).getIssuYmString(), fontCh12, 0, CENTER); // 核定年月
                    addColumn(table, 5, 1, subCaseList.get(i + 1).getPayYmString(), fontCh12, 0, CENTER); // 給付年月
                    addColumn(table, 6, 1, subCaseList.get(i + 1).getBenIdn(), fontCh12, 0, LEFT); // 身分證號碼
                    if (StringUtils.equalsIgnoreCase(caseData.getBenEvtRel(), "Z")) {
                        addColumn(table, 10, 1, subCaseList.get(i + 1).getCompName2(), fontCh12, 0, LEFT); // 申請代算單位
                    }
                    else {
                        addColumn(table, 10, 1, " ", fontCh12, 0, LEFT);
                    }
            ...
            } // ] ... end for (int i = 0; i < subCaseList.size(); i++)

            document.close();
            // 20180103 輸出報表檔至主機上 因為FORTIY 修改寫法
            printToDisk((PropertyHelper.getProperty("rpt.path") + Encode.forJava(payCode) + "_MonthlyRpt10Type2_" + Encode.forJava(printDate) + ".pdf"), bao);
        }
        finally {
            document.close();
        }
