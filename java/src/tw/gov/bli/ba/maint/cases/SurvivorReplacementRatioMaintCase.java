 ...
 * @author Noctis
 */
public class SurvivorReplacementRatioMaintCase implements Serializable {
    ...
    public String getDeleteUpdateStr() {
    	if(StringUtils.isNotBlank(effYm)){
        if (Integer.parseInt(effYm) <= Integer.parseInt(DateUtility.getNowWestDate().substring(0, 6)))
            return "Y";
        else
            return "";
    	}else{
	...
	public String getEffYmStr() {
        if (StringUtils.isNotEmpty(effYm))
            return DateUtility.changeWestYearMonthType(effYm);
        else
            return "";
    }
