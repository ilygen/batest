...
<html:html lang="true">
<head>
    <acl:setProgId progId="BAAP0D061A" />
    ...
    <html:javascript formName="SurvivorAnnuityWalkInReceiptForm" page="2" />
    <html:javascript formName="SurvivorAnnuityWalkInReceiptBenForm" page="1" />
    <script language="javascript" type="text/javascript">
    
    var procType = '<c:out value="${SurvivorAnnuityWalkInReceiptForm.procType}" />';
    
    function initAll(){
        if('<c:out value="${SurvivorAnnuityWalkInReceiptBenForm.benOptionMode}" />'=="updateMode"){
        ...
        //事故者國籍別
        if('<c:out value="${SurvivorAnnuityWalkInReceiptForm.evtNationTpe}" />'!=""){
            if('<c:out value="${SurvivorAnnuityWalkInReceiptForm.evtNationTpe}" />'=="1"){
                ...
                if('<c:out value="${SurvivorAnnuityWalkInReceiptForm.evtNationCodeOption}" />'!=""){
                    $("evtNationCodeOption").value = '<c:out value="${SurvivorAnnuityWalkInReceiptForm.evtNationCodeOption}" />';
                }else{
                    $("evtNationCodeOption").value = '<c:out value="${SurvivorAnnuityWalkInReceiptForm.evtNationCode}" />';
                }
        ...
        if('<c:out value="${SurvivorAnnuityWalkInReceiptBenForm.benNationTyp}" />'!=""){
            if('<c:out value="${SurvivorAnnuityWalkInReceiptBenForm.benNationTyp}" />'=="1"){
                ...
                if('<c:out value="${SurvivorAnnuityWalkInReceiptBenForm.commTyp}" />'=="1"){
                ...
                $("commZip").disabled = false;
                $("commAddr").disabled = false;
                if('<c:out value="${SurvivorAnnuityWalkInReceiptBenForm.benNationCodeOption}" />'!=""){
                    $("benNationCodeOption").value = '<c:out value="${SurvivorAnnuityWalkInReceiptBenForm.benNationCodeOption}" />';
                }else{
                    $("benNationCodeOption").value = '<c:out value="${SurvivorAnnuityWalkInReceiptBenForm.benNationCode}" />';
                }
            }            
        }else{
        ...
        //給付方式
        if('<c:out value="${SurvivorAnnuityWalkInReceiptBenForm.payCategory}" />' == "1"){
            $("payCategory1").checked=true;
            $("accData").value="";
            $("accData").disabled = true;
            $("payTyp").disabled = false;
            if('<c:out value="${SurvivorAnnuityWalkInReceiptBenForm.payTyp}" />' == ""){ 
                $("payTyp").value="";
            }
        }else if('<c:out value="${SurvivorAnnuityWalkInReceiptBenForm.payCategory}" />' == "2"){
        ...
        if($("mobilePhone").disabled == false){
            $("mobilePhone").value='<c:out value="${SurvivorAnnuityWalkInReceiptBenForm.mobilePhone}" />';
        }else{
            $("mobilePhone").value="";
        }
        
        if('<c:out value="${SurvivorAnnuityWalkInReceiptBenForm.focusLocation}" />'=="benNationTyp"){
            $("benNationTyp").focus();
        }else{
            $("apNo1").focus();
        }
    ...
    function setBenData(benCase){
        ...
        if (procType != '1') {
	     	// 計息存儲
	     	$('savingMk').value = benCase.savingMk;
	        if (benCase.savingMk == "Y") {
	        	$('savingMkCheck').checked = true;
	        } else {
	        	$('savingMkCheck').checked = false;
	        }
	        // 專戶
	        $('specialAcc').value = benCase.specialAcc;
	        if (benCase.specialAcc == "Y") {
	        	$('specialAccCheck').checked = true;
	        } else {
	        	$('specialAccCheck').checked = false;
	        }
	        $('accName').value = benCase.accName;
        }
        // bb 受理轉入欄位
        if (procType == '4') {
        	$('schoolCode').value = benCase.schoolCode;// 學校代碼
        	$('benMarrMk').value = benCase.benMarrMk;// 婚姻狀況
        	$('adoPtDate').value = benCase.adoPtDate;// 收養日期
        	$('assignName').value = benCase.assignName;// 代辦人姓名
        	$('assignIdnNo').value = benCase.assignIdnNo;// 代辦人身分證號
        	$('assignBrDate').value = benCase.assignBrDate;// 代辦人出生日期
        	
        	// 被保險人扶養
        	$('raiseEvtMk').value = benCase.raiseEvtMk;
        	if (benCase.raiseEvtMk == "Y") {
	        	$('raiseEvtMkCheck').checked = true;
	        } else {
	        	$('raiseEvtMkCheck').checked = false;
	        }
        }
    ...
    function cleanBenData(){
        ...
        // 他系統轉入(BB、BC、BE)
        if ("2,3,4".indexOf(procType) > -1) {
	        $('savingMkCheck').checked = false;// 計息存儲
	        $('savingMk').value = '';// 計息存儲
	        $('specialAccCheck').checked = false;// 專戶
	        $('specialAcc').value = '';// 專戶
	        $('accName').value = '';// 戶名
        }
        // 他系統轉入(BE)
        if (procType == '4') {
        	$('schoolCode').value = '';// 學校代碼
        	$('benMarrMk').value = '';// 婚姻狀況
        	$('adoPtDate').value = '';// 收養日期
	        $('raiseEvtMkCheck').checked = false;// 被保險人扶養
	        $('raiseEvtMk').value = '';// 被保險人扶養
	        $('assignName').value = '';// 代辦人姓名
	        $('assignIdnNo').value = '';// 代辦人身分證號
    ...
    function getBankName() {
    	...
    	if ('1,2'.indexOf(payTyp) > -1 && payBankIdBranchId.length == 7 && !isNaN(payBankIdBranchId)) {
    		receiptCommonAjaxDo.getBankName($('payTyp').value, payBankIdBranchId, setBankName);
    	} else {
    		$('bankNameDiv').style.display = 'none';
    	}
    }
    
    // ajax 取得金融機構名稱 callback
    ...
    function doSubmit() {
		...
		if (procType != '1') {
			...
			if (uncheckeds.length > 0) {
				alert('未檢核之遺屬：【' + uncheckeds + '】\r\n請先將轉入之遺屬資料透過修改遺屬功能進行資料檢核。');
				return false;
			}
		}
    ...
    // 載入遺屬資料
    function loadBenData(bafamilytempId, seqNo, rowNum) {
    	getBenDetailData(bafamilytempId, seqNo);
    	$('dataIdx').value = rowNum;
    	$('dataIndex').innerHTML = rowNum;
    	$('benOptionMode').value='updateMode';
    }
...
<div id="mainContent">
    ...
    <div id="main" class="mainBody">
        <fieldset>
            <c:if test="${SurvivorAnnuityWalkInReceiptForm.procType == '1' }">
	            <legend>&nbsp;普通遺屬年金臨櫃受理作業&nbsp;</legend>                                                                                                                                                                                                                                                                                                 
            </c:if>
            <c:if test="${SurvivorAnnuityWalkInReceiptForm.procType == '2' }">
	            <legend>&nbsp;職災死亡一次金(轉入)臨櫃受理作業&nbsp;</legend>                                                                                                                                                                                                                                                                                                 
            </c:if>
            <c:if test="${SurvivorAnnuityWalkInReceiptForm.procType == '3' }">
	            <legend>&nbsp;普通死亡一次金(轉入)臨櫃受理作業&nbsp;</legend>                                                                                                                                                                                                                                                                                                 
            </c:if>
            <c:if test="${SurvivorAnnuityWalkInReceiptForm.procType == '4' }">
	            <legend>&nbsp;職災遺屬年金(轉入)臨櫃受理作業&nbsp;</legend>                                                                                                                                                                                                                                                                                                 
            </c:if>
            ...
            <table width="98%" border="0" align="center" cellpadding="2" cellspacing="2" class="px13">
                <html:form action="/survivorAnnuityWalkInReceiptInsert" method="post" onsubmit="return validateSurvivorAnnuityWalkInReceiptForm(this);">
                    ...
                    <tr>
                        <td align="right" colspan="8">
                            <acl:checkButton buttonName="btnConfirm">
                                <input tabindex="450" type="button" name="btnConfirm" class="button" value="確　認" onclick="doSubmit();" />&nbsp;
                            </acl:checkButton>
                            ...
                            <acl:checkButton buttonName="btnBack">
                                <input tabindex="470" type="button" name="btnBack" class="button" value="返　回" onclick="doBack();"/>
                            </acl:checkButton>                        
                        </td>
                    </tr>
                    <tr>
                            ...
                            <table width="98%" cellpadding="3" cellspacing="4" class="px13">
                                ...
                                <tr>
                                    <td width="50%" id="iss">
                                        ...
                                        <html:text tabindex="30" property="evtNationTpe" styleId="evtNationTpe" styleClass="textinput" size="1" maxlength="1"
                                        	onkeyup="changeEvtNationTpe();" onkeypress="" onblur="this.value=asc(this.value);"/>
                                        <span class="formtxt">(1-本國，2-外籍)</span>
                                    </td>
                                    <td id="iss" width="50%">
                                        <span class="needtxt">＊</span>
                                        <span class="issuetitle_L_down">死亡日期：</span>
                                        <html:text tabindex="40" property="evtDieDate" styleId="evtDieDate" styleClass="textinput" size="7" maxlength="7"
                                        	onblur="this.value=asc(this.value);"/>
                                    </td>
                                </tr>
                                <tr>
                                    <td id="iss">
                                        ...
                                        <html:text tabindex="50" property="evtSex" styleId="evtSex" styleClass="textinput" size="1" maxlength="1"
                                        	onblur="this.value=asc(this.value);"/>
                                        <span class="formtxt">(外籍者請填寫，1-男，2-女)</span>
                                    </td>
                                    <td id="iss">
                                        <span class="issuetitle_L_down">　國　籍：</span>
                                        <html:text tabindex="60" property="evtNationCode" styleId="evtNationCode" styleClass="textinput" size="3" maxlength="3"
                                        	onblur="this.value=asc(this.value);"/>
                                        <label>
                                ...
                                <tr>
                                    <td colspan="2" id="iss">
                                        ...
                                        <html:text tabindex="80" property="evtName" styleId="evtName" styleClass="textinput" size="50" maxlength="50"
                                        	onblur="this.value=asc(this.value);"/>
                                        <span class="formtxt">
                                ...
                                <tr>
                                    <td id="iss"<c:if test="${SurvivorAnnuityWalkInReceiptForm.procType eq '1' }"> colspan="2"</c:if>>
                                        ...
                                        <html:text tabindex="110" property="evAppTyp" styleId="evAppTyp" styleClass="textinput"  size="1" maxlength="1"
                                        	onblur="this.value=asc(this.value);"/>
                                        <c:if test="${SurvivorAnnuityWalkInReceiptForm.procType ne '1' }">
	                                    	<span class="formtxt">(1-職業傷害，2-職業病，3-普通傷害，4-普通疾病)</span>
	                                    </c:if>
	                                    <c:if test="${SurvivorAnnuityWalkInReceiptForm.procType eq '1' }">
                                ...
                                <tr>
                                    <td id="iss" colspan="2">
                                    	<span class="needtxt">＊</span><span class="issuetitle_L_down">申請項目：</span>
                                        <html:text tabindex="120" property="apItem" styleId="apItem" styleClass="textinput"  size="1" maxlength="1"
                                        	onblur="this.value=asc(this.value);"/>
                                        <span class="formtxt">(4-遺屬年金加喪葬津貼，5-遺屬年金，7-領取失能年金期間死亡，8-領取老年年金期間死亡)</span>
                                        <%-- for pass validation --%>
                                        <html:hidden styleId="judgeDate" property="judgeDate"/>
                                    </td>
                                </tr>
                                </c:if>
                                <c:if test="${SurvivorAnnuityWalkInReceiptForm.procType ne '1' }">
                                <tr>
                                    ...
                                    <td id="iss">
                                    	...
                                    	<html:text tabindex="121" property="apnoFm" styleId="apnoFm" styleClass="textinput" size="12" maxlength="12" readonly="true"/>
                                    </td>
                                </tr>
                                <tr>
                                	<td id="iss">
                                		<span class="issuetitle_L_down">　寄送月通知表：</span>
                                		<html:checkbox tabindex="122" property="monNotifyingMkCheck" styleId="monNotifyingMkCheck" onclick="updateCheckBoxValue('monNotifyingMk');"/>
                                		<html:hidden styleId="monNotifyingMk" property="monNotifyingMk"/>
                                	</td>
                                	<td id="iss">
                                		<span class="issuetitle_L_down">　判決日期：</span>
                                		<html:text tabindex="123" property="judgeDate" styleId="judgeDate" styleClass="textinput"  size="7" maxlength="7"
                                        	onblur="this.value=asc(this.value);"/>
                                	</td>
                                </tr>
                                </c:if>
                ...
                <html:form action="/survivorAnnuityWalkInReceiptInsertBen" method="post" onsubmit="return validateSurvivorAnnuityWalkInReceiptBenForm(this);">
                            ...
                            <table width="98%" cellpadding="3" cellspacing="4" class="px13">
                                            ...
                                            <tr>
                                                ...
                                                <td align="right">
                                                    ...
                                                    <acl:checkButton buttonName="btnClearBen">
                                                        <input tabindex="440" type="button" name="btnClearBen" class="button" value="清除遺屬"
                                                        	onclick="cleanBenData();$('benOptionMode').value='insertMode';" />&nbsp;&nbsp;
                                                    </acl:checkButton>
                                                </td>
                                            </tr>
                                        </table>
                                ...
                                <tr>
                                    <td colspan="2" id="iss">
                                        <span class="needtxt">＊</span>
                                        <span class="issuetitle_L_down">國籍別：</span>
                                        <html:text tabindex="130" property="benNationTyp" styleId="benNationTyp" styleClass="textinput"  size="1" maxlength="1"
                                        	onkeyup="changeBenNationTyp();" onkeypress="chgBenNationTypFocus();" onblur="this.value=asc(this.value);"/>
                                        <span class="formtxt">(1-本國，2-外籍)</span>
                                    </td>                
                                </tr>
                                <tr>
                                    <td id="iss" width="50%">
                                        <span class="issuetitle_L_down">　性　別：</span>
                                        <html:text tabindex="140" property="benSex" styleId="benSex" styleClass="textinput"  size="1" maxlength="1"
                                        	onblur="this.value=asc(this.value);"/>
                                        <span class="formtxt">(外籍者請填寫，1-男，2-女)</span>
                                    </td>
                                    <td id="iss" width="50%">
                                        <span class="issuetitle_L_down">　國　籍：</span>
                                        <html:text tabindex="150" property="benNationCode" styleId="benNationCode" styleClass="textinput"  size="3" maxlength="3"
                                        	onblur="this.value=asc(this.value);" />
                                ...
                                <tr>
                                    <td id="iss">
                                        ...
                                        <html:text tabindex="170" property="benName" styleId="benName" styleClass="textinput"  size="20" maxlength="50"
                                        	onblur="this.value=asc(this.value);"/>
                                    </td>
                                    <td id="iss">
                                        <span class="needtxt">＊</span>
                                        <span class="issuetitle_L_down">遺屬申請日期：</span>
                                        <html:text tabindex="180" property="benAppDate" styleId="benAppDate" styleClass="textinput"  size="7" maxlength="7"
                                        	onblur="this.value=asc(this.value);" onfocus="if($('benOptionMode').value=='insertMode'&& Trim(this.value)==''){this.value=$('appDate').value}"/>
                                    </td>
                                </tr>
                                <tr>
                                    <td id="iss">
                                    ...
                                    <td id="iss">
                                        ...
                                        <html:text tabindex="200" property="benBrDate" styleId="benBrDate" styleClass="textinput"  size="8" maxlength="8"
                                        	onblur="this.value=asc(this.value);"/>
                                ...
                                <tr>
                                    <td colspan="2" id="iss">
                                        ...
                                        <html:text tabindex="210" property="benEvtRel" styleId="benEvtRel" styleClass="textinput"  size="1" maxlength="1"
                                        	onkeyup="chgBenEvtRel();" onblur="this.value=asc(this.value);chgBenEvtRel();"/>
                                ...
                                <tr>
                                    <td id="iss">
                                        ...
                                        <html:text tabindex="220" property="tel1" styleId="tel1" styleClass="textinput"  size="20" maxlength="20"
                                        	onblur="this.value=asc(this.value); enableMobilePhone();"/>
                                    </td>
                                    <td id="iss">
                                        <span class="issuetitle_L_down">　電　話2：</span>
                                        <html:text tabindex="230" property="tel2" styleId="tel2" styleClass="textinput"  size="20" maxlength="20"
                                        	onblur="this.value=asc(this.value); enableMobilePhone();"/>
                                    </td>
                                </tr>
                                <tr>
                                    <td colspan="2" id="iss">
                                        ...
                                        <html:text tabindex="240" property="commTyp" styleId="commTyp" styleClass="textinput"  size="1" maxlength="1"
                                        	onkeyup="changeCommTyp();" onkeypress="chgCommTypFocus();" onblur="this.value=asc(this.value);"/>
                                        <span class="formtxt">(1-同戶籍地，2-現住址)　 現住址：
                                        	<html:text tabindex="250" property="commZip" styleId="commZip" styleClass="textinput"  size="6" maxlength="6"
                                        		onblur="this.value=asc(this.value);"/>(郵遞區號)&nbsp;-
                                            <html:text tabindex="260" property="commAddr" styleId="commAddr" styleClass="textinput"  size="72" maxlength="240"
                                            	onblur="this.value=asc(this.value);"/>
                                        </span>
                                    </td>
                                </tr>
                                ...
                                <tr>
                                    <td colspan="2" id="iss">
                                        ...
                                        <html:text tabindex="262" property="mobilePhone" styleId="mobilePhone" styleClass="textinput"  size="10" maxlength="10"
                                        	onblur="this.value=asc(this.value);"/>
                                    </td>
                                </tr>
                                </c:if>
                                <c:if test="${SurvivorAnnuityWalkInReceiptForm.procType ne '1' }">
                                    ...
                                    <td id="iss">
                                        ...
                                        <html:checkbox tabindex="263" property="savingMkCheck" styleId="savingMkCheck" onclick="updateCheckBoxValue('savingMk');"/>
                                        <html:hidden styleId="savingMk" property="savingMk"/>
                                    </td>
                                </c:if>
                                <tr>
                                    <td colspan="2" id="iss">
                                        ...
                                        <logic:notEmpty name="<%=ConstantKey.SURVIVOR_ANNUITY_RECEIPT_BEN_DATA_LIST%>">
                                            <html:text tabindex="265" property="defaultGrdData" styleId="defaultGrdData" styleClass="textinput"  size="1" maxlength="1"
                                ...
                                <tr>
                                    <td colspan="2" id="iss">
                                        ...
                                        <html:text tabindex="270" property="grdName" styleId="grdName" styleClass="textinput"  size="50" maxlength="50"
                                        	onblur="this.value=asc(this.value);"/>
                                    </td>
                                </tr>
                                <tr>
                                    <td id="iss">
                                        ...
                                        <html:text tabindex="280" property="grdIdnNo" styleId="grdIdnNo" styleClass="textinput"  size="25" maxlength="20"
                                        	onblur="this.value=asc(this.value).toUpperCase();"/>
                                    </td>
                                    <td id="iss">
                                        <span class="issuetitle_L_down">　法定代理人出生日期：</span>
                                        <html:text tabindex="290" property="grdBrDate" styleId="grdBrDate" styleClass="textinput"  size="8" maxlength="8"
                                        	onblur="this.value=asc(this.value);"/>
                                    </td>
                                </tr>
                                <tr>
                                    <td id="iss">
                                        ...
                                        <html:text tabindex="300" property="marryDate" styleId="marryDate" styleClass="textinput"  size="7" maxlength="7"
                                        	onblur="this.value=asc(this.value);"/>
                                ...
                                <tr>
                                    <td id="iss" colspan="2">
                                        ...
                                        <html:text tabindex="320" property="monIncomeMk" styleId="monIncomeMk" styleClass="textinput"  size="1" maxlength="1"
                                        ...
                                        <html:text tabindex="330" property="monIncome" styleId="monIncome" styleClass="textinput"  size="7" maxlength="7"
                                        	onblur="this.value=asc(this.value);"/>元
                                        <%-- for pass validation --%>
										<html:hidden styleId="adoPtDate" property="adoPtDate"/>
                                    </td>
                                </tr>
                                </c:if>
                                <c:if test="${SurvivorAnnuityWalkInReceiptForm.procType eq '4' }">
                                <tr>
                                    <td id="iss">
                                        <span class="issuetitle_L_down">　每月工作收入：</span>
                                        <html:text tabindex="320" property="monIncomeMk" styleId="monIncomeMk" styleClass="textinput"  size="1" maxlength="1"
                                    ...
                                    <td id="iss">
                                        <span class="issuetitle_L_down">　學校代碼：</span>
                                        <html:text tabindex="331" property="schoolCode" styleId="schoolCode" styleClass="textinput"  size="10" maxlength="10"
                                        	onblur="this.value=asc(this.value).toUpperCase();"/>
                                    </td>
                                </tr>
                                <tr>
                                    <td id="iss">
                                        ...
                                        <html:text tabindex="332" property="benMarrMk" styleId="benMarrMk" styleClass="textinput"  size="1" maxlength="1"
                                        	onblur="this.value=asc(this.value).toUpperCase();"/>
                                       	<span class="formtxt">(已婚，請輸入Y)</span>
                                    </td>
                                    <td id="iss">
                                        <span class="issuetitle_L_down">　收養日期：</span>
                                        <html:text tabindex="333" property="adoPtDate" styleId="adoPtDate" styleClass="textinput"  size="7" maxlength="7"
                                        	onblur="this.value=asc(this.value).toUpperCase();"/>
                                    </td>
                                </tr>
                                </c:if>
                                ...
                                <tr>
                                    <td id="iss" colspan="2">
                                        ...
                                        <html:text tabindex="340" property="handIcapMk" styleId="handIcapMk" styleClass="textinput"  size="1" maxlength="1"
                                        	onblur="this.value=asc(this.value).toUpperCase();"/>         
                                ...
                                <tr>
                                    ...
                                    <td id="iss">
                                        ...
                                        <html:checkbox tabindex="341" property="raiseEvtMkCheck" styleId="raiseEvtMkCheck" onclick="updateCheckBoxValue('raiseEvtMk');"/>
                                        <html:hidden styleId="raiseEvtMk" property="raiseEvtMk"/>
                                    </td>
                                </tr>
                                </c:if>
                                <tr>
                                    <td colspan="2" id="iss">
                                        ...
                                        <html:text tabindex="350" property="interDictMk" styleId="interDictMk" styleClass="textinput"  size="1" maxlength="1"
                                        	onblur="this.value=asc(this.value).toUpperCase();"/>
                                        <span class="formtxt">(受禁治產(監護)宣告者，請輸入Y)</span>
                                        <%-- for pass validation --%>
                                        <html:hidden styleId="assignBrDate" property="assignBrDate"/>
                                    </td>
                                </tr>
                                <c:if test="${SurvivorAnnuityWalkInReceiptForm.procType eq '4' }">
                                <tr>
                                	<td id="iss" colspan="2">
                                        ...
                                        <html:text tabindex="351" property="assignName" styleId="assignName" styleClass="textinput"  size="50" maxlength="50"
                                        	onblur="this.value=asc(this.value).toUpperCase();"/>         
                                    </td>
                                </tr>
                                <tr>
                                	<td id="iss">
                                        ...
                                        <html:text tabindex="352" property="assignIdnNo" styleId="assignIdnNo" styleClass="textinput"  size="25" maxlength="20"
                                        	onblur="this.value=asc(this.value).toUpperCase();"/>         
                                    </td>
                                	<td id="iss">
                                        <span class="issuetitle_L_down">　代辦人出生日期：</span>
                                        <html:text tabindex="353" property="assignBrDate" styleId="assignBrDate" styleClass="textinput"  size="7" maxlength="7"
                                        	onblur="this.value=asc(this.value).toUpperCase();"/>         
                                    </td>
                                </tr>
                                </c:if>
                                ...
                                <tr>
                                    <td id="iss" colspan="2">
                                        ...
                                        <span class="formtxt">
                                            <html:radio tabindex="360" styleId="payCategory1" property="payCategory" value="1" onclick="chgPayCategory();"
                                            	onkeypress="chgPayCategoryFocus();" />本人領取
                                            <html:text tabindex="370" property="payTyp" styleId="payTyp" styleClass="textinput"  size="1" maxlength="1"
                                            	onkeyup="changePayTyp();" onkeypress="chgPayTypFocus();" onblur="this.value=asc(this.value).toUpperCase();tabChange();"/>
                                        ...
                                        <c:if test="${SurvivorAnnuityWalkInReceiptForm.procType ne '1' }">
                                        <span class="formtxt" style="padding-left: 20px;">
                                        	<html:checkbox tabindex="371" property="specialAccCheck" styleId="specialAccCheck" onclick="updateCheckBoxValue('specialAcc');"/>
	                                        <label class="issuetitle_L_down" for="specialAccCheck">專戶</label>
                                ...
                                <tr>
                                    <td id="iss">                                    
                                        ...
                                        <html:text tabindex="380" property="payBankId" styleId="payBankId" styleClass="textinput" size="1" maxlength="3"
                                        	onblur="this.value=asc(this.value); getBankName();"/>&nbsp;-
                                        <html:text tabindex="385" property="branchId" styleId="branchId" styleClass="textinput" size="1" maxlength="4"
                                        	onblur="this.value=asc(this.value); getBankName();"/>&nbsp;-
                                        <html:text tabindex="390" property="payEeacc" styleId="payEeacc" styleClass="textinput" size="14" maxlength="14"
                                        	onblur="if(Trim(asc(this.value))!=''){this.value=leftPad(asc(this.value), 14, '0')};"/>
                                        <html:hidden styleId="payBankIdBranchId" property="payBankIdBranchId"/>
                                        <span id="bankNameDiv" style="display: none;">
                                            (<font color="#8c8c8c" id="bankName"></font>)
                                        </span>
                                    </td>
                                    <td id="iss">
                                        <span class="issuetitle_L_down">　帳號複驗：</span>
                                        <html:text tabindex="400" property="chkPayBankId" styleId="chkPayBankId" styleClass="textinput" size="1" maxlength="3"
                                        	onblur="this.value=asc(this.value);"/>&nbsp;-
                                        <html:text tabindex="405" property="chkBranchId" styleId="chkBranchId" styleClass="textinput" size="1" maxlength="4"
                                        	onblur="this.value=asc(this.value);"/>&nbsp;-
                                        <html:text tabindex="410" property="chkPayEeacc" styleId="chkPayEeacc" styleClass="textinput" size="14" maxlength="14"
                                        	onblur="if(Trim(asc(this.value))!=''){this.value=leftPad(asc(this.value), 14, '0')};"/>
                                ...
                                <tr>
                                    <td id="iss" colspan="2">
                                        ...
                                        <html:text tabindex="411" property="accName" styleId="accName" styleClass="textinput" size="50" maxlength="50"
                                        	onblur="this.value=asc(this.value);"/>
                                    </td>
                                </tr>
                                </c:if>
                            ...
                            <display:table name="pageScope.list" id="listItem" pagesize="<%=ConstantKey.LIST_PAGE_SIZE%>">
                                ...
                                <display:column title="操作區" style="width:16%; text-align:center;">
                                    <acl:checkButton buttonName="btnUpdate">
                                        <input type="button" name="btnUpdate" class="button" value="修　改"
                                        	onclick="loadBenData('<c:out value="${listItem.bafamilytempId}" />', '<c:out value="${listItem.seqNo}" />', <c:out value="${listItem_rowNum}" />);" />&nbsp;
                                    </acl:checkButton>
                                    <acl:checkButton buttonName="btnDeleteBen">
                                        <input type="button" name="btnDeleteBen" class="button" value="刪　除"
                                        	onclick="$('benPage').value='1'; $('benMethod').value='doDeleteBenData'; $('seqNo').value='<c:out value="${listItem.seqNo}" />'; if (confirm('<bean:message bundle="<%=Global.BA_MSG%>" key="msg.delete.confirm"/>')){document.SurvivorAnnuityWalkInReceiptBenForm.submit();}else{return false;}" />
                                    </acl:checkButton>
                                    <input type="hidden" id="checkMk_${listItem_rowNum}" class="checkMk" value="${listItem.checkMk }">
                                </display:column>
                            </display:table>
                        </td>
                    </tr>
