...
import tw.gov.bli.ba.rpt.forms.DataUpdateRpt05Form;
 ...
 * @author swim
 */
public class DataUpdateRpt05Action extends BaseDispatchAction {
    private static Log log = LogFactory.getLog(DataUpdateRpt05Action.class);
    
    private RptService rptService;

    /**
     * 列印作業 - 更正作業相關報表 - 止付單 - 查詢頁面 (balp0d150l.jsp) <br>
     * 按下「列印」的動作
     * 
     * @param mapping
     * @param form
    ...
    public ActionForward doReport(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 列印作業 - 更正作業相關報表 - 止付單 - 查詢頁面 DataUpdateRpt05Action.doReport() 開始 ... ");

        HttpSession session = request.getSession();
        UserBean userData = (UserBean) UserSessionHelper.getUserData(request);

        DataUpdateRpt05Form queryForm = (DataUpdateRpt05Form) form;
        ...
        try {
            ...
            // 抓報表資料
            List<DataUpdateRpt05Case> caseData = rptService.getDataUpdateRpt05DataBy(apNo1+apNo2+apNo3+apNo4);
            ...
            // 設定查詢失敗訊息
            log.error("產製 止付單 發生錯誤:" + ExceptionUtility.getStackTrace(e));
