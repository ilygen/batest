...
import tw.gov.bli.ba.rpt.forms.MonthlyRpt13Form;
 ...
 * @author swim
 */
public class MonthlyRpt13Action extends BaseDispatchAction {
    private static Log log = LogFactory.getLog(MonthlyRpt02Action.class);

    private RptService rptService;

    /**
     * 列印作業 - 月核定處理相關報表 - 改匯核定清單 - 查詢頁面 (balp0d340l.jsp) <br>
     * 按下「列印」的動作
     * 
     * @param mapping
     * @param form
    ...
    public ActionForward doReport(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 列印作業 - 月核定處理相關報表 - 改匯核定清單 - 查詢頁面 MonthlyRpt13Action.doReport() 開始 ... ");

        HttpSession session = request.getSession();
        UserBean userData = (UserBean) UserSessionHelper.getUserData(request);

        MonthlyRpt13Form queryForm = (MonthlyRpt13Form) form;
        ...
        try {
            ...
            // 抓報表資料
            List<MonthlyRpt13Case> caseDataList = rptService.getMonthlyRpt13DataBy(payCode, DateUtility.changeChineseYearMonthType(issuYm), DateUtility.changeDateType(chkDate));
            List<MonthlyRpt13Case> caseDataFooter = rptService.getMonthlyRpt13DataFooterBy(payCode, DateUtility.changeChineseYearMonthType(issuYm), DateUtility.changeDateType(chkDate));
            // 報表列印資料
            List<MonthlyRpt13Case> reportDataList = new ArrayList<MonthlyRpt13Case>();
            ...
            if ((ConstantKey.BAAPPBASE_PAGE_PAYKIND_K).equals(payCode) || (ConstantKey.BAAPPBASE_PAGE_PAYKIND_S).equals(payCode)) {// 失能
                ...
                if (caseDataList.size() > 0) {
                    ...
                    // 【NACHGMK is null】
                    for (MonthlyRpt13Case caseData : caseDataList) {
            ...
            if (caseDataList.size() == 0) {
                ...
                response.setHeader("Content-disposition", "attachment; filename=\"" + Encode.forJava(payCode) + "_MonthlyRpt13_" + Encode.forJava(printDate) + ".pdf" + "\"");
            ...
            // 設定查詢失敗訊息
            log.error("產製 改匯核定清單 發生錯誤:" + ExceptionUtility.getStackTrace(e));
