 ...
 * @author Goston
 */
public class MonthlyRptS18Report extends ReportBase {
    private String printDate = ""; // 印表日期

    public MonthlyRptS18Report() throws Exception {
        super();
    ...
    public Table addHeader(MonthlyRpt18Case caseData, String payCode) throws Exception {
        ...
        // addColumn(table, 20, 1, "給付別：" + caseData.getPayCodeString(), fontCh12, 0, LEFT);
    ...
    public void addFooter(Table table, BigDecimal lendAccountingTitleAmt, BigDecimal loanAccountingTitleAmt) throws Exception {
        ...
        addColumn(table, 48, 1, "借：110212  0051  4銀行存款－土銀台北分行１０１６３－６帳戶", fontCh12, 1, nBorderLeftStyle, LEFT);
        addColumn(table, 12, 1, formatBigDecimalToInteger(lendAccountingTitleAmt), fontCh12, 1, nBorderRightStyle, RIGHT);
        // ---
        addColumn(table, 48, 1, "　　貸：2825122 29    4暫收及待結轉帳項－醫─暫收款", fontCh12, 1, nBorderLeftStyle, LEFT);
    ...
    public ByteArrayOutputStream doReport(UserBean userData, List<MonthlyRpt18Case> caseList, HashMap<String, Object> map) throws Exception {
        try {
            ...
            printToDisk((PropertyHelper.getProperty("rpt.path") + Encode.forJava(payCode) + "_MonthlyRptS18_" + Encode.forJava(printDate) + ".pdf"), bao);
        }
        finally {
            document.close();
        }
