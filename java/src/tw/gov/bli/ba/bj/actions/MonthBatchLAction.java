...
import tw.gov.bli.ba.bj.forms.MonthBatchLForm;
 ...
 * @author Noctis
 */
public class MonthBatchLAction extends BaseDispatchAction {
    private static Log log = LogFactory.getLog(MonthBatchLAction.class);
    ...
    /**
     * 批次處理 - 批次月處理作業 - 老年年金批次月處理作業 - 查詢頁面 (baba0m050x.jsp) 按下 確認
     * 
     * @param mapping
     * @param form
     * @param request
    ...
    public ActionForward doConfirm(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 批次處理 - 批次月處理作業 - 老年年金批次月處理作業 - 查詢頁面  MonthBatchLAction.doQuery() 開始 ... ");

        HttpSession session = request.getSession();
        UserBean userData = (UserBean) UserSessionHelper.getUserData(request);
        MonthBatchLForm iform = (MonthBatchLForm) form;
        ...
        try {
            ...
            // log.debug("批次處理 - 批次月處理作業 - 老年年金批次月處理作業 - 查詢頁面 MonthBatchLAction.doConfirm() 完成 ... ");
            ...
            if (iform.getProcType().equals("2")) {
                ...
                log.debug("執行 批次月處理作業 - 老年年金批次月處理作業 - 呼叫批次排程編審結果... " + returnCode);
            ...
            if (returnCode.equals("0")) {
                ...
                scheduleMsg = "老年年金" + iform.getIssuYm() + "核定年月批次月處理作業(月處理類型(" + procTypeStr + "))";

                // 批次作業完成後，直接將批次月處理條件帶入[進度查詢_BABA0M050X]頁面查明，
                List<MonthBatchLCase> dataList = bjService.selectMonthBatchQueryDataListBy(payCode, null, DateUtility.getNowWestDate(), iform.getProcType());

                // 將 查詢結果清單 存到 Request Scope
                CaseSessionHelper.setMonthBatchLCaseList(dataList, request);
            ...
            // 設定查詢失敗訊息
            log.error("MonthBatchLAction.doConfirm() 發生錯誤:" + ExceptionUtility.getStackTrace(e));
    ...
    /**
     * 批次處理 - 批次月處理作業 - 老年年金批次月處理作業 - 查詢頁面 (baba0m050x.jsp) 按下 進度查詢
     * 
     * @param mapping
     * @param form
     * @param request
    ...
    public ActionForward doQuery(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            List<MonthBatchLCase> dataList = bjService.selectMonthBatchQueryDataListBy(payCode, DateUtility.changeChineseYearMonthType(iform.getIssuYm()), null, iform.getProcType());

            if (dataList.size() < 1) {

                saveMessages(session, DatabaseMessageHelper.getNoResultMessage());
                log.debug("批次處理 - 批次月處理作業 - 老年年金批次月處理作業 - 查詢頁面  MonthBatchLAction.doQuery() 完成 ... ");
                return mapping.findForward(ConstantKey.FORWARD_QUERY_FAIL);
            ...
            // 設定查詢失敗訊息
            log.error("MonthBatchLAction.doQuery() 發生錯誤:" + ExceptionUtility.getStackTrace(e));
            saveMessages(session, DatabaseMessageHelper.getQueryFailMessage());
            return mapping.findForward(ConstantKey.FORWARD_FAIL);
        }
    }
    ...
    /**
     * 批次處理 - 批次月處理作業 - 老年年金批次月處理作業 - 查詢頁面 (baba0m051q.jsp) 按下 檔案下載
     * 
     * @param mapping
     * @param form
     * @param request
    ...
    public ActionForward doDownLoad(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 批次處理 - 批次月處理作業 - 老年年金批次月處理作業 - 查詢頁面  MonthBatchLAction.doDownLoad() 開始 ... ");
        ...
        try {
            ...
            // 設定查詢失敗訊息
            log.error("MonthBatchLAction.doDownLoad() 發生錯誤:" + ExceptionUtility.getStackTrace(e));
            saveMessages(session, DatabaseMessageHelper.getQueryFailMessage());
            return mapping.findForward(ConstantKey.FORWARD_FAIL);
        }
    }
    ...
    /**
     * 批次處理 - 批次月處理作業 - 老年年金批次月處理作業 - 查詢頁面 (baba0m051q.jsp) 按下 狀態明細
     * 
     * @param mapping
     * @param form
     * @param request
    ...
    public ActionForward doDetail(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 批次處理 - 批次月處理作業 - 老年年金批次月處理作業 - 資料頁面  MonthBatchLAction.doDetail() 開始 ... ");
        ...
        try {
            ...
            if (dataList.size() < 1) {
                ...
                saveMessages(session, DatabaseMessageHelper.getNoResultMessage());
                log.debug("批次處理 - 批次月處理作業 - 老年年金批次月處理作業 - 資料頁面  MonthBatchLAction.doDetail() 完成 ... ");
                return mapping.findForward(FORWARD_QUERY_LIST_FAIL);
            ...
            // 設定查詢失敗訊息
            log.error("MonthBatchLAction.doDetail() 發生錯誤:" + ExceptionUtility.getStackTrace(e));
            saveMessages(session, DatabaseMessageHelper.getQueryFailMessage());
            return mapping.findForward(ConstantKey.FORWARD_FAIL);
        }
    }
    ...
    /**
     * 批次處理 - 批次月處理作業 - 老年年金批次月處理作業 - 資料頁面 (baba0m111q.jsp,baba0m112a.jsp) 按下 返回
     * 
     * @param mapping
     * @param form
     * @param request
    ...
    public ActionForward doBack(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 批次處理 - 批次月處理作業 - 老年年金批次月處理作業 - 返回 MonthBatchLAction.doBack() 開始 ... ");
        ...
        iform.setIssuYm("");
        iform.setProcType("");
        CaseSessionHelper.removeMonthBatchLCaseList(request);

        log.debug("執行批次處理 - 批次月處理作業 - 老年年金批次月處理作業 - 返回 MonthBatchLAction.doBack() 完成 ... ");
