...
import tw.gov.bli.ba.rpt.forms.OtherRpt01Form;
import tw.gov.bli.ba.rpt.report.OtherRpt01Report;
 ...
 * @author Goston
 */
public class OtherRpt01Action extends BaseDispatchAction {
    private static Log log = LogFactory.getLog(OtherRpt01Action.class);

    private RptService rptService;

    /**
     * 列印作業 - 其他類報表 - 調卷/還卷單 - 查詢頁面 (balp0d510l.jsp) <br>
     * 按下「列印」的動作
     * 
     * @param mapping
     * @param form
    ...
    public ActionForward doReport(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        log.debug("執行 列印作業 - 其他類報表 - 調卷/還卷單 - 查詢頁面 OtherRpt01Action.doReport() 開始 ... ");

        HttpSession session = request.getSession();
        UserBean userData = (UserBean) UserSessionHelper.getUserData(request);

        OtherRpt01Form queryForm = (OtherRpt01Form) form;
        ...
        try {
            ...
            // 抓報表資料
            OtherRpt01Case caseData = rptService.getOtherRpt01DataBy(apNo);
            ...
            if (caseData == null) {
                ...
                OtherRpt01Report report = new OtherRpt01Report();
                ...
                response.setHeader("Content-disposition", "attachment; filename=\"OtherRpt01_" + Encode.forJava(apNo) + ".pdf" + "\"");
            ...
            // 設定查詢失敗訊息
            log.error("產製 調卷/還卷單 發生錯誤:" + ExceptionUtility.getStackTrace(e));
