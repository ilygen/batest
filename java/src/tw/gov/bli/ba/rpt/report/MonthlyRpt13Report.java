 ...
 * @author Rickychi
 */
public class MonthlyRpt13Report extends ReportBase {

    private RptService rptService;

    public MonthlyRpt13Report() throws Exception {
        super();
    }
    ...
    public Table getHeader(boolean bPrintMainHeader, String payCode, String payTypStr, String chkDate, String pageNum, String cPrnDate, String naChgMk, String nlWkMk, String adWkMk, String isNaChgMk) throws Exception {
        document.newPage();
        // 建立表格
        ...
        if (bPrintMainHeader) {
            ...
            addColumn(table, 99, 1, "改匯核定清單", fontCh10, 0, CENTER);
            if (!("").equals(payTypStr)) {
    ...
    public Table addFooter(Table table, String payCode, String payTyp, BigDecimal tissueAmt, BigDecimal tpayAmt, BigDecimal toffsetAmt, BigDecimal tmitRate) throws Exception {
        table.setBorderWidth(1);

        int rowCount = 0;
        addColumn(table, 86, 1, "借：2822121 11111 0 應付保管款－普通－老年給付（年金） ", fontCh10, 0, LEFT);
        addColumn(table, 13, 1, formatBigDecimalToInteger(tissueAmt), fontCh10, 0, RIGHT);
        ...
        // if (("L").equals(payCode)) {
        // addColumn(table, 30, 1, "生老給付科", fontCh10, 0, LEFT);
        // }
        // else if (("K").equals(payCode)) {
        // addColumn(table, 30, 1, "傷殘給付科", fontCh10, 0, LEFT);
        // }
        // else if (("S").equals(payCode)) {
        // addColumn(table, 30, 1, "死亡給付科", fontCh10, 0, LEFT);
        // }
        // else {
    ...
    public ByteArrayOutputStream doReport(UserBean userData, List<MonthlyRpt13Case> caseData, List<MonthlyRpt13Case> caseDataFooter, HashMap<String, Object> map) throws Exception {
        try {
            ...
            // 2010.12.06 Kiyomi add 輸出報表檔至主機上
            String printDate = DateUtility.getNowChineseDate();
            // FileOutputStream bao = new FileOutputStream(PropertyHelper.getProperty("rpt.path") + Encode.forJava(payCode) + "_MonthlyRpt13_" + Encode.forJava(printDate) + ".pdf");
            // FileOutputStream bao = new FileOutputStream("E:\\"+"MonthlyRpt13_" + printDate + ".pdf");
            ...
            Map<String, List<MonthlyRpt13Case>> dataMap = new TreeMap<String, List<MonthlyRpt13Case>>();
            Map<String, List<MonthlyRpt13Case>> dataMapFooter = new TreeMap<String, List<MonthlyRpt13Case>>(); // 20120406
            for (int i = 0; i < caseData.size(); i++) {
                MonthlyRpt13Case obj = caseData.get(i);
                dataMap.put(obj.getPayTyp(), new ArrayList<MonthlyRpt13Case>());
            }
            ...
            for (int i = 0; i < caseData.size(); i++) { // 20120416
                MonthlyRpt13Case obj = caseData.get(i);
                dataMapFooter.put(obj.getPayTyp(), new ArrayList<MonthlyRpt13Case>());
            }

            for (int i = 0; i < caseDataFooter.size(); i++) { // 20120416
                MonthlyRpt13Case obj = caseDataFooter.get(i);
            ...
            for (Iterator it = dataMap.keySet().iterator(); it.hasNext();) {
                ...
                List<MonthlyRpt13Case> caseList = dataMap.get(key);
                MonthlyRpt13Case tmpCaseObj = caseList.get(0);

                // 拿取頁次 20130731
                List<MonthlyRpt13Case> caseListPageNum = dataMapFooter.get(key);
                ...
                for (MonthlyRpt13Case caseObj : caseList) {
                ...
                // table = addFooter(table, payCode, key, tissueAmt, tpayAmt, toffsetAmt, tmitRate);
                // 20120406 begin
                table.setBorderWidth(1);
                int rowCount = 0;
                List<MonthlyRpt13Case> caseList1 = dataMapFooter.get(key);
                for (MonthlyRpt13Case caseObj1 : caseList1) {
