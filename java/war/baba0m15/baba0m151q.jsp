...
<html:html lang="true">
<head>
    <acl:setProgId progId="BABA0M151Q" />
                        ...
                        <display:table name="pageScope.list" id="listItem" pagesize="<%=ConstantKey.LIST_PAGE_SIZE%>">                            
                            ...
                            <display:column title="檔案下載" headerClass="issuetitle_L" class="issueinfo_C" style="width:8%; text-align:center;">
                                <acl:checkButton buttonName="btnDetail">
                                   <input type="button" name="btnDetail" class="button" value="檔案下載" onclick="$('page').value='1'; $('method').value='doDownload';  $('mfileName').value='<c:out value="${listItem.mfileName}" />';   document.DisabledMediaBatchForm.submit();" />&nbsp;
                                 </acl:checkButton>                            
                            </display:column>
                            <display:column title="傳送" headerClass="issuetitle_L" class="issueinfo_C" style="width:10%; text-align:center;">
                                <acl:checkButton buttonName="btnDetail">
                                    <input type="button" name="btnDetail" class="button" value="傳送" onclick="$('page').value='1'; $('method').value='doUploadFile';$('mfileName').value='<c:out value="${listItem.mfileName}" />';$('mfileDate').value='<c:out value="${listItem.mfileDate}" />';$('ftpSeq').value='<c:out value="${listItem.ftpSeq}" />'; document.DisabledMediaBatchForm.submit();" />&nbsp;
                                </acl:checkButton>
                            </display:column>
